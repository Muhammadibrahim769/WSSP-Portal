...
dataSource = null;
  ...
  factorCode:any[]=[];
  constructor(private formBuilder: FormBuilder,private factor:BillingService) { }
 factorColumns: string[] = [ 'numFactorCode', 'txtFactorName','txtDescription'
//  , 'Actions'
];
...
factorDataList:any[]=[];
factorReportDTOList:any[]=[];
pdfImplementation() {
    debugger;
    this.factor.getFactorFindAll().subscribe(Response=>{
    this.factorDataList = Response['data'];
    console.log("daownload data ")
    console.log(this.factorDataList)
    debugger;
    this.factorReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Factor Code', bold: true },
      { text: 'Factor Name', bold: true },
      { text: 'Dexcription', bold: true }    
    ]);
    this.factorDataList.forEach((factorDataList, i) => {
      debugger;
      this.factorReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: factorDataList.txtFactorCode, alignment: 'left' },
        { text: factorDataList.txtFactorName, alignment: 'left' },
        { text: factorDataList.txtDescription, alignment: 'left' }      
      ]);
    });  
  })
}
downloadPDF() {  
  ...
  this.factorReportDTOList = [];
  let tableHeader: any[] = []
  this.selection.value.forEach(element => {
    if (element === "sr") {
      tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
    } else if (element === "numFactorCode") {
      tableHeader.push({ text: 'Factor Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
    } else if (element === "txtFactorName") {
      tableHeader.push({ text: 'Factor Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
    } else if (element === "txtDescription") {
      tableHeader.push({ text: 'Description', bold: true, fillColor: '#dddddd', alignment: 'center' });
    } 
  });
  this.factorReportDTOList.push(tableHeader);
  console.log(this.factorReportDTOList);
  this.factorDataList.forEach((factorDataList, i) => {
    debugger;
    let tableRows: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        ...
        tableRows.push({ text: factorDataList.txtFactorCode, alignment: 'left' });
      } else if (element === "txtFactorName") {
        tableRows.push({ text: factorDataList.txtFactorName, alignment: 'left' });
      } else if (element === "txtDescription") {
        tableRows.push({ text: factorDataList.txtDescription, alignment: 'left' });
      } 
    });
    this.factorReportDTOList.push(tableRows)
  });
  debugger;
    ...
    content: [
      {
        ...
        table: {
          ...
          body: [
            ...
            }, [{ text: 'Factor Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
            {
  ...
  pdfMake.createPdf(documentDefinition).download('Factor List.pdf');
}
tableColumn: string[] = [
  "sr",
  "numFactorCode",
