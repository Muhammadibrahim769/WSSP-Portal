import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatTableDataSource } from '@angular/material/table';
import { MatSort } from '@angular/material/sort';
import { MatPaginator } from '@angular/material/paginator';
import { BillingService } from '@app/services/billing.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { Router } from '@angular/router';
import { CrudService } from '@app/_services/crud.service';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './kpiList.component.html'
})
export class KPIListComponent implements OnInit {
  kpiForm: FormGroup;
  step = 0;
  selectkpiList: any[] = [];
  kpiDataList: any[] = [];
  kpiReportDTOList: any[] = [];
  searchKPIData: any;
  isLoading = true;
  dataSource = null;
  constructor(private formBuilder: FormBuilder, private kpi: BillingService, private router: Router, private shared: CrudService) { }
  kpiColumns: string[] = ['txtperformanceAreaName', 'txtperformanceAreaCode', 'numkpiCode', 'txtKpiName', 'numMinValue', 'numMaxValue', 'txtFormula', 'txtUom', 'txtCostCenter', 'Actions'];
  kpiListData = new MatTableDataSource();
  @ViewChild("kpiSort", { static: true }) kpiSort: MatSort;
  @ViewChild("kpiPaginator", { static: true }) kpiPaginator: MatPaginator;
  ngOnInit(): void {
    this.kpiForm = this.formBuilder.group({
      txtAreaCode: [''],
      txtAreaName: [""],
      txtKPICode: [''],
      txtKPIName: [''],
      txtCostCenterName: ['']
    })
    this.getCostCentreName();
    this.onGetKPIByFindAll();
    this.pdfImplementation();    
  }
  CostCentre: any;
  getCostCentreName() {
    debugger;
    this.kpi.getCostCentreName().subscribe(response => {
      debugger;
      this.CostCentre = response;
    })
  }
  onSearch() {
    let txtAreaCode = this.kpiForm.controls.txtAreaCode.value;
    let txtAreaName = this.kpiForm.controls.txtAreaName.value;
    let txtKPICode = this.kpiForm.controls.txtKPICode.value;
    let txtKPIName = this.kpiForm.controls.txtKPIName.value;
    let txtCostCenterName = this.kpiForm.controls.txtCostCenterName.value;
    this.kpi.getKpiFindAllByParam(txtAreaCode, txtAreaName, txtKPICode, txtKPIName, txtCostCenterName).subscribe(Response => {
      this.searchKPIData = Response["data"];
      this.isLoading = false;
      this.dataSource = Response["data"];
      this.selectkpiList = [];
      this.kpiListData = new MatTableDataSource(Response["data"]);
      this.kpiListData.sort = this.kpiSort;
      this.kpiListData.paginator = this.kpiPaginator;
      this.pdfImplementation();
    })
  }
  onGetKPIByFindAll() {
    this.kpi.getKpiFindAll().subscribe(Response => {      
      this.searchKPIData = Response["data"];
      console.log("response")
      console.log(this.searchKPIData)
      this.isLoading = false;
      this.dataSource = Response["data"];
      this.selectkpiList = [];
      this.kpiListData = new MatTableDataSource(Response["data"]);
      this.kpiListData.sort = this.kpiSort;
      this.kpiListData.paginator = this.kpiPaginator;
      this.pdfImplementation();
    })
  }
  viewKPI: any;
  onViewKPI(element) {
    this.viewKPI = element;
    this.shared.changeMessage(this.viewKPI);
    this.router.navigate(['/maintenanceAndEveluation/viewKPI']);
  }
  onReset() {
    this.kpiForm.controls.txtAreaCode.reset('')
    this.kpiForm.controls.txtAreaName.reset('')
    this.kpiForm.controls.txtKPICode.reset('')
    this.kpiForm.controls.txtKPIName.reset('')
    this.kpiForm.controls.txtCostCenterName.reset('')
    this.onGetKPIByFindAll();
  }
  pdfImplementation() {
  
    this.kpiDataList = this.searchKPIData;
  
    this.kpiReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Performance Area Name', bold: true },
      { text: 'Performance Area Code', bold: true },
      { text: 'KPI Code', bold: true },
      { text: 'KPI Name', bold: true },
      { text: 'Min Value', bold: true },
      { text: 'Max Value', bold: true },
      { text: 'Formula', bold: true },
      { text: 'UOM', bold: true },
      { text: 'Cost Center', bold: true },
    ]);
    this.kpiDataList.forEach((kpiDataList, i) => {
    
      this.kpiReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: kpiDataList.txtAreaName, alignment: 'left' },
        { text: kpiDataList.txtAreaCode, alignment: 'left' },
        { text: kpiDataList.txtKPICode, alignment: 'left' },
        { text: kpiDataList.txtKPIName, alignment: 'left' },
        { text: kpiDataList.numMinValue, alignment: 'right' },
        { text: kpiDataList.numMaxValue, alignment: 'right' },
        { text: kpiDataList.txtFormula, alignment: 'right' },
        { text: kpiDataList.txtUom, alignment: 'right' }
      ])
    })
  }
  downloadPDF() {

    this.kpiReportDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtperformanceAreaName") {
        tableHeader.push({ text: 'Performance Area Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtperformanceAreaCode") {
        tableHeader.push({ text: 'Performance Area Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numkpiCode") {
        tableHeader.push({ text: 'KPI Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtKpiName") {
        tableHeader.push({ text: 'KPI Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numMinValue") {
        tableHeader.push({ text: 'Min Value', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numMaxValue") {
        tableHeader.push({ text: 'Max Value', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtFormula") {
        tableHeader.push({ text: 'Formula', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtUom") {
        tableHeader.push({ text: 'UOM', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCostCenter") {
        tableHeader.push({ text: 'Cost Center', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.kpiReportDTOList.push(tableHeader);
    this.kpiDataList.forEach((kpiDataList, i) => {
    
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtperformanceAreaName") {
          tableRows.push({ text: kpiDataList.txtAreaName, alignment: 'left' });
        } else if (element === "txtperformanceAreaCode") {
          tableRows.push({ text: kpiDataList.txtAreaCode, alignment: 'left' });
        } else if (element === "numkpiCode") {
          tableRows.push({ text: kpiDataList.txtKPICode, alignment: 'left' });
        } else if (element === "txtKpiName") {
          tableRows.push({ text: kpiDataList.txtKPIName, alignment: 'left' });
        } else if (element === "numMinValue") {
          tableRows.push({ text: kpiDataList.numMinValue, alignment: 'left' });
        } else if (element === "numMaxValue") {
          tableRows.push({ text: kpiDataList.numMaxValue, alignment: 'left' });
        } else if (element === "txtFormula") {
          tableRows.push({ text: kpiDataList.txtFormula, alignment: 'left' });
        } else if (element === "txtUom") {
          tableRows.push({ text: kpiDataList.txtUom, alignment: 'left' });
        } else if (element === "txtCostCenter") {
          tableRows.push({ text: kpiDataList.txtCostCenterName, alignment: 'left' });
        }
      });
      this.kpiReportDTOList.push(tableRows)
    });
    

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'KPI Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.kpiReportDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('KPI List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "txtperformanceAreaName",
    "txtperformanceAreaCode",
    "numkpiCode",
    "txtKpiName",
    "numMinValue",
    "numMaxValue",
    "txtFormula",
    "txtUom",
    "txtCostCenter"
  ];
  selection = new FormControl([
    "sr",
    "txtperformanceAreaName",
    "txtperformanceAreaCode",
    "numkpiCode",
    "txtKpiName",
    "numMinValue",
    "numMaxValue",
    "txtFormula",
    "txtUom",
    "txtCostCenter"
  ]);
  selectingColumns() {

    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
