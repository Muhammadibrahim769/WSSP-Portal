import { Component, OnInit, ViewChild } from '@angular/core';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { MatTableDataSource } from '@angular/material/table';
import { MatSort } from '@angular/material/sort';
import { MatPaginator } from '@angular/material/paginator';
import { BillingService } from '@app/services/billing.service';
import { MatDatepicker } from '@angular/material/datepicker';
import { MomentDateAdapter, MAT_MOMENT_DATE_ADAPTER_OPTIONS } from '@angular/material-moment-adapter';
import { DateAdapter, MAT_DATE_FORMATS, MAT_DATE_LOCALE } from '@angular/material/core';
import * as _moment from 'moment';
import { Moment } from 'moment';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';

import { MatDialog } from '@angular/material/dialog';
import { ChartModalComponent } from '@app/modals/chartModal.component';

const moment = _moment;
export const MY_FORMATS = {
  parse: {
    dateInput: 'MMM-y',
  },
  display: {
    dateInput: 'MMM-y',
    monthYearLabel: 'MMM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MMMM YYYY',
  },
};
@Component({
  selector: 'app-dashboard',
  templateUrl: './kpidashboard.component.html',
  providers: [
    {
      provide: DateAdapter,
      useClass: MomentDateAdapter,
      deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS]
    },

    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class KpidashboardComponent implements OnInit {

  factorForm: FormGroup;
  step = 0;
  isLoading: boolean;
  dataSource = null;
  selectFactorList: any[] = [];
  searchFactorData: any;
  performanceArea: any[] = [];
  performanceAreaName: any[] = []
  kpiDashboardData: any[] = [];
  kpiDashboardReportDTO: any[] = [];
  factorCode: any[] = [];
  kpiName: any[] = [];
  constructor(private formBuilder: FormBuilder, private kpiDashboard: BillingService,private dateFormat: DateFormatPipe, public dialog: MatDialog) { }
  kpiDashboardColumns: string[] = ['txtAreaName', 'txtAreaCode', 'txtKPIName', 'txtKPICode', 'numMinValue', 'numMaxValue',  'txtUom', 'zoneA', 'zoneB', 'zoneC', 'zoneD', 'headOffice', 'Total', 'Actions'
    //  , 'Actions''txtFormula',
  ];
  kpiDashboardList = new MatTableDataSource();
  @ViewChild("kpiDashboardSort", { static: true }) kpiDashboardSort: MatSort;
  @ViewChild("kpiDashboardPaginator", { static: true }) kpiDashboardPaginator: MatPaginator;
  ngOnInit(): void {
    this.factorForm = this.formBuilder.group({
      serFactorId: [''],
      serPerformanceAreaId: [''],
      txtKPICode: [""],
      txtCostCenterName: [""],
      txtAreaCode: [''],
      txtAreaName: [''],
      txtKPIName: [''],
      blnStatus: [true],
      txtMonth: [moment()],
    })
    this.onGetFactorCode();
    this.onGetPerformanceArea();
    this.getCostCentreName()
  }
  CostCentre: any;
  getCostCentreName() {
    this.kpiDashboard.getCostCentreName().subscribe(response => {
      this.CostCentre = response;
    })
  }
  onSearch() {
    this.isLoading = true;
    let txtAreaCode = this.factorForm.controls.txtAreaCode.value;
    let txtAreaName = this.factorForm.controls.txtAreaName.value;
    let txtKPICode = this.factorForm.controls.txtKPICode.value;
    let txtKPIName = this.factorForm.controls.txtKPIName.value;
    let txtCostCenterName = this.factorForm.controls.txtCostCenterName.value;
    let txtMonth = this.dateFormat.transform(this.factorForm.controls.txtMonth.value);
    this.kpiDashboard.getKpiFindAllForMonth(txtAreaCode, txtAreaName, txtKPICode, txtKPIName, txtCostCenterName, txtMonth).subscribe(Response => {
      this.searchFactorData = Response["data"];
      this.isLoading = false;
      this.dataSource = Response["data"];
      this.selectFactorList = [];
      this.kpiDashboardList = new MatTableDataSource(Response["data"]);
      this.kpiDashboardList.sort = this.kpiDashboardSort;
      this.kpiDashboardList.paginator = this.kpiDashboardPaginator;
      console.log(this.searchFactorData)
      this.pdfImplementation();

    })
  }
  onGetFactorCode() {
    this.kpiDashboard.getFactorFindAll().subscribe(Response => {
      this.factorCode = Response["data"];
    })
  }  
  onGetPerformanceArea() {
    this.kpiDashboard.getPerformanceAreaService().subscribe(Response => {
      this.performanceArea = Response["data"];
    })
  }

  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue = this.factorForm.controls.txtMonth.value;
    ctrlValue.year(normalizedYear.year());
    this.factorForm.controls.txtMonth.setValue(ctrlValue);
  }

  chosenMonthHandler(normalizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
    const ctrlValue = this.factorForm.controls.txtMonth.value;
    ctrlValue.month(normalizedMonth.month());
    this.factorForm.controls.txtMonth.setValue(ctrlValue);
    console.log("ghjsdkasd");
    console.log(this.factorForm.controls.txtMonth);
    datepicker.close();
  }
  onTableDataKpiDashboard() {
    // this.performanceAreaName.push()
  }
  onRefreshData(){
    this.kpiDashboard.getRefreshData().subscribe(response=>{
      console.log(response);
      if(response.length > 0){
        this.isLoading = false;
        this.kpiDashboardList = new MatTableDataSource(response["data"]);
        this.kpiDashboardList.sort = this.kpiDashboardSort;
        this.kpiDashboardList.paginator = this.kpiDashboardPaginator;
      }else{
        alert('No Record Found');
        this.isLoading = false;
        this.kpiDashboardList = new MatTableDataSource([]);
      }

    },
    (error=>{
      alert(error)
      this.isLoading = false;
    }))
  }
  onReset() { }
  pdfImplementation() {
    debugger;
    this.kpiDashboardData = this.searchFactorData;
    debugger;
    this.kpiDashboardReportDTO.push([
      { text: 'Sr#', bold: true },
      { text: 'Area Name', bold: true },
      { text: 'Area Code', bold: true },
      { text: 'KPI Name', bold: true },
      { text: 'KPI Code', bold: true },
      { text: 'Min Value', bold: true },
      { text: 'Max Value', bold: true },
      { text: 'Formula', bold: true },
      { text: 'UoM', bold: true },
      { text: 'Zone A', bold: true },
      { text: 'Zone B', bold: true },
      { text: 'Zone C', bold: true },
      { text: 'Zone D', bold: true },
      { text: 'HO', bold: true },
      { text: 'Total', bold: true },
    ]);

    this.kpiDashboardData.forEach((kpiDashboardData, i) => {
      debugger;
      this.kpiDashboardReportDTO.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: kpiDashboardData.txtAreaName, alignment: 'left' },
        { text: kpiDashboardData.txtAreaCode, alignment: 'left' },
        { text: kpiDashboardData.txtKPIName, alignment: 'left' },
        { text: kpiDashboardData.txtKPICode, alignment: 'left' },
        { text: kpiDashboardData.numMinValue, alignment: 'left' },
        { text: kpiDashboardData.numMaxValue, alignment: 'left' },
        { text: kpiDashboardData.txtFormula, alignment: 'left' },
        { text: kpiDashboardData.txtUom, alignment: 'left' },
        { text: kpiDashboardData.numBranchA, alignment: 'left' },
        { text: kpiDashboardData.numBranchB, alignment: 'left' },
        { text: kpiDashboardData.numBranchC, alignment: 'left' },
        { text: kpiDashboardData.numBranchD, alignment: 'left' },
        { text: kpiDashboardData.numBranchHO, alignment: 'right' },
        { text: kpiDashboardData.numValue, alignment: 'right' }
      ])
    })
  }
  downloadPDF() {
    debugger;
    this.kpiDashboardReportDTO = [];
    let tableHeader: any[] = []   
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtAreaName") {
        tableHeader.push({ text: ' Area Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtAreaCode") {
        tableHeader.push({ text: 'Area Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtKPIName") {
        tableHeader.push({ text: 'KPI Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtKPICode") {
        tableHeader.push({ text: ' KPI Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numMinValue") {
        tableHeader.push({ text: 'Min Value', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numMaxValue") {
        tableHeader.push({ text: 'Max Value', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtFormula") {
        tableHeader.push({ text: ' Formula', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtUom") {
        tableHeader.push({ text: 'UOM', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "zoneA") {
        tableHeader.push({ text: 'Zone A', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "zoneB") {
        tableHeader.push({ text: 'Zone B', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "zoneC") {
        tableHeader.push({ text: 'Zone C', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "zoneD") {
        tableHeader.push({ text: 'Zone D', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "headOffice") {
        tableHeader.push({ text: 'HO', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "Total") {
        tableHeader.push({ text: 'Total', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.kpiDashboardReportDTO.push(tableHeader);
    this.kpiDashboardData.forEach((kpiDashboardData, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtAreaName") {
          tableRows.push({ text: kpiDashboardData.txtAreaName, alignment: 'left' });
        } else if (element === "txtAreaCode") {
          tableRows.push({ text: kpiDashboardData.txtAreaCode, alignment: 'left' });
        } else if (element === "txtKPIName") {
          tableRows.push({ text: kpiDashboardData.txtKPIName, alignment: 'left' });
        } else if (element === "txtKPICode") {
          tableRows.push({ text: kpiDashboardData.txtKPICode, alignment: 'left' });
        } else if (element === "numMinValue") {
          tableRows.push({ text: kpiDashboardData.numMinValue, alignment: 'left' });
        } else if (element === "numMaxValue") {
          tableRows.push({ text: kpiDashboardData.numMaxValue, alignment: 'left' });
        } else if (element === "txtFormula") {
          tableRows.push({ text: kpiDashboardData.txtFormula, alignment: 'left' });
        } else if (element === "txtUom") {
          tableRows.push({ text: kpiDashboardData.txtUom, alignment: 'left' });
        } else if (element === "zoneA") {
          tableRows.push({ text: kpiDashboardData.numBranchA, alignment: 'left' });
        } else if (element === "zoneB") {
          tableRows.push({ text: kpiDashboardData.numBranchB, alignment: 'left' });
        } else if (element === "zoneC") {
          tableRows.push({ text: kpiDashboardData.numBranchC, alignment: 'left' });
        } else if (element === "zoneD") {
          tableRows.push({ text: kpiDashboardData.numBranchD, alignment: 'left' });
        } else if (element === "headOffice") {
          tableRows.push({ text: kpiDashboardData.numBranchHO, alignment: 'left' });
        } else if (element === "Total") {
          tableRows.push({ text: kpiDashboardData.numValue, alignment: 'left' });
        }
      });
      this.kpiDashboardReportDTO.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'KPI Dashboard', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.kpiDashboardReportDTO
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Kpi Dashboard List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "txtAreaCode",
    "txtAreaName",
    "txtKPIName",
    "txtKPICode",
    "numMinValue",
    "numMaxValue",
    "txtFormula",
    "txtUom",
    "zoneA",
    "zoneB",
    "zoneC",
    "zoneD",
    "headOffice",
    "Total"
  ];
  selection = new FormControl([
    "sr",
    "txtAreaCode",
    "txtAreaName",
    "txtKPIName",
    "txtKPICode",
    "numMinValue",
    "numMaxValue",
    "txtFormula",
    "txtUom",
    "zoneA",
    "zoneB",
    "zoneC",
    "zoneD",
    "headOffice",
    "Total"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }

  openChart(data: any){
    console.log("openChart")
    const document = this.dialog.open(ChartModalComponent, {
      data: {
        data
      }
    });
  }
}
