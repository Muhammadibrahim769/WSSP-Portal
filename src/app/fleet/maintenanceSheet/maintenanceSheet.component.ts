import { Component, ViewChild } from '@angular/core';
import { User } from '@app/_models';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';
import { MatTableDataSource } from '@angular/material/table';
import { MatSort } from '@angular/material/sort';
import { MatPaginator } from '@angular/material/paginator';
import { CrudService } from '@app/_services/crud.service';
import { BillingService } from '@app/services/billing.service';
import { Router, ActivatedRoute } from '@angular/router';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './maintenanceSheet.component.html'
})
export class MaintenanceSheetComponent {
  form: FormGroup;
  user = "";
  ucList: any;
  branchList: any;
  getBranchId = [];
  unionCouncilList: any[] = [];
  assettypeList: any[] = [];
  maintenanceSheetDataList: any;
  selectStatusList: any[] = [];
  maintenanceItem: any[] = []
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  getMaintenanceItemList() {
    debugger;
    this.billingService.getMaintenanceItemList().subscribe(response => {
      debugger;
      this.maintenanceItem = response
    })
  }
  onMaintenanceItemChange(event) {
    debugger;
    this.billingService.getMaintenanceItemList().subscribe(response => {
      debugger;
      this.maintenanceItem.push(response)
      this.maintenanceItem.forEach(element => {
        debugger;
        if (event.value == element.serMaintainanceItemId) {
          debugger;
          this.form.controls["txtItemCode"].patchValue(element.txtItemCode)
          this.form.controls["txtItemName"].patchValue(element.txtItemName)
          this.form.controls["txtUom"].patchValue(element.txtUom)
          this.form.controls["serUomId"].patchValue(element.serUomId)        
        }
      })
    })
  }
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder,
    private billingService: BillingService, private utilsFunctions: UtilsFunctions,
    private shared: CrudService, private route: ActivatedRoute,
    private router: Router, private dateFormat: DateFormatPipe, ) {
  }
  maintenanceSheetColumns: string[] = ['sNo', 'txtBranchName', 'txtLocationName','txtBatchNo', 'txtBatchName',
    'txtItemCode', 'txtUom', 'dteLastMaintenanceDate', 'dteNextMaintenanceDate', 'numLastOdoMeterReading'
    , 'numNextOdoMeterReading', 'numLastMaintenancequantity', 'numLastMaintenancePrice'];
  maintenanceSheetList = new MatTableDataSource();
  @ViewChild("maintenanceSheetListSort", { static: true }) maintenanceSheetListSort: MatSort;
  @ViewChild("maintenanceSheetListPaginator", { static: true }) maintenanceSheetListPaginator: MatPaginator;
  maintenanceSheetDueInMonth = new MatTableDataSource();
  @ViewChild("maintenanceSheetDueInMonthSort", { static: true }) maintenanceSheetDueInMonthSort: MatSort;
  @ViewChild("maintenanceSheetDueInMonthPaginator", { static: true }) maintenanceSheetDueInMonthPaginator :MatPaginator;
  maintenanceSheetOverDue = new MatTableDataSource();
  @ViewChild("maintenanceSheetOverDueSort", { static: true }) maintenanceSheetOverDueSort: MatSort;
  @ViewChild("maintenanceSheetOverDuePaginator", { static: true }) maintenanceSheetOverDuePaginator: MatPaginator;
 
  ngOnInit() {
    debugger;
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      txtBatchNo: [''],
      txtItemCode: [''],
      dteDateFrom: [''],
      dteDateTo: [''],
      serBranchId: [''],
      serLocationId: [''],
      txtBranchName:[''],
      txtLocationName:[''],
      txtItemName: [''],
      txtProductCode: [''],
      txtProductName: [''],
      txtVehicleType: [''],
      serMaintainanceItemId:[],
      txt_product_name:['']
    });
    this.getMaintenanceSheetFindAll();
    this.getMaintenanceDueInMonthFindAll();
    this.getMaintenanceOverDueFindAll();
    this.tabIndex = 0;
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.getDistinctAsset();
    this.getBranchList();
    this.getMaintenanceItemList();
    
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);
    debugger;
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {
      debugger;
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  getDistinctAsset() {
    this.billingService.getDistinctAsset().subscribe(response => {
      console.log("response");
      console.log(response);
      this.assettypeList = response
    })
  }
  maintenanceSheetFindAll:any
  wasteCollectionList: any = "";
  wasteCollectionDTO: any[] = [];
  getMaintenanceDueInMonthFindAll() {
    this.billingService.getMaintenanceDueInMonthFindAll().subscribe(response => {
      this.maintenanceSheetFindAll = response;     
      this.wasteCollectionList = response;
      this.maintenanceSheetDueInMonth = new MatTableDataSource(this.maintenanceSheetFindAll);
      this.maintenanceSheetDueInMonth.sort = this.maintenanceSheetDueInMonthSort;
      this.maintenanceSheetDueInMonth.paginator = this.maintenanceSheetDueInMonthPaginator;
    })
  }
  getMaintenanceOverDueFindAll() {
    this.billingService.getMaintenanceOverDueFindAll().subscribe(response => {
      this.maintenanceSheetFindAll = response;     
      this.wasteCollectionList = response;
      this.maintenanceSheetOverDue = new MatTableDataSource( this.maintenanceSheetFindAll);
      this.maintenanceSheetOverDue.sort = this.maintenanceSheetOverDueSort;
      this.maintenanceSheetOverDue.paginator = this.maintenanceSheetOverDuePaginator;
    })
  }
  getMaintenanceSheetFindAll() {
    this.billingService.getMaintenanceSheetFindAll().subscribe(response => {
      this.maintenanceSheetFindAll = response;     
      console.log(response)
      this.wasteCollectionList = response;
      this.maintenanceSheetList = new MatTableDataSource(this.maintenanceSheetFindAll);
      this.maintenanceSheetList.sort = this.maintenanceSheetListSort;
      this.maintenanceSheetList.paginator = this.maintenanceSheetListPaginator;
    })
  }
  onSearch() {
    debugger;
    console.log(this.form.value)
    let dteDateFrom = '';
    if (this.form.controls.dteDateFrom.value != "") {
      dteDateFrom = this.dateFormat.transformOperation( this.form.controls.dteDateFrom.value)
    }
    let dteDateTo = '';
    if (this.form.controls.dteDateTo.value != "") {
      dteDateTo = this.dateFormat.transformOperation( this.form.controls.dteDateTo.value)
    }
     let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtBatchNo = this.form.controls.txtBatchNo.value;
    let serMaintainanceItemId = this.form.controls.txtItemCode.value;
    let txtItemName = this.form.controls.txtItemName.value;
    let txtProductCode = this.form.controls.txtProductCode.value
    let txtProductName = this.form.controls.txt_product_name.value
    // let dteDateFrom = this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateFrom.value);
    // let dteDateTo = this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateTo.value);
    // console.log(dteDateFrom);
    if (this.tabIndex == 0) {   
   
    this.billingService.getAllMaintenanceSheet(serBranchId, serLocationId, txtBatchNo, dteDateFrom, dteDateTo, serMaintainanceItemId, txtItemName, txtProductCode, txtProductName).subscribe(response => {
      this.maintenanceSheetDataList = response;
      this.wasteCollectionList = response;
      this.selectStatusList = [];
      debugger;
      this.maintenanceSheetList = new MatTableDataSource( this.maintenanceSheetDataList);
      this.maintenanceSheetList.sort = this.maintenanceSheetListSort;
      this.maintenanceSheetList.paginator = this.maintenanceSheetListPaginator;
    })
  }
  else if (this.tabIndex == 1) {       
    this.billingService.getAllMaintenanceSheet(serBranchId, serLocationId, txtBatchNo, dteDateFrom, dteDateTo, serMaintainanceItemId, txtItemName, txtProductCode, txtProductName).subscribe(response => {
      this.maintenanceSheetDataList = response;
      this.wasteCollectionList = response;
      this.selectStatusList = [];
      debugger;
      this.maintenanceSheetDueInMonth = new MatTableDataSource(this.maintenanceSheetFindAll);
      this.maintenanceSheetDueInMonth.sort = this.maintenanceSheetDueInMonthSort;
      this.maintenanceSheetDueInMonth.paginator = this.maintenanceSheetDueInMonthPaginator;
    })
  }
  else if (this.tabIndex == 2) {
    this.billingService.getAllMaintenanceSheet(serBranchId, serLocationId, txtBatchNo, dteDateFrom, dteDateTo, serMaintainanceItemId, txtItemName, txtProductCode, txtProductName).subscribe(response => {
      this.maintenanceSheetDataList = response;
      this.wasteCollectionList = response;
      this.selectStatusList = [];
      debugger;
      this.maintenanceSheetOverDue = new MatTableDataSource( this.maintenanceSheetFindAll);
      this.maintenanceSheetOverDue.sort = this.maintenanceSheetOverDueSort;
      this.maintenanceSheetOverDue.paginator = this.maintenanceSheetOverDuePaginator;
    })
  }
  }
  onReset(){
    this.form.reset();
  }
  
  downloadPDF() {
    this.wasteCollectionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sNo") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBatchName") {
        tableHeader.push({ text: 'Vehicle/Asset Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
      else if (element === "txtItemCode") {
        tableHeader.push({ text: 'Item Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtUom") {
        tableHeader.push({ text: 'UoM', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "dteLastMaintenanceDate") {
        tableHeader.push({ text: 'Last Maintenance Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "dteNextMaintenanceDate") {
        tableHeader.push({ text: 'Next Maintenance Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numLastOdoMeterReading") {
        tableHeader.push({ text: 'Last Odo Meter Reading', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numNextOdoMeterReading") {
        tableHeader.push({ text: 'Next Odo Meter Reading', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numLastMaintenancequantity") {
        tableHeader.push({ text: 'Last Maintenance Quantity', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numLastMaintenancePrice") {
        tableHeader.push({ text: 'Last Maintenance Price', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }  
    });
    this.wasteCollectionDTO.push(tableHeader);
    this.wasteCollectionList.forEach((wasteCollectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sNo") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: wasteCollectionList.txt_branch_name, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: wasteCollectionList.txt_location_name, alignment: 'left' });
        } else if (element === "txtBatchName") {
          tableRows.push({ text: wasteCollectionList.txt_product_name, alignment: 'left' });
        } else if (element === "txtItemCode") {
          tableRows.push({ text: wasteCollectionList.txt_item_code, alignment: 'left' });
        }else if (element === "txtUom") {
          tableRows.push({ text: wasteCollectionList.txt_uom, alignment: 'left' });
        } else if (element === "dteLastMaintenanceDate") {
          tableRows.push({ text: wasteCollectionList.dte_maintainance_date, alignment: 'left' });
        } else if (element === "dteNextMaintenanceDate") {
          tableRows.push({ text: wasteCollectionList.next_maintenance_date, alignment: 'left' });
        } else if (element === "numLastOdoMeterReading") {
          tableRows.push({ text: wasteCollectionList.numOdometerReading, alignment: 'right' });
        } else if (element === "numNextOdoMeterReading") {
          tableRows.push({ text: wasteCollectionList.num_next_odometer_reading, alignment: 'right' });
        }
        else if (element === "numLastMaintenancequantity") {
          tableRows.push({ text: wasteCollectionList.numItemQuantity, alignment: 'right' });
        } else if (element === "numLastMaintenancePrice") {
          tableRows.push({ text: wasteCollectionList.num_item_price, alignment: 'right' });
        }
      });
      this.wasteCollectionDTO.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Maintenance Sheet', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.wasteCollectionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Maintenance Sheet.pdf');
  }

  tableColumn: string[] = [
    'sNo', 'txtBranchName', 'txtLocationName', 'txtBatchName','txtBatchNo',
    'txtItemCode', 'txtUom', 'dteLastMaintenanceDate', 'dteNextMaintenanceDate', 'numLastOdoMeterReading'
    , 'numNextOdoMeterReading', 'numLastMaintenancequantity', 'numLastMaintenancePrice'
   
  ];
  selection = new FormControl([
    'sNo', 'txtBranchName', 'txtLocationName', 'txtBatchName',
    'txtItemCode', 'txtUom', 'dteLastMaintenanceDate', 'dteNextMaintenanceDate', 'numLastOdoMeterReading'
    , 'numNextOdoMeterReading', 'numLastMaintenancequantity', 'numLastMaintenancePrice'



  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sNo") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
  tabIndex: number;
  onTabChanged(event) {
    this.tabIndex = event.index;
  }
}
