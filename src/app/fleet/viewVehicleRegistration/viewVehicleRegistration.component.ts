import { Component } from '@angular/core';
import { User } from '@app/_models';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';
import { CrudService } from '@app/_services/crud.service';
import Swal from 'sweetalert2';
import { VehicleService } from '../vehicleRegistration/vehicle-service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
// import { debug } from 'node:console';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { BillingService } from '@app/services/billing.service';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: 'viewVehicleRegistration.component.html'
})
export class ViewVehicleRegistrationComponent {


  model;
  currenctVehicle: any = '';
  viewRegistrationDataList: any;
  reportDtoList: any = [];
  user: User;
  add: boolean = true;
  isExpanded = true;
  constructor(private vehicleService: VehicleService,
    private modalService: NgbModal,
    private formBuilder: FormBuilder,
    private shared: CrudService,
    private utilsFunctions: UtilsFunctions,
    private billingService :BillingService
  ) {

  }
  maintenanceItemList:any[]=[];
  ngOnInit() {
    debugger;
    this.vehicleService.currentVehicle.subscribe(
      vehicle => {
        this.currenctVehicle = vehicle;
      });
    debugger;
    this.billingService.getFleetByTxtBatchNo(this.currenctVehicle.txtBatchNo).subscribe(response => {
      
     this.maintenanceItemList = response["lstMaintainanceStandardDto"]
     console.log("daownload data ")
     console.log(this.maintenanceItemList)
    })
    this.viewRegistrationDataList = this.currenctVehicle; 
  }
 
  downloadPD1F() {
    debugger;
   
    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
      
           {
          style: 'tableExample',
          table: {           
            headerRows: 1,
            body: this.reportDtoList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 16,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableExample1: {
          margin: [15, 35, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 8,
          color: 'black',

        },
        tableHeaderValue: {
          fontSize: 8,
          color: 'black',

        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('View Vehicle Registration.pdf');
  }

  downloadPDF() {
    let tableHeader: any[] = []
    tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' })
    tableHeader.push({ text: 'Item Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
    tableHeader.push({ text: 'Item Name', bold: true, fillColor: '#dddddd', alignment: 'center' })
    tableHeader.push({ text: 'Distance (km)', bold: true, fillColor: '#dddddd', alignment: 'center' });    
    tableHeader.push({ text: 'Duration (in months)', bold: true, fillColor: '#dddddd', alignment: 'center' })
    this.reportDtoList.push(tableHeader);   
    this.maintenanceItemList.forEach((maintenanceItemList, i) => {
      debugger;
      let tableRows: any[] = []
      tableRows.push({ text: (i + 1), alignment: 'right' });      
      tableRows.push({ text: maintenanceItemList.txtItemCode, alignment: 'left' }),
      tableRows.push({ text: maintenanceItemList.txtItemName, alignment: 'left' }),
      tableRows.push( { text: maintenanceItemList.numDueInDistance, alignment: 'right' }),
      tableRows.push({ text: maintenanceItemList.numDueInMonths, alignment: 'right' })      
      this.reportDtoList.push(tableRows)
    });    
    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Vehicle Registration', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -20, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        { text: 'Basic Information:', fontSize: 14, bold: true, margin: [0, 20, 0, 0] },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [200, 170, 200, 170],
            body: [
              [
                { text: 'Zone/Branch:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtBranchName, style: 'tableHeaderValue' },
                { text: 'Union Council:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtLocationName, style: 'tableHeaderValue' }
              ],
              [
                { text: 'Serial No:', style: 'tableHeader' },
                { text: this.currenctVehicle.serBatchNo, style: 'tableHeaderValue' },
                { text: 'Vehicle No:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtProductCode, style: 'tableHeaderValue' }
              ],
              [
                { text: 'Vehicle Name:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtProductName, style: 'tableHeaderValue' },
                { text: 'Vehicle Type:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtAssetType, style: 'tableHeaderValue' }
              ],
              [
                { text: 'Description:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtDescription, style: 'tableHeaderValue' },
                { text: '' },
                { text: '' }
              ]
            ]
          },
          layout: 'noBorders'
        },
        { text: 'Dynamic Information:', fontSize: 14, bold: true, margin: [0, 20, 0, 0] },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [200, 170, 200, 170],
            body:
              [
                [
                  { text: 'Brand:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtBrand, style: 'tableHeaderValue' },
                  { text: 'Model:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtModel, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Engine No:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtEngineNo, style: 'tableHeaderValue' },
                  { text: 'Chassis No:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtChassisNo, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Color:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtColor, style: 'tableHeaderValue' },
                  { text: 'Fuel Type' , style: 'tableHeader'},
                  { text: this.currenctVehicle.txtFuelType, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Fuel Capacity:', style: 'tableHeader' },
                  { text: this.currenctVehicle.numFuelCapacity, style: 'tableHeaderValue' },
                  { text: 'Average Fuel Consumption' , style: 'tableHeader'},
                  { text: this.currenctVehicle.numAvgFuelConsumption, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Reserve Tank Capacity:', style: 'tableHeader' },
                  { text: this.currenctVehicle.numReserveTankCapacity, style: 'tableHeaderValue' },
                  { text: 'Engine Cylinder' , style: 'tableHeader'},
                  { text: this.currenctVehicle.txtEngineCylinder, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Engine Size:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtEngineSize, style: 'tableHeaderValue' },
                  { text: 'Load Capacity' , style: 'tableHeader'},
                  { text: this.currenctVehicle.txtLoadCapacity, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Body Type:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtBodyType, style: 'tableHeaderValue' },
                  { text: 'Body Size' , style: 'tableHeader'},
                  { text: this.currenctVehicle.txtBodySize, style: 'tableHeaderValue' }
                ],
              ]
          },
          layout: 'noBorders'
        },
        { text: 'Driver Information:', fontSize: 14, bold: true, margin: [0, 20, 0, 0] },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [200, 170, 200, 170],
            body:
              [
                [
                  { text: 'Driver No:', style: 'tableHeader' },
                  { text: this.currenctVehicle.numDriverNo, style: 'tableHeaderValue' },
                  { text: 'Driver Name:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtEmployeeName, style: 'tableHeaderValue' }
                ],
                [
                  { text: 'Driver Designation:', style: 'tableHeader' },
                  { text: this.currenctVehicle.txtDriverDesignation, style: 'tableHeaderValue' },
                  { text: '' },
                  { text: '' }
                ],               
              ]
          },
          layout: 'noBorders'
        },
        { text: 'Operation & Maintenance:', fontSize: 14, bold: true, margin: [0, 20, 0, 0] },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [200, 170, 200, 170],
            body: [
              [
                { text: 'Current Status:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtCurrentStatus, style: 'tableHeaderValue' },
                { text: 'Current Condition:', style: 'tableHeader' },
                { text: this.currenctVehicle.txtCurrentCondition, style: 'tableHeaderValue' }
              ],
              [
                { text: 'Duration In Svc:', style: 'tableHeader' },
                { text: this.currenctVehicle.dteServiceStartDate, style: 'tableHeaderValue' },
                { text: 'Duration Out Svc:', style: 'tableHeader' },
                { text: this.currenctVehicle.dteCreatedDate, style: 'tableHeaderValue' }
              ],
              [
                { text: 'Measuremnet Unit', style: 'tableHeader' },
                { text: this.currenctVehicle.txtUom, style: 'tableHeaderValue', colSpan: 3 },

              ],
            ]
          },
          layout: 'noBorders'
        },
        {

          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [100, 150, 150,150,150],
            body: this.reportDtoList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
          padding: [50, 0, 0, 0]
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
          padding: [50, 0, 0, 0]
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };
    pdfMake.createPdf(documentDefinition).download('Vehicle Registration.pdf');
  }



}