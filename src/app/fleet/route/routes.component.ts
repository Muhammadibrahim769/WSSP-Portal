...
export class RoutesComponent implements OnInit {
  ...
  routesColumn: string[] = ['txtBranchName', 'txtLocationName', 'txtRouteType', 'txtRouteNo', 'txtRouteName',
    'distance', 'Actions'];
  // 'startPoint', 'endPoint'
  ...
  onSearch() {
    let ser_branch_id = this.searchForm.controls.serBranchId.value
    let ser_location_id = this.searchForm.controls.serLocationId.value
    let txt_code = this.searchForm.controls.txtRouteNo.value
    let txt_route_type = this.searchForm.controls.txtRouteType.value

    let txt_name = this.searchForm.controls.name.value
    this.billingService.getRoutesTableByParamService(ser_branch_id, ser_location_id, txt_code, txt_name,txt_route_type).subscribe(response => {
      console.log(response.data);
      debugger;
  ...
  pdfImplementation() {
    this.billingService.getRoutesTableService().subscribe(Response => {
      debugger;
      this.routeDataList = Response["data"];;
      console.log("daownload data ")
      console.log(this.routeDataList)
      debugger;
      this.reportDtoList.push([
        ...
        { text: 'Distance', bold: true },
        { text: 'Route Type', bold: true }
      ]);
      this.routeDataList.forEach((routeDataList, i) => {
        debugger;
        this.reportDtoList.push([
          { text: (i + 1).toString(), alignment: 'right' },
          { text: routeDataList.txtBranchName, alignment: 'left' },
          { text: routeDataList.txtLocationName, alignment: 'left' },
          { text: routeDataList.code, alignment: 'left' },
          { text: routeDataList.name, alignment: 'left' },
          { text: routeDataList.totalDistance, alignment: 'right' },
          { text: routeDataList.txtRouteType, alignment: 'left' }
        ]);
      });
    },
    );
  ...
  downloadPDF() {
    ...
    this.routeDataList.forEach((routeDataList, i) => {
      ...
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: routeDataList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: routeDataList.txtLocationName, alignment: 'left' });
        } else if (element === "txtRouteNo") {
          tableRows.push({ text: routeDataList.code, alignment: 'left' });
        } else if (element === "txtRouteName") {
          tableRows.push({ text: routeDataList.name, alignment: 'left' });
        } else if (element === "distance") {
          tableRows.push({ text: routeDataList.totalDistance, alignment: 'left' });
        }else if (element === "txtRouteType") {
          tableRows.push({ text: routeDataList.txtRouteType, alignment: 'left' });
        } 
      });
      ...
      content: [
        {
          ...
          table: {
            ...
            body: [
              ...
              }, [{ text: 'Route Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
    ...
    pdfMake.createPdf(documentDefinition).download('Route List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "txtBranchName",
