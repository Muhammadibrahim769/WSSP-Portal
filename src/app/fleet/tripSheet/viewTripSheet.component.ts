import { Component, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { CrudService } from '@app/_services/crud.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({  selector: 'app-dashboard',

  templateUrl: './viewTripSheet.component.html',
})
export class ViewTripSheetComponent implements OnInit { 
  tripData:any[]=[]; 
  reportDtoList: any = [];
  tripDataList:any;
  constructor(private route: ActivatedRoute,
    private billingService: BillingService,
    private shared: CrudService,
    private utilsFunctions: UtilsFunctions,
    private dateFormat:DateFormatPipe
    ) { }

  ngOnInit(): void {
      
    this.shared.currentMessage.subscribe(message => {
      this.tripData.push(message);
      console.log('hjghjgk');
      console.log(this.tripData)
    })        
        this.tripDataList =  this.tripData;
        console.log("daownload data ")
        console.log(this.tripDataList)     
        debugger;
            debugger;
      //       this.reportDtoList.push([
      //         { text: 'Branch Name', bold: true },
      //         { text: 'Union Council', bold: true },             
      //         { text: 'Date', bold: true },  
      //         { text: 'Asset/Vehicle No', bold: true },  
      //         { text: 'Asset/Vehicle Name', bold: true },  
      //         { text: 'Driver Name', bold: true },  
      //         { text: 'Route No', bold: true },  
      //         { text: 'Route Name', bold: true },
      //         { text: 'Route Distance', bold: true },
      //         { text: 'Distance Covered', bold: true },
      //         { text: 'Trip No', bold: true },
      //         { text: 'Time Out', bold: true },
      //         { text: 'Odo Meter Out', bold: true },
      //         { text: 'Time In', bold: true }, 
      //         { text: 'Odo Meter In', bold: true }, 
      //         { text: 'Total Distance', bold: true }, 
      //         { text: 'Total Load', bold: true }, 
      //       ]);
      //       this.tripDataList.forEach((tripDataList, i) => {
      //         debugger;
      //         this.reportDtoList.push([
      //           // { text: (i + 1).toString(), alignment: 'right' },

      //           { text: tripDataList.txtBranchName},
      //           { text: tripDataList.txtLocationName},
      //           { text: tripDataList.dteDate},
      //           { text: tripDataList.serBatchNo},
      //           { text: tripDataList.txtBatchNo},
      //           { text: tripDataList.txt_employee_name},
      //           { text: tripDataList.txtRouteNo},
      //           { text: tripDataList.route_name},
      //           { text: tripDataList.numRouteDistance},
      //           { text: tripDataList.numCoveredDistance},
      //           { text: tripDataList.serTripSheetId},
      //           { text: tripDataList.dteTimeout},
      //           { text: tripDataList.numOdometerOut},
      //           { text: tripDataList.dteTimein},
      //           { text: tripDataList.numOdometerIn},
      //           { text: tripDataList.numGarbageDisposed},                
      //           { text: tripDataList.numTotalDistance}                
      //         ]); 
      //       });
      }
      downloadFile(fileData: any, filetype: string) {
        debugger;
        this.utilsFunctions.downloadFile(fileData, filetype);
      }
      downloadPDF() {
        debugger;
        console.log("this.tripDataList")
        console.log(this.tripDataList)
        const documentDefinition = {
          pageOrientation: 'A4',
          content: [
            {
              style: 'tableExample',
              table: {
                headerRows: 1,
                body: [
                  [{
                    image: 'data:image/png;base64,',
                    width: 80,
                    height: 80,
                    alignment: 'left'
                  }, 
                  [{ text: 'TripSheetReport', style: 'tableHeaderValue', alignment: 'center',          headerRows: 1,
                  margin: [100, 40, 150, 0], bold: true, fontSize: 16
                }],
                  {
                    image: 'data:image/png;base64,', width: 70,
                    height: 70,
                    alignment: 'right', margin: [20, 5, 25, 0],
                  }],
      
      
                ]
              },
              layout: 'noBorders'
            },
            {
              style: 'tableExample',
              table: {
                headerRows: 1,
                widths: [110, 145, 110, 145],
                body: [
                  [
                    { text: 'Zone/Branch:', style: 'tableHeader' },
                    { text: this.tripDataList[0].txtBranchName, style: 'tableHeaderValue' },
                    { text: 'Union Council:', style: 'tableHeader' },
                    { text: this.tripDataList[0].txtLocationName, style: 'tableHeaderValue' },
                  ],
                  [
                    { text: 'Date:', style: 'tableHeader' },
                    { text: this.dateFormat.transformOperation(this.tripDataList[0].dteDate), style: 'tableHeaderValue' },
                    { text: 'Asset/Vehicle No:', style: 'tableHeader' },
                    { text: this.tripDataList[0].serBatchNo, style: 'tableHeaderValue' }
    
                  ],
                  [
                    { text: 'Asset/Vehicle Name:', style: 'tableHeader' },
                    { text: this.tripDataList[0].txtBatchNo, style: 'tableHeaderValue' },
                    { text: 'Driver Name:', style: 'tableHeader' },
                    { text: this.tripDataList[0].txt_employee_name, style: 'tableHeaderValue' }
                  ],
                  [
                    { text: 'Route No:', style: 'tableHeader' },
                    { text: this.tripDataList[0].txtRouteNo, style: 'tableHeaderValue' },
                    { text: 'Route Name:', style: 'tableHeader' },
                    { text: this.tripDataList[0].route_name, style: 'tableHeaderValue' }
                  ],
                  [
                    { text: 'Route Distance', style: 'tableHeader' },
                    { text: this.tripDataList[0].numRouteDistance, style: 'tableHeaderValue' },
                    { text: 'Garbage Disposed:', style: 'tableHeader' },
                    { text: this.tripDataList[0].numGarbageDisposed, style: 'tableHeaderValue' }
                    
                  ],
                  [
                    { text: 'Distance Covered:', style: 'tableHeader' },
                    { text: this.tripDataList[0].numCoveredDistance, style: 'tableHeaderValue' },
                    { text: 'Trip No:', style: 'tableHeader' },
                    { text: this.tripDataList[0].serTripSheetId, style: 'tableHeaderValue' }
                  ],
                  [
                    { text: 'Time Out:', style: 'tableHeader' },
                    { text: this.tripDataList[0].dteTimeout, style: 'tableHeaderValue' },
                    { text: 'Odo Meter Out :', style: 'tableHeader' },
                    { text: this.tripDataList[0].numOdometerOut, style: 'tableHeaderValue' }
                  ],
                  [
                    { text: 'Time In', style: 'tableHeader' },
                    { text: this.tripDataList[0].dteTimein, style: 'tableHeaderValue' },
                    { text: 'Odo Meter In :', style: 'tableHeader' },
                    { text: this.tripDataList[0].numOdometerIn, style: 'tableHeaderValue' }
                  ],
                  [
                    { text: 'Total Distance', style: 'tableHeader' },
                    { text: this.tripDataList[0].numTotalDistance, style: 'tableHeaderValue' },
                    { text: '', style: 'tableHeader' },
                    {  }
                  ],
                ]     
              },
              layout: 'noBorders'
            },
          ],
          styles: {
            header: {
              fontSize: 16,
              bold: true,
              margin: [0, 0, 0, 10]
            },
            subheader: {
              fontSize: 16,
              bold: true,
              margin: [0, 10, 0, 5]
            },
            tableExample: {
              margin: [0, 5, 0, 15]
            },
            tableHeader: {
              bold: true,
              fontSize: 10,
              color: 'black',
              padding: [50, 0, 0, 0]
            },
            tableHeaderValue: {
              fontSize: 10,
              color: 'black',
              padding: [50, 0, 0, 0]
            }
          },
          defaultStyle: {
          }
        };
        pdfMake.createPdf(documentDefinition).download('TripSheet Detail.pdf');
      }

}