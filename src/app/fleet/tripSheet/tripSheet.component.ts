...
export class TripSheetComponent implements OnInit {
  ...
  getBatchNo: any;
  pageSize = 5;
  tripSheetColumn: string[] = ['sNo', 'txtBranchName', 'txtLocationName', 'tripDate', 'asset_no', 'txtDriverName',
    'txtRouteNo', 'txtRouteName', 'route_distance', 'txtRouteType', 'distance_covered', 'trip_no', 'time_out', 'odometer_out', 'time_in',
    'odometer_in',  'total_load', 'Actions'];
  ...
  onSearch() {
    ...
    let dteDateStart = '';
    if (this.form.controls.dteDateStart.value != "") {
      dteDateStart = this.dateFormat.transformOperation(this.form.controls.dteDateStart.value);
    }
   
    let dteDateEnd = '';
    if (this.form.controls.dteDateEnd.value != "") {
      let dteDateEnd = this.dateFormat.transformOperation(this.form.controls.dteDateEnd.value);
     }
  ...
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      if (element === "sr") {
        ...
        tableHeader.push({ text: 'Route Distance', bold: true, fillColor: '#dddddd', alignment: 'center' });
    ...
    this.tripDataList.forEach((tripDataList, i) => {
      ...
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: this.dateFormat.transformOperation(tripDataList.dteDate), alignment: 'left' });
        } else if (element === "asset_no") {
          tableRows.push({ text: tripDataList.txtBatchNo, alignment: 'left' });
        } else if (element === "txtDriverName") {
          ...
          tableRows.push({ text: tripDataList.numCoveredDistance, alignment: 'left' });
        } else if (element === "time_out") {
          tableRows.push({ text:this.dateFormat.transformDateTime(tripDataList.dteTimeout), alignment: 'left' });
        } else if (element === "odometer_out") {
          ...
          tableRows.push({ text: this.dateFormat.transformDateTime(tripDataList.dteTimein), alignment: 'left' });
        } else if (element === "total_load") {
