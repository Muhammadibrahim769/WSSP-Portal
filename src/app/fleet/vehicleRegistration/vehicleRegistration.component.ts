import { AfterViewInit, Component, ViewChild } from '@angular/core';
import { MatPaginator } from '@angular/material/paginator';
import { MatTableDataSource } from '@angular/material/table';

import { User } from '@app/_models';

import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import {
  FormBuilder,
  FormGroup,
  Validators,
  FormControl,
} from '@angular/forms';
import { CrudService } from '@app/_services/crud.service';
import { Subscription } from 'rxjs';
import Swal from 'sweetalert2';
import { MatSort } from '@angular/material/sort';
import { BillingService } from '@app/services/billing.service';
import { VehicleService } from './vehicle-service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { ActivatedRoute, Router } from '@angular/router';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

export interface PeriodicElement {
  sr: number;
  Vehicle_Type: string;
  Brand: string;
  Model: number;
  Asset_No: string;
  Vehicle_Name: string;
  Serial_No: number;
  EngineNo: string;
  txtChassisNo: string;
  colour: string;
  Fuel_Type: string;
  Fuel_Capacity: string;
  Average_Fuel_Consumption: string;
  Engine_Cylinder: string;
  Engine_Size: string;
  Load_Capacity: string;
  Body_Type: string;
  Current_Status: string;
  Current_Condition: string;
  With_Effect_From: string;
  Servicable_Time: string;
  Un_Servicable_Time: string;
}
@Component({
  selector: 'app-dashboard',
  templateUrl: 'vehicleRegistration.component.html',
})
export class VehicleRegistrationComponent implements AfterViewInit {
  form: FormGroup;
  addForm: FormGroup;
  user: User;
  search: boolean = true;
  add: boolean = false;
  panelOpenState = true;
  page = 1;
  vehicleDataList: any;
  pageSize = 2;
  message: any;
  subscription: Subscription;
  branchList: any[] = [];
  unionCouncilList: any[] = [];
  createVehicle: any[] = [];
  reportDtoList: any = [];
  vehicleRegistrationDataList: any;

  statusList = ['Operational', 'UnOperations'];

  displayedColumns: string[] = [
    'sr',
    'txtBranchName',
    'txtLocationName',
    'txtVehicleType',
    'txtModel',
    'txtAssetNo',
    'txtVehicleName',
    'numSerialNo',
    'txtEngineNo',
    'txtChassisNo',
    'txtColour',
    'txtFuelType',
    'txtFuelCapacity',
    'txtAverageFuelConsumption',
    'txtEngineCylinder',
    'txtEngineSize',
    'txtLoadCapacity',
    'txtBodyType',
    'txtCurrentStatus',
    'txtCurrentCondition',
    'dteServicableTime',
    'dteUnServicableTime',
    'Actions',
  ];
  dataSource = new MatTableDataSource<PeriodicElement>();
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  ngAfterViewInit() {
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
  }
  constructor(
    private vehicleService: VehicleService,
    private utilsFunctions: UtilsFunctions,
    private billingService: BillingService,
    private formBuilder: FormBuilder,
    private shared: CrudService,
    private router: Router
  ) {}
  OnlyNumbersAllowed(event): boolean {
    const charcode = event.which ? event.which : event.keycode;
    if (charcode > 31 && (charcode < 48 || charcode > 57)) {
      return false;
    }
    return true;
  }
  step = 0;

  setStep(index: number) {
    this.step = index;
  }
  ngOnInit() {
    this.shared.currentMessage.subscribe((message) => {
      this.createVehicle.push(message);
    });
    this.form = this.formBuilder.group({
      txtMainCategory: ['Vehicle'],
      txtVehicleCategory: [''],
      txtVehicleSubCategory: [''],
      txtVehicleType: [''],
      txtVehicleId: [''],
      txtVehicleName: [''],
      txtSerialNo: [''],
      txtCurrentStatus: [''],
      // txtbrand: [""],
      txtVehicleMake: [''],
      txtVehicleModel: [''],
      txtEngineNo: [''],
      txtChassisNo: [''],
      txtVehicleColor: [''],
      txtVehicleStatus: [''],
      txtFuelType: [''],
      txtFuelCapacity: [''],
      txtAverageFuel: [''],
      txtVehicleLoadCapcity: [''],
      txtDescription: [''],
      txtBranchId: [''],
      serBranchId: [''],
      txtModel: [''],
      txtServicableTime: [''],
      txtUnServicableTime: [''],
      txtEngineCylinder: [''],
      txtWithEffectFromDate: [''],
      txtBodyType: [''],
      txtBodySize: [''],
      txtxCurrentCondition: [''],
      txtEngineSize: [''],
      txtReserveTankCapacity: [''],
      txtTotalRunning: [''],
      txtUom: [''],
      txtDriverDesignation: [''],
      txtWithEffectFrom: [''],
      txtDriverName: [''],
      txtDriverNo: [''],
      serLocationId: [''],
      txtBatchNo: [''],
      serBatchNo: [''],
      txtCurrentCondition: [''],
    });
    this.billingService
      .getAllCustomerTemplateService()
      .subscribe((response) => {
        debugger;
        console.log(response['branchList']);
        this.branchList = response['branchList'];
        console.log(response['unionConcilList']);
        this.unionCouncilList = response['unionConcilList'];
      });
    this.getDistinctAsset();
    this.getFleetList();
    this.billingService.getFleetService('', '').subscribe((response) => {
      debugger;
      this.vehicleDataList = response;
      console.log('daownload data ');
      console.log(this.vehicleDataList);
      // this.tripSheetData = new MatTableDataSource(data["data"].lstAwardDto);
      // this.tripSheetData.sort = this.tripSheetDataSort;
      // this.tripSheetData.paginator = this.tripSheetDataPaginator;
      debugger;
      debugger;
      this.reportDtoList.push([
        { text: 'S.No', bold: true },
        { text: 'Vehicle Type', bold: true },
        { text: 'Brand', bold: true },
        { text: 'Model', bold: true },
        { text: 'Asset/Vehicle No', bold: true },
        { text: 'Asset/Vehicle Name', bold: true },
        { text: 'Serial No', bold: true },
        { text: 'EngineNo', bold: true },
        { text: 'Chassis No', bold: true },
        { text: 'colour', bold: true },
        { text: 'Fuel Type ', bold: true },
        { text: 'Fuel Capacity', bold: true },
        { text: 'Average Fuel Consumption', bold: true },
        { text: 'Engine Cylinder', bold: true },
        { text: 'Engine Size', bold: true },
        { text: 'Load Capacity', bold: true },
        { text: 'Body Type', bold: true },
        { text: 'Current Status', bold: true },
        { text: 'Current Condition', bold: true },
        { text: 'Duration In svc', bold: true },
        { text: 'Duration out svc', bold: true },
      ]);
      this.vehicleDataList.forEach((vehicleDataList, i) => {
        debugger;
        this.reportDtoList.push([
          { text: (i + 1).toString(), alignment: 'right' },
          { text: vehicleDataList.txtVehicleType, alignment: 'left' },
          // { text: vehicleDataList.txtBrand, alignment: 'left' },
          { text: vehicleDataList.txtModel, alignment: 'left' },
          { text: vehicleDataList.txtProductCode, alignment: 'left' },
          { text: vehicleDataList.txtProductName, alignment: 'left' },
          { text: vehicleDataList.serBatchNo, alignment: 'left' },
          { text: vehicleDataList.EngineNo, alignment: 'left' },
          { text: vehicleDataList.txtChassisNo, alignment: 'left' },
          { text: vehicleDataList.colour, alignment: 'left' },
          { text: vehicleDataList.Fuel_Type, alignment: 'left' },
          { text: vehicleDataList.Fuel_Capacity, alignment: 'left' },
          { text: vehicleDataList.Average_Fuel_Consumption, alignment: 'left' },
          { text: vehicleDataList.Engine_Cylinder, alignment: 'left' },
          { text: vehicleDataList.Engine_Size, alignment: 'left' },
          { text: vehicleDataList.Load_Capacity, alignment: 'left' },
          { text: vehicleDataList.Body_Type, alignment: 'left' },
          { text: vehicleDataList.Current_Status, alignment: 'left' },
          { text: vehicleDataList.Current_Condition, alignment: 'left' },
          { text: vehicleDataList.Servicable_Time, alignment: 'left' },
          { text: vehicleDataList.Un_Servicable_Time, alignment: 'left' },
        ]);
      });
    });
  }
  downloadFile(fileData: any, filetype: string) {
    debugger;
    this.utilsFunctions.downloadFile(fileData, filetype);
  }
  getFleetList() {
    this.billingService.getFleetService('', '').subscribe((response) => {
      console.log('getFleetService');
      console.log(response);
      this.productList = response;
      this.vehicleDataList = response;
      this.dataSource = new MatTableDataSource(this.productList);
      this.dataSource.sort = this.sort;
      this.dataSource.paginator = this.paginator;
    });
  }
  assettypeList: any[] = [];
  getDistinctAsset() {
    this.billingService.getDistinctAsset().subscribe((response) => {
      console.log('response');
      console.log(response);
      this.assettypeList = response;
    });
  }

  viewData: any = '';
  editButton(vehicle: any) {
    debugger;
    this.viewData = vehicle;
    console.log(this.viewData);
    this.shared.changeMessage(this.viewData);
    this.router.navigate(['/fleet/vehicleRegistration/edit']);
  }
  onReset() {
    this.form.reset();
  }

  viewButton(vehicle: any) {
    this.vehicleService.selectVehicle(vehicle);
  }
  productList: any = '';
  onBranchChange(event) {
    let serBranchId = event.value;
    this.billingService
      .getFleetByBranchId(serBranchId)
      .subscribe((response) => {
        this.productList = response;
        this.dataSource = new MatTableDataSource(this.productList);
        this.dataSource.sort = this.sort;
        this.dataSource.paginator = this.paginator;
      });
  }
  onLocationChange(event) {
    let serLocationId = event.value;
    this.billingService
      .getFleetByLocationId(serLocationId)
      .subscribe((response) => {
        this.productList = response;
        this.dataSource = new MatTableDataSource(this.productList);
        this.dataSource.sort = this.sort;
        this.dataSource.paginator = this.paginator;
      });
  }

  searchFunction() {
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtAssetType = this.form.controls.txtVehicleType.value;
    let txtProductName = this.form.controls.txtVehicleName.value;
    let txtBatchNo = this.form.controls.txtBatchNo.value;
    let serBatchNo = this.form.controls.serBatchNo.value;
    let txtEngineNo = this.form.controls.txtEngineNo.value;
    let txtChassisNo = this.form.controls.txtChassisNo.value;
    let txtBodyType = this.form.controls.txtBodyType.value;
    let txtCurrentStatus = this.form.controls.txtCurrentStatus.value;
    // let txtCurrentCondition = this.form.controls.txtCurrentCondition.value
    this.billingService
      .getFleetByParam(
        serBranchId,
        serLocationId,
        txtBatchNo,
        txtAssetType,
        txtProductName,
        serBatchNo,
        txtEngineNo,
        txtChassisNo,
        txtBodyType,
        txtCurrentStatus
      )
      .subscribe((response) => {
        debugger;
        this.productList = response;
        this.vehicleDataList = response;
        this.dataSource = new MatTableDataSource(this.productList);
        this.dataSource.sort = this.sort;
        this.dataSource.paginator = this.paginator;
      });
  }
  onSave() {
    // console.log(this.addForm.value);
  }
  // deleteFieldValue(index) {

  //   if(confirm('Do you want to delete this Row ?'))
  //       this.createVehicle.splice(index, 1);
  //   }
  onEdit(index) {
    console.log(this.createVehicle);
    this.form.patchValue(index);
    this.shared.changeMessage(this.createVehicle);
  }
  // sweetAlert to show popup on delete button
  openSweetAlert(index) {
    // if(confirm('Do you want to delete this Row ?'))
    Swal.fire({
      title: 'Are you sure?',
      text: 'You will not be able to recover this Row!',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Yes, delete it!',
      cancelButtonText: 'No, keep it',
    }).then((result) => {
      if (result.value) {
        this.createVehicle.splice(index, 1);
        Swal.fire('Deleted!', 'Your Row has been deleted.', 'success');
        // For more information about handling dismissals please visit
        // https://sweetalert2.github.io/#handling-dismissals
      } else if (result.dismiss === Swal.DismissReason.cancel) {
        Swal.fire('Cancelled', 'Your Row is safe :)', 'error');
      }
    });
  }
  // ['sr', 'txtVehicleType', 'txtBrand', 'txtModel', 'txtAssetNo', 'txtVehicleName', 'numSerialNo',
  // 'txtEngineNo', 'txtChassisNo', 'txtColour', 'txtFuelType', 'txtFuelCapacity', 'txtAverageFuelConsumption', 'txtEngineCylinder',
  // 'txtEngineSize', 'txtLoadCapacity', 'txtBodyType', 'txtCurrentStatus', 'txtCurrentCondition', 'dteWithEffectFrom',
  // 'dteServicableTime', 'dteUnServicableTime', 'Actions'];
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.reportDtoList = [];
    let tableHeader: any[] = [];
    this.selection.value.forEach((element) => {
      if (element === 'sr') {
        tableHeader.push({
          text: 'Sr#',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtVehicleType') {
        tableHeader.push({
          text: 'Vehicle Type',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtBrand') {
        tableHeader.push({
          text: 'Brand',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtModel') {
        tableHeader.push({
          text: 'Model',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtAssetNo') {
        tableHeader.push({
          text: 'Asset/Vehicle No',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtVehicleName') {
        tableHeader.push({
          text: 'Asset/Vehicle Name',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numSerialNo') {
        tableHeader.push({
          text: 'SerialNo',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtEngineNo') {
        tableHeader.push({
          text: 'Engine No',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtChassisNo') {
        tableHeader.push({
          text: 'Chassis No',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtColour') {
        tableHeader.push({
          text: 'Colour',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtFuelType') {
        tableHeader.push({
          text: 'Fuel Type',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtFuelCapacity') {
        tableHeader.push({
          text: 'Fuel Capacity:',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtAverageFuelConsumption') {
        tableHeader.push({
          text: 'Average Fuel Consumption',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtEngineCylinder') {
        tableHeader.push({
          text: 'Engine Cylinder',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtEngineSize') {
        tableHeader.push({
          text: 'Engine Size',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtLoadCapacity') {
        tableHeader.push({
          text: 'Load Capacity',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtBodyType') {
        tableHeader.push({
          text: 'Body Type',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtCurrentStatus') {
        tableHeader.push({
          text: 'Current Status',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtCurrentCondition') {
        tableHeader.push({
          text: 'Current Condition',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'dteWithEffectFrom') {
        tableHeader.push({
          text: 'With Effect From',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'dteServicableTime') {
        tableHeader.push({
          text: 'Servicable Time',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'dteUnServicableTime') {
        tableHeader.push({
          text: 'Un Servicable Time',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
    });
    this.reportDtoList.push(tableHeader);
    console.log(this.reportDtoList);
    this.vehicleDataList.forEach((vehicleDataList, i) => {
      debugger;
      let tableRows: any[] = [];
      this.selection.value.forEach((element) => {
        if (element === 'sr') {
          tableRows.push({ text: i + 1, alignment: 'right' });
        } else if (element === 'txtVehicleType') {
          tableRows.push({
            text: vehicleDataList.txtAssetType,
            alignment: 'left',
          });
        } else if (element === 'txtBrand') {
          tableRows.push({ text: vehicleDataList.txtBrand, alignment: 'left' });
        } else if (element === 'txtModel') {
          tableRows.push({
            text: this.vehicleDataList.dteDate,
            alignment: 'left',
          });
        } else if (element === 'txtAssetNo') {
          tableRows.push({
            text: vehicleDataList.txtProductCode,
            alignment: 'left',
          });
        } else if (element === 'txtVehicleName') {
          tableRows.push({
            text: vehicleDataList.txtProductName,
            alignment: 'left',
          });
        } else if (element === 'numSerialNo') {
          tableRows.push({
            text: vehicleDataList.serBatchNo,
            alignment: 'right',
          });
        } else if (element === 'txtEngineNo') {
          tableRows.push({
            text: vehicleDataList.txtEngineNo,
            alignment: 'lefrightt',
          });
        } else if (element === 'txtChassisNo') {
          tableRows.push({
            text: vehicleDataList.txtChassisNo,
            alignment: 'left',
          });
        } else if (element === 'txtColour') {
          tableRows.push({
            text: vehicleDataList.txtColour,
            alignment: 'right',
          });
        } else if (element === 'txtFuelType') {
          tableRows.push({
            text: vehicleDataList.txtFuelType,
            alignment: 'left',
          });
        } else if (element === 'txtFuelCapacity') {
          tableRows.push({
            text: vehicleDataList.txtFuelCapacity,
            alignment: 'left',
          });
        } else if (element === 'txtAverageFuelConsumption') {
          tableRows.push({
            text: vehicleDataList.txtAverageFuelConsumption,
            alignment: 'left',
          });
        } else if (element === 'txtEngineCylinder') {
          tableRows.push({
            text: vehicleDataList.txtEngineCylinder,
            alignment: 'right',
          });
        } else if (element === 'txtEngineSize') {
          tableRows.push({
            text: vehicleDataList.txtEngineSize,
            alignment: 'right',
          });
        } else if (element === 'txtLoadCapacity') {
          tableRows.push({
            text: vehicleDataList.txtLoadCapacity,
            alignment: 'left',
          });
        } else if (element === 'txtBodyType') {
          tableRows.push({
            text: vehicleDataList.txtBodyType,
            alignment: 'right',
          });
        } else if (element === 'txtCurrentStatus') {
          tableRows.push({
            text: vehicleDataList.txtCurrentStatus,
            alignment: 'left',
          });
        } else if (element === 'txtCurrentCondition') {
          tableRows.push({
            text: vehicleDataList.txtCurrentCondition,
            alignment: 'right',
          });
        } else if (element === 'dteWithEffectFrom') {
          tableRows.push({
            text: vehicleDataList.dteWithEffectFrom,
            alignment: 'left',
          });
        } else if (element === 'dteServicableTime') {
          tableRows.push({
            text: vehicleDataList.dteServicableTime,
            alignment: 'right',
          });
        } else if (element === 'dteUnServicableTime') {
          tableRows.push({
            text: vehicleDataList.dteUnServicableTime,
            alignment: 'left',
          });
        }
      });
      this.reportDtoList.push(tableRows);
    });
    debugger;
    const documentDefinition = {
      pageSize: 'LEGAL',
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: [
              [
                {
                  image:
                    'data:image/png;base64,',
                },
                [
                  {
                    text: 'Vehicle Registration Report',
                    alignment: 'center',
                    margin: [200, 60, 150, 0],
                    bold: true,
                    fontSize: 16,
                  },
                ],
                {
                  image:
                    'data:image/png;base64,',
                  width: 100,
                  height: 100,
                  alignment: 'right',
                  margin: [0, 0, -80, 0],
                },
              ],
            ],
          },
          layout: 'noBorders',
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            width: '600f',
            body: this.reportDtoList,
          },
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10],
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5],
        },
        tableExample: {
          margin: [0, 5, 0, 15],
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
        },
      },
      defaultStyle: {
        // alignment: 'justify'
      },
    };
    pdfMake
      .createPdf(documentDefinition)
      .download('Vehicle Registration List.pdf');
  }
  tableColumn: string[] = [
    'sr',
    'txtVehicleType',
    'txtBrand',
    'txtModel',
    'txtAssetNo',
    'txtVehicleName',
    'numSerialNo',
    'txtEngineNo',
    'txtChassisNo',
    'txtColour',
    'txtFuelType',
    'txtFuelCapacity',
    'txtAverageFuelConsumption',
    'txtEngineCylinder',
    'txtEngineSize',
    'txtLoadCapacity',
    'txtBodyType',
    'txtCurrentStatus',
    'txtCurrentCondition',
    'dteWithEffectFrom',
    'dteServicableTime',
    'dteUnServicableTime',
  ];
  selection = new FormControl([
    'sr',
    'txtVehicleType',
    'txtBrand',
    'txtModel',
    'txtAssetNo',
    'txtVehicleName',
    'numSerialNo',
    'txtEngineNo',
    'txtChassisNo',
    'txtColour',
    'txtFuelType',
    'txtFuelCapacity',
    'txtAverageFuelConsumption',
    'txtEngineCylinder',
    'txtEngineSize',
    'txtLoadCapacity',
    'txtBodyType',
    'txtCurrentStatus',
    'txtCurrentCondition',
    'dteWithEffectFrom',
    'dteServicableTime',
    'dteUnServicableTime',
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach((singleColumn) => {
        let display = '';
        if (singleColumn !== 'sr') {
          let columnSelected = this.selection.value.findIndex(
            (x) => x === singleColumn
          );
          if (columnSelected >= 0) {
            display = 'table-cell';
          } else {
            display = 'none';
          }
          const slides = document.getElementsByClassName(
            'mat-column-' + singleColumn
          );
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
