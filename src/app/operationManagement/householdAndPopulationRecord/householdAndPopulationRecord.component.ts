import { ViewChild } from '@angular/core';
import { Component, OnInit } from '@angular/core';
import { FormBuilder, FormGroup, Validators, FormControl } from '@angular/forms';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { BillingService } from '@app/services/billing.service';
import { UtilsService } from '@app/_services/utils.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { CustomFormatterPipe } from '@app/_helpers/custom-formatter.pipe';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './householdAndPopulationRecord.component.html',
})
export class HouseholdAndPopulationRecordComponent implements OnInit {
  searchHouseholdAndPopulationForm: FormGroup
  step = 0;
  user = "";
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  isLoading = true;
  dataSource = null;
  //  numb = 123.23454;
  //  pi: number = this.avgMemberPerHouse;
  //  avgMemberPerHouse = this.pi.toFixed(2);
  constructor(private formBuilder: FormBuilder, private utilService: UtilsService, public dialog: MatDialog, private billingService: BillingService,private numberFormat:CustomFormatterPipe) { }
  houseHoldColumns: string[] = ['serBranchId', 'unionCouncil', 'numOfPrimaryMainholes', 'numOfSecondaryMainholes', 'numOfTeriaryMainholes', 'numTotalSewerageLines','numTotalSecondarySewerageLines', 'numDrainageLines', 'totalPopulation', 'numTotalHouseHold', 'avgMemberPerHouse', 'Actions'];
  houseHold = new MatTableDataSource();
  @ViewChild("houseHoldSort", { static: true }) houseHoldSort: MatSort;
  @ViewChild("houseHoldPaginator", { static: true }) houseHoldPaginator: MatPaginator;
  ngAfterViewInit() { }
  clearDate() {
    this.searchHouseholdAndPopulationForm.controls.txtMonth.reset('');
  }
  unionCouncilList: any[] = [];

  ngOnInit(): void {
    this.user = JSON.parse(sessionStorage.getItem('user'));

    this.searchHouseholdAndPopulationForm = this.formBuilder.group({
      txtBranchName: [''],
      txtLocationName: [''],
      serBranchId: [''],
      serLocationId: ['']
    })
    // this.pdfImplementation();
    this.billingService.getAllCustomerTemplateService().subscribe(response => {
      debugger;
      console.log(response["branchList"]);
      this.branchList = response["branchList"];
      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];
    })
    this.onGet();
    this.getBranchList();
    this.searchHouseholdAndPopulationForm.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);
    debugger;
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {
      debugger;
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  onSearch() {
    // window.location.reload();
    let serBranchId = this.searchHouseholdAndPopulationForm.controls.serBranchId.value;
    let serLocationId = this.searchHouseholdAndPopulationForm.controls.serLocationId.value;
    this.billingService.getHouseHoldServiceByParams(serBranchId, serLocationId).subscribe(response => {
      console.log(response.data);
      // this.waterTestListData = response.data;
      debugger;
      this.isLoading = false;
      this.dataSource = response.data;
      this.houseHold = new MatTableDataSource(response.data);
      this.houseHold.sort = this.houseHoldSort;
      this.houseHold.paginator = this.houseHoldPaginator;
      this.pdfImplementation();
    })
  }
  onGet() {
    // window.location.reload();
    this.billingService.getHouseHoldService().subscribe(response => {
      console.log(response.data);
      // this.waterTestListData = response.data;
      debugger;
      this.isLoading = false;
      this.dataSource = response.data;
      this.houseHold = new MatTableDataSource(response.data);
      this.houseHold.sort = this.houseHoldSort;
      this.houseHold.paginator = this.houseHoldPaginator;
      this.pdfImplementation();
    })
  }
  responseList: any[] = [];
  selectStatusList: any[] = [];
 
  setStep(index: number) {
    this.step = index;
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList =  this.dataSource;
    console.log("daownload data ")
    console.log(this.tubewellDataList)
    debugger;
   
    this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'No of RPC Mainholes', bold: true },
      { text: 'No of RCC Mainholes', bold: true },
      { text: 'No of IronGrid Mainholes', bold: true },
      { text: 'Sewerage Lines(Total Length)', bold: true },
      { text: 'Drainage Lines(Total Length)', bold: true },
      { text: 'Total Population', bold: true },
      { text: 'Total HouseHold', bold: true },
      { text: 'Avg Member Per House', bold: true },
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      this.tubewellReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tubewellDataList.txtBranchName, alignment: 'left' },
        { text: tubewellDataList.txtLocationName, alignment: 'left' },
        { text: tubewellDataList.numTotalMainholes, alignment: 'right' },
        { text: tubewellDataList.numTotalSecMainholes, alignment: 'right' },
        { text: tubewellDataList.numTotalTerMainholes, alignment: 'right' },
        { text: tubewellDataList.numTotalSewerageLines, alignment: 'right' },
        { text: tubewellDataList.numTotalDranageLines, alignment: 'right' },
        { text: tubewellDataList.numTotalPopulation, alignment: 'right' },
        { text: tubewellDataList.numTotalHouseHold, alignment: 'right' },
        { text: tubewellDataList.avgMemberPerHouse, alignment: 'right' }
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.tubewellReportDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "serBranchId") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numOfPrimaryMainholes") {
        tableHeader.push({ text: 'No of RPC Mainholes', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numOfSecondaryMainholes") {
        tableHeader.push({ text: 'No of RCC Mainholes', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numOfTeriaryMainholes") {
        tableHeader.push({ text: 'No of IronGrid Mainholes', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numTotalSewerageLines") {
        tableHeader.push({ text: 'Sewerage Lines(Total Length)', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numTotalSecondarySewerageLines") {
        tableHeader.push({ text: 'Sec Sewerage Lines(Total Length)', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numDrainageLines") {
        tableHeader.push({ text: 'Drainage Lines (Total Length)', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "totalPopulation") {
        tableHeader.push({ text: 'Total Population', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numTotalHouseHold") {
        tableHeader.push({ text: 'Total HouseHold', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "avgMemberPerHouse") {
        tableHeader.push({ text: 'Avg Member Per House', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.tubewellReportDTOList.push(tableHeader);
    console.log(this.tubewellReportDTOList);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "serBranchId") {
          tableRows.push({ text: tubewellDataList.txtBranchName, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: tubewellDataList.txtLocationName, alignment: 'left' });
        } else if (element === "numOfPrimaryMainholes") {
          tableRows.push({ text: tubewellDataList.numTotalMainholes, alignment: 'right' });
        } else if (element === "numOfSecondaryMainholes") {
          tableRows.push({ text: tubewellDataList.numTotalSecMainholes, alignment: 'right' });
        } else if (element === "numOfTeriaryMainholes") {
          tableRows.push({ text: tubewellDataList.numTotalTerMainholes, alignment: 'right' });
        } else if (element === "numTotalSewerageLines") {
          tableRows.push({ text: tubewellDataList.numTotalSewerageLines, alignment: 'right' });
        }else if (element === "numTotalSecondarySewerageLines") {
          tableRows.push({ text: tubewellDataList.numTotalSecondarySewerageLines, alignment: 'right' });
        }
         else if (element === "numDrainageLines") {
          tableRows.push({ text: tubewellDataList.numTotalDranageLines, alignment: 'right' });
        } else if (element === "totalPopulation") {
          tableRows.push({ text: tubewellDataList.numTotalPopulation, alignment: 'right' });
        } else if (element === "numTotalHouseHold") {
          tableRows.push({ text: tubewellDataList.numTotalHouseHold, alignment: 'right' });
        } else if (element === "avgMemberPerHouse") {
          tableRows.push({ text: (tubewellDataList.numTotalPopulation/tubewellDataList.numTotalHouseHold)?
            (this.numberFormat.transform((tubewellDataList.numTotalPopulation/tubewellDataList.numTotalHouseHold).toString()))
            :'0', alignment: 'right' });
        }

        // { text: (this.houseHoldDetails.numTotalPopulation / this.houseHoldDetails.numTotalHouseHold)?
        //   (this.numberFormat.transform((this.houseHoldDetails.numTotalPopulation / this.houseHoldDetails.numTotalHouseHold).toString())):'0', style: 'tableHeaderValue' },

      });
      this.tubewellReportDTOList.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'HouseHold & Population', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -20, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.tubewellReportDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
      }
    };

    pdfMake.createPdf(documentDefinition).download('HouseHold & Population List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "serBranchId",
    "unionCouncil",
    "numOfPrimaryMainholes",
    "numOfSecondaryMainholes",
    "numOfTeriaryMainholes",
    "numTotalSewerageLines",
    "numDrainageLines",
    "totalPopulation",
    "numTotalHouseHold",
    "avgMemberPerHouse",
    "numTotalSecondarySewerageLines"
  ];
  selection = new FormControl([
    "sr",
    "serBranchId",
    "unionCouncil",
    "numOfPrimaryMainholes",
    "numOfSecondaryMainholes",
    "numOfTeriaryMainholes",
    "numTotalSewerageLines",
    "numDrainageLines",
    "totalPopulation",
    "numTotalHouseHold",
    "avgMemberPerHouse",
    "numTotalSecondarySewerageLines"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
  onReset(){
    this.searchHouseholdAndPopulationForm.controls.serBranchId.reset('')
    this.searchHouseholdAndPopulationForm.controls.serLocationId.reset('')
    this.searchHouseholdAndPopulationForm.controls.txtBranchName.reset('')
    this.searchHouseholdAndPopulationForm.controls.txtLocationName.reset('')   
    
    this.onGet();
  }
}
