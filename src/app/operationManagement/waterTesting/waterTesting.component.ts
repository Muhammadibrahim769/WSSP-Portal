import { AfterViewInit, Component, OnInit, ViewChild } from '@angular/core';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { saveAs } from 'file-saver';


pdfMake.vfs = pdfFonts.pdfMake.vfs;


@Component({
  selector: 'app-dashboard',
  templateUrl: './waterTesting.component.html',
})
export class WaterTestingComponent implements AfterViewInit {
  Id: any = "";
  step = 0;
  user = "";
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  isLoading = true;
  dataSource= null;
  waterTestingForm: FormGroup;
  constructor(private formBuilder: FormBuilder, private billingService: BillingService, private router: Router, private route: ActivatedRoute, private dateFormat: DateFormatPipe) { }

  clearDate() {
    this.waterTestingForm.controls.txtSamplingDate.reset('');
  }
  onReset(){
    this.waterTestingForm.controls.serBranchId.reset('')
    this.waterTestingForm.controls.serLocationId.reset('')
    this.waterTestingForm.controls.txtAssetType.reset('')
    this.waterTestingForm.controls.txtConsumerType.reset('')
    this.waterTestingForm.controls.txtSamplingDate.reset('')
    this.waterTestingForm.controls.txtTestResult.reset('')
    this.waterTestingForm.controls.txtTestingAgency.reset('')
    this.waterTestingForm.controls.dteDateTo.reset('')     
    this.onGet();
  }
  // Table Data
  waterTestColumns: string[] = ['branch', 'unionCouncil', 'txtAssetType', 'txtConsumerType', 'numConsumerNo', 'txtSamplingDate', 'txtSamplingPoint', 'txtTestingAgency', 'txtResultSubmissionDate', 'txtTestResult', 'Actions'];

  waterTestListData = new MatTableDataSource();
  @ViewChild("waterTestSort", { static: true }) waterTestSort: MatSort;
  @ViewChild("waterTestPaginator", { static: true }) waterTestPaginator: MatPaginator;


  ngAfterViewInit() {
  }
  unionCouncilList: any[] = [];
  ngOnInit(): void {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.waterTestingForm = this.formBuilder.group({
      serBranchId: [''],
      numWaterTestId: [],
      numConsumerNo: [],
      numLongitude: [''],
      numLatitude: [''],
      txtAddress: [''],
      txtConsumerName: [''],
      serLocationId: [''],
      txtAssetType: [''],
      txtBranchName:[''],
      txtLocationName:[''],
      txtComments: [''],
      txtResultSubmissionDate: [''],
      txtConsumerType: [''],
      txtSamplingDate: [''],
      txtTestResult: [''],
      txtTestingAgency: [''],
      txtSamplingPoint: [''],
      txtStreetName: [''],
      txtTestingType: [''],
      dteDateFrom: [""],
      dteDateTo: ['']
    })
    this.onGet();
    this.getBranchList();
    this.waterTestingForm.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);
    debugger;
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {
      debugger;
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  onSearch() {
    debugger
    // let txtSamplingDate =null ;
    // if(txtSamplingDate != null)
    // {
    //    txtSamplingDate = this.dateFormat.transformOperation(this.waterTestingForm.controls.txtSamplingDate.value)
    // }

    // let dteDateTo = null;
    // if(dteDateTo !=null){
    //   dteDateTo = this.dateFormat.transformOperation(this.waterTestingForm.controls.dteDateTo.value)

    // }

    let txtSamplingDate = this.dateFormat.transformtoStandard(this.waterTestingForm.controls.txtSamplingDate.value);
    let dteDateTo = this.dateFormat.transformtoStandard(this.waterTestingForm.controls.dteDateTo.value);
    // let txtSamplingDate = this.waterTestingForm.controls.txtSamplingDate.value
    // let dteDateTo = this.waterTestingForm.controls.dteDateTo.value
    
    let serBranchId = this.waterTestingForm.controls.serBranchId.value;
    let serLocationId = this.waterTestingForm.controls.serLocationId.value;
    let txtAssetType = this.waterTestingForm.controls.txtAssetType.value;
    let txtConsumerType = this.waterTestingForm.controls.txtConsumerType.value;
    let txtTestResult = this.waterTestingForm.controls.txtTestResult.value;
    let txtTestingAgency = this.waterTestingForm.controls.txtTestingAgency.value;
    this.billingService.getWaterTestServiceByParams(txtSamplingDate,dteDateTo,txtAssetType, serBranchId, serLocationId, txtConsumerType, txtTestResult, txtTestingAgency).subscribe(response => {
      console.log(response.data);
      debugger;
      this.isLoading = false;
      this.dataSource = response.data;
      this.waterTestListData = new MatTableDataSource(response.data);
      this.waterTestListData.sort = this.waterTestSort;
      this.waterTestListData.paginator = this.waterTestPaginator;
    })
  }
  onGet() {
    this.billingService.getWaterTestService().subscribe(response => {
      console.log(response.data);
      this.dataSource = response.data;
      debugger;
      this.isLoading = false;
      this.waterTestListData = new MatTableDataSource(response.data);
      this.waterTestListData.sort = this.waterTestSort;
      this.waterTestListData.paginator = this.waterTestPaginator;
      this.pdfImplementation();
    })
  }
  onClick(){
    for(var i=0; i<5;i++){
        console.log(i);
    }
  }
  onDelete(id: any) {
    debugger;
    console.log(id)
    this.billingService.deleteWaterTestServiceById(id)
    debugger;
    // console.log(this.waterTestListData)
    this.onGet();
  }
  onEdit(id: any) {
    debugger;
    var waterTestDto = "";
    let formData = new FormData();
    this.billingService.updateWaterTestServiceById(this.Id ,this.waterTestingForm.value).subscribe(response => {
      console.log(response)
    })
    this.router.navigate(['operationManagement/waterTesting/editWaterTesting']);
  }
  onDownload(id: any) {
    debugger;
    console.log("On Download: " + id);
    this.billingService.getWaterTestingDoc(id).subscribe(response => {
      debugger;
      console.log(response);
      if (response["lstWaterTestingDocuments"].length != 0) {
        response["lstWaterTestingDocuments"].forEach((element: any) => {
          debugger
          var n = element.filePath.lastIndexOf('/');
          var result = element.filePath.substring(n + 1);
          var output = atob(element.byteDocumentFile);
          var length = output.length;
          var arrayBuffer = new ArrayBuffer(length);
          var uintArray = new Uint8Array(arrayBuffer);
          for (var i = 0; i < length; i++) {
            uintArray[i] = output.charCodeAt(i);
          }
          let file = new Blob([uintArray], { type: "application/octet-stream" });
          const a = document.createElement('a');
          var fileURL = URL.createObjectURL(file);
          a.href = fileURL;
          a.download = result;
          a.click();
          window.URL.revokeObjectURL(fileURL);
        });
      }
      else {
        alert("No File Found")
      }
    })
  }
  setStep(index: number) {
    this.step = index;
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList = this.dataSource;
    ['txtBranchName','txtAssetType', 'txtConsumerType',  'numConsumerNo' ,'txtSamplingDate', 'txtSamplingPoint', 'txtTestingAgency','txtResultSubmissionDate', 'txtTestResult','Actions'];
    this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Asset Type', bold: true },
      { text: 'Consumer Type', bold: true },
      { text: 'Consumer No', bold: true },
      { text: 'Sampling Date', bold: true },
      { text: 'Sampling Point', bold: true },
      { text: 'Testing Agency', bold: true },
      { text: 'Result Submission Date', bold: true },
      { text: 'Test Result', bold: true }
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      this.tubewellReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tubewellDataList.txtBranchName, alignment: 'left' },
        { text: tubewellDataList.txtLocationName, alignment: 'left' },
        { text: tubewellDataList.txtAssetType, alignment: 'left' },
        { text: tubewellDataList.txtConsumerType, alignment: 'left' },
        { text: tubewellDataList.numConsumerNo, alignment: 'right' },
        // { text: tubewellDataList.txtSamplingDate, alignment: 'right' },
        { text: this.dateFormat.transformOperation(tubewellDataList.txtSamplingDate), alignment: 'right' },  
        { text: tubewellDataList.txtSamplingPoint, alignment: 'right' },
        { text: tubewellDataList.txtTestingAgency, alignment: 'right' },
        // { text: tubewellDataList.txtResultSubmissionDate, alignment: 'right' },
        { text: this.dateFormat.transformOperation(tubewellDataList.txtResultSubmissionDate), alignment: 'right' },  
        { text: tubewellDataList.txtTestResult, alignment: 'right' }
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.tubewellReportDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "branch") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtAssetType") {
        tableHeader.push({ text: 'Asset Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtConsumerType") {
        tableHeader.push({ text: 'Consumer Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numConsumerNo") {
        tableHeader.push({ text: 'Consumer No', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtSamplingDate") {
        tableHeader.push({ text: 'Sampling Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtSamplingPoint") {
        tableHeader.push({ text: 'Sampling Point', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtTestingAgency") {
        tableHeader.push({ text: 'Testing Agency', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtResultSubmissionDate") {
        tableHeader.push({ text: 'Result Submission Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtTestResult") {
        tableHeader.push({ text: 'Test Result', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.tubewellReportDTOList.push(tableHeader);
    console.log(this.tubewellReportDTOList);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "branch") {
          tableRows.push({ text: tubewellDataList.txtBranchName, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: tubewellDataList.txtLocationName, alignment: 'left' });
        } else if (element === "txtAssetType") {
          tableRows.push({ text: tubewellDataList.txtAssetType, alignment: 'left' });
        } else if (element === "txtConsumerType") {
          tableRows.push({ text: tubewellDataList.txtConsumerType, alignment: 'left' });
        } else if (element === "numConsumerNo") {
          tableRows.push({ text: tubewellDataList.numConsumerNo, alignment: 'left' });
        } else if (element === "txtSamplingDate") {
          // tableRows.push({ text: tubewellDataList.txtSamplingDate, alignment: 'left' });
          tableRows.push({ text:  this.dateFormat.transformOperation(tubewellDataList.txtSamplingDate), alignment: 'left' });
        } else if (element === "txtSamplingPoint") {
          tableRows.push({ text: tubewellDataList.txtSamplingPoint, alignment: 'left' });
        } else if (element === "txtTestingAgency") {
          tableRows.push({ text: tubewellDataList.txtTestingAgency, alignment: 'left' });
        } else if (element === "txtResultSubmissionDate") {
          // tableRows.push({ text: tubewellDataList.txtResultSubmissionDate, alignment: 'left' });
          tableRows.push({ text:  this.dateFormat.transformOperation(tubewellDataList.txtResultSubmissionDate), alignment: 'left' });
        } else if (element === "txtTestResult") {
          tableRows.push({ text: tubewellDataList.txtTestResult, alignment: 'left' });
        }
      });
      this.tubewellReportDTOList.push(tableRows)
    });
    debugger;
    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Water Test Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.tubewellReportDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };
    pdfMake.createPdf(documentDefinition).download('Water Testing List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "branch",
    "unionCouncil",
    "txtAssetType",
    "txtConsumerType",
    "numConsumerNo",
    "txtSamplingDate",
    "txtTestingAgency",
    "txtSamplingPoint",
    "txtResultSubmissionDate",
    "txtTestResult"
  ];
  selection = new FormControl([
    "sr",
    "branch",
    "unionCouncil",
    "txtAssetType",
    "txtConsumerType",
    "numConsumerNo",
    "txtSamplingDate",
    "txtTestingAgency",
    "txtSamplingPoint",
    "txtResultSubmissionDate",
    "txtTestResult"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}

