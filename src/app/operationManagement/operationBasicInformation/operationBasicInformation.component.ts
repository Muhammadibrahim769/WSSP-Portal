...
@Component({
  ...
  templateUrl: './operationBasicInformation.component.html',
  providers: [
    {
      provide: DateAdapter,
...
export class OperationBasicInformationComponent implements OnInit {
  operationBasicInfoForm: FormGroup
  step = 0;
  show = false;
  updatedTWBasicinfo: any[] = [];
  responseList: any[] = [];
  searchTubeWellData: any[] = [];
  isLoading = true;
  dataSource = null;
  date = new FormControl(moment());
  pageSizeOptions: number[] = [10, 20, 50];
  operationBasicInfoColumns: string[] = ['txt_branch_name', 'txt_location_name','txtTubewellRefNo', 'txtProductCode', 'txtProductName', 'txtMeterType', 'numOfConnections', 'AvgDischarge', 'numAvgDischargeVolume', 'blnIsFunctional', 'blnIsMetered', 'dteWarrenty', 'Actions'];
  ...
  constructor(private formBuilder: FormBuilder, private utilService: UtilsService, private changeDetectorRefs: ChangeDetectorRef, public dialog: MatDialog, private billingService: BillingService, private router: Router, private dateFormat: DateFormatPipe) { }
  ucList: any;
  user = "";
  getBranchId = [];
  getUClist = [];
  ...
  ngOnInit(): void {
    ...
    this.refresh();
    this.getBranchList();
    this.operationBasicInfoForm.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
  ...
  onSearch() {
   debugger;  
    let blnIsFunctional = this.operationBasicInfoForm.controls.blnIsFunctional.value;
    let blnIsMetered = this.operationBasicInfoForm.controls.blnIsMetered.value;
    let serBranchId = this.operationBasicInfoForm.controls.serBranchId.value;
    let serLocationId = this.operationBasicInfoForm.controls.serLocationId.value;
    let txtProductCode = this.operationBasicInfoForm.controls.txtProductCode.value;
    let txtProductName = this.operationBasicInfoForm.controls.txtProductName.value;
    let txtTubewellRefNo = this.operationBasicInfoForm.controls.txtTubewellRefNo.value;
    this.billingService.getOperationBasicInfoServiceByParams(txtTubewellRefNo,blnIsFunctional, blnIsMetered, serBranchId, serLocationId, txtProductCode, txtProductName).subscribe(Response => {
      debugger;
      this.searchTubeWellData = Response["data"];
      console.log("Response");
      console.log(Response);
      this.isLoading = false;
      this.dataSource = Response["data"];
      // this.selecttubewellList = [];
  ...
  openEditModal(id: any) {
    const EditData = this.dialog.open(EditModalComponent, {
      data: id
    })
   ...
   debugger;
    ...
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      ...
      this.tubewellReportDTOList.push([
        ...
        { text: tubewellDataList.txtMeterType, alignment: 'right' },
        { text: tubewellDataList.numOfConnections, alignment: 'right' },
        { text: tubewellDataList.numAvgDischargeVolume, alignment: 'right' },
        { text: tubewellDataList.numAvgDischargeVolum, alignment: 'right' },
        { text: tubewellDataList.blnIsFunctional, alignment: 'right' },
        { text: tubewellDataList.blnIsMetered, alignment: 'right' },
        { text: this.dateFormat.transformOperation(tubewellDataList.dteWarrenty), alignment: 'right' }        
      ]);
    });
  }
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      ...
      } else if (element === "txtMeterType") {
        tableHeader.push({ text: 'Meter Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numOfConnections") {
        tableHeader.push({ text: 'No of Connections', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "AvgDischarge") {
        tableHeader.push({ text: 'Avg Discharge m3/hr', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numAvgDischargeVolume") {
        tableHeader.push({ text: 'Avg Discharge m3/unit', bold: true, fillColor: '#dddddd', alignment: 'center' });
      ...
      } else if (element === "dteWarrenty") {
        tableHeader.push({ text: 'Warrenty', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    ...
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      ...
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: tubewellDataList.txtTubewellRefNo, alignment: 'left' });
        }else if (element === "txtProductCode") {
          ...
          tableRows.push({ text: tubewellDataList.txtMeterType, alignment: 'left' });
        } else if (element === "numOfConnections") {
          tableRows.push({ text: tubewellDataList.numOfConnections, alignment: 'right' });
        } else if (element === "AvgDischarge") {
          tableRows.push({ text: tubewellDataList.numAvgDischargeVolume, alignment: 'right' });
        } else if (element === "numAvgDischargeVolume") {
          tableRows.push({ text: tubewellDataList.numAvgDischargeVolum, alignment: 'right' });
        } else if (element === "blnIsFunctional") {
          ...
          tableRows.push({ text:  this.dateFormat.transformOperation(tubewellDataList.dteWarrenty), alignment: 'left' });
        }
      });
      ...
      content: [
        {
          ...
          table: {
            ...
            body: [
              ...
              }, [{ text: 'Tubewell Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
    ...
    pdfMake.createPdf(documentDefinition).download('TubeWell List.pdf');
  } 
  tableColumn: string[] = [
    "sr",
    "txt_branch_name",
  ...
  onReset(){
    this.operationBasicInfoForm.controls.serBranchId.reset('')
    this.operationBasicInfoForm.controls.serLocationId.reset('')
    this.operationBasicInfoForm.controls.txtBranch.reset('')
    this.operationBasicInfoForm.controls.txtUnionCouncil.reset('')
    this.operationBasicInfoForm.controls.txtProductCode.reset('')
    this.operationBasicInfoForm.controls.txtProductName.reset('')
    this.operationBasicInfoForm.controls.blnIsMetered.reset('')
    this.operationBasicInfoForm.controls.blnIsFunctional.reset('')     
    this.onGet();
  }
}
