...
@Component({
  ...
  templateUrl: './generaloperation.component.html',
  providers: [
    {
      provide: DateAdapter,
...
export class GeneraloperationComponent implements OnInit {

  generalOperationForm: FormGroup
  step = 0;
  user = "";
  ucList: any;
  getBranchId = [];
  ...
  branchList: any;
  // i=0;
  constructor(private formBuilder: FormBuilder, private router: Router, private shared: CrudService, private route: ActivatedRoute, private dateFormat: DateFormatPipe, private utilService: UtilsService, public dialog: MatDialog, private billingService: BillingService) { }
  
  generalOperationColumns: string[] = ['serNo', 'branch', 'unionCouncil', 'txtMonth','numOfBillDistributed', 'numOfPrimaryMainholes', 'numOfSecondaryMainholes', 'numOfTeriaryMainholes','numSewerageLinesRepaired','numDrainageLinesRepaired','numDistributionLinesRepaired',  'numPriMhCleaned', 'numSecMhCleaned', 'numTerMhCleaned', 'numSewerageLinesCleaned','numDrainageLinesCleaned','numDistributionLinesCleaned','numSewerageLineDiscelitation','numDrainageLinesDiscelitation','numDistributionLinesDiscelitation',  'Actions'];
  generalOperation = new MatTableDataSource();
  @ViewChild("generalOperationSort", { static: true }) generalOperationSort: MatSort;
  @ViewChild("generalOperationPaginator", { static: true }) generalOperationPaginator: MatPaginator;
  ngAfterViewInit() { }
  clearPeriod() {
    this.generalOperationForm.controls.txtMonth.reset('');
  }
  // date = new FormControl(moment());

  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue = this.generalOperationForm.controls.txtMonth.value;
    ctrlValue.year(normalizedYear.year());
    this.generalOperationForm.controls.txtMonth.setValue(ctrlValue);
  }
  ...
  chosenMonthHandler(normalizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
    ...
    console.log(this.generalOperationForm.controls.txtMonth);
    datepicker.close();
  }
  ...
  ngOnInit(): void {
    ...
    this.generalOperationForm = this.formBuilder.group({
      txtBranchName: [''],
      txtLocationName: [''],
      serBranchId: [''],
      serLocationId: [''],
    ...
    this.getBranchList();
    this.generalOperationForm.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
  ...
  onSubmit() {
    debugger;
    console.log(this.generalOperationForm.value)
    let txtMonth = this.dateFormat.transform(this.generalOperationForm.controls.txtMonth.value);
    let serBranchId = this.generalOperationForm.controls.serBranchId.value;
    let serLocationId = this.generalOperationForm.controls.serLocationId.value;

    console.log(txtMonth);
    // let brachID = this.generalOperationForm.controls.txtBranchName.value;
    // let UcID = this.generalOperationForm.controls.txtLocationName.value;
    this.billingService.getGeneralOperationByMonth(txtMonth, serBranchId, serLocationId).subscribe(response => {
      debugger;
      this.selectStatusList = [];
      console.log(response);
      this.responseList = response["data"]
      
      this.generalOperation = new MatTableDataSource(response["data"]);
      this.generalOperation.sort = this.generalOperationSort;
      this.generalOperation.paginator = this.generalOperationPaginator;
  ...
  openEditModel(element) {
    ...
    console.log(this.generalOperationForm.value)
    const EditData = this.dialog.open(EditGeneralOperationComponent, {
      data: element

    })
    EditData.afterClosed().subscribe(() => {
      this.onSubmit()
     });
  }
  onView(element) {
    ...
    this.shared.changeMessage(this.viewData);
    this.router.navigate(['/operationManagement/generalOperation/viewGeneralOperation']);
    // this.router.navigate(['/waterProduction/viewwaterProduction']);
  }
  setStep(index: number) {
    this.step = index;
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList = this.responseList; 
    debugger;
     this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Month', bold: true },
      { text: 'No of RPC Mainholes Repaired', bold: true },
      { text: 'No of RCC Mainholes Repaired', bold: true },
      { text: 'No of IronGrid Mainholes Repaired', bold: true },
      { text: 'Sewerage Lines Repaired', bold: true },
      { text: 'Drainage Line Repaired', bold: true },
      { text: 'RPC Mainholes Cleaned', bold: true },
      { text: 'RCC Mainholes Cleaned', bold: true },
      { text: 'Irongrid Mainholes Cleaned', bold: true },
      { text: 'Sewerage Lines Cleaned', bold: true },
      { text: 'Drainage Line Cleaned', bold: true },
      { text: 'Distribution Line Repaired', bold: true },
      { text: 'Distribution Line Cleaned', bold: true },
      { text: 'Sewerage Line Discelitation', bold: true },      
      { text: 'Drainage Line Discelitation', bold: true },
      { text: 'Distribution Line Discelitation', bold: true }
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      this.tubewellReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tubewellDataList.txtBranchName, alignment: 'left' },
        { text: tubewellDataList.txtLocationName, alignment: 'left' },
        { text: tubewellDataList.txtMonth, alignment: 'left' },
        { text: tubewellDataList.numPriMhRepaired, alignment: 'left' },
        { text: tubewellDataList.numSecMhRepaired, alignment: 'right' },
        { text: tubewellDataList.numTerMhRepaired, alignment: 'right' },
        { text: tubewellDataList.numSewerageLinesRepaired, alignment: 'right' },
        { text: tubewellDataList.numDrainageLinesRepaired, alignment: 'right' },
        { text: tubewellDataList.numPriMhCleaned, alignment: 'right' },
        { text: tubewellDataList.numSecMhCleaned, alignment: 'right' },
        { text: tubewellDataList.numTerMhCleaned, alignment: 'right' },
        { text: tubewellDataList.numSewerageLinesCleaned, alignment: 'right' },
        { text: tubewellDataList.numDrainageLinesCleaned, alignment: 'right' },
        { text: tubewellDataList.numDistributionLinesRepaired, alignment: 'right' },
        { text: tubewellDataList.numDistributionLinesCleaned, alignment: 'right' },
        { text: tubewellDataList.numSewerageLineDiscelitation, alignment: 'right' },
        { text: tubewellDataList.numDrainageLinesDiscelitation, alignment: 'right' },
        { text: tubewellDataList.numDistributionLinesDiscelitation, alignment: 'right' }
  ...
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      if (element === "sr") {
        ...
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtMonth") {
        tableHeader.push({ text: 'Month', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numOfPrimaryMainholes") {
        tableHeader.push({ text: 'No of RPC Mainholes Repaired', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numOfSecondaryMainholes") {
        tableHeader.push({ text: 'No of RCC Mainholes Repaired', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numOfTeriaryMainholes") {
        tableHeader.push({ text: 'No of IronGrid Mainholes Repaired', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numSewerageLinesRepaired") {
        tableHeader.push({ text: 'Sewerage Lines Repaired', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }  else if (element === "numDrainageLinesRepaired") {
        tableHeader.push({ text: 'Drainage Lines Repaired', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "numPriMhCleaned") {
        tableHeader.push({ text: 'RPC Mainholes Cleaned', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numSecMhCleaned") {
        tableHeader.push({ text: 'RCC Mainholes Cleaned', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numTerMhCleaned") {
        tableHeader.push({ text: 'Irongrid Mainholes Cleaned', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numSewerageLinesCleaned") {
        tableHeader.push({ text: 'Sewerage Lines Cleaned', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }     
       else if (element === "numDrainageLinesCleaned") {
        tableHeader.push({ text: 'Drainage Lines Cleaned', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numDistributionLinesRepaired") {
        tableHeader.push({ text: 'Distribution Line Repaired', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numDistributionLinesCleaned") {
        tableHeader.push({ text: 'Distribution Line Cleaned', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numSewerageLineDiscelitation") {
        tableHeader.push({ text: 'Sewerage Line Discelitation', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numDrainageLinesDiscelitation") {
        tableHeader.push({ text: 'Drainage Line Discelitation', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }     
       else if (element === "numDistributionLinesDiscelitation") {
        tableHeader.push({ text: 'Distribution Line Discelitation', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.tubewellReportDTOList.push(tableHeader);
    console.log(this.tubewellReportDTOList);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: tubewellDataList.txtBranchName, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: tubewellDataList.txtLocationName, alignment: 'left' });
        } else if (element === "txtMonth") {
          tableRows.push({ text: tubewellDataList.txtMonth, alignment: 'left' });
        } else if (element === "numOfPrimaryMainholes") {
          tableRows.push({ text: tubewellDataList.numPriMhRepaired, alignment: 'left' });
        } else if (element === "numOfSecondaryMainholes") {
          tableRows.push({ text: tubewellDataList.numSecMhRepaired, alignment: 'left' });
        } else if (element === "numOfTeriaryMainholes") {
          tableRows.push({ text: tubewellDataList.numTerMhRepaired, alignment: 'left' });
        } else if (element === "numSewerageLinesRepaired") {
          tableRows.push({ text: tubewellDataList.numSewerageLinesRepaired, alignment: 'left' });
        }else if (element === "numDrainageLinesRepaired") {
          tableRows.push({ text: tubewellDataList.numDrainageLinesRepaired, alignment: 'left' });
        } else if (element === "numPriMhCleaned") {
          tableRows.push({ text: tubewellDataList.numPriMhCleaned, alignment: 'left' });
        } else if (element === "numSecMhCleaned") {
          tableRows.push({ text: tubewellDataList.numSecMhCleaned, alignment: 'left' });
        } else if (element === "numTerMhCleaned") {
          tableRows.push({ text: tubewellDataList.numTerMhCleaned, alignment: 'left' });
        } else if (element === "numSewerageLinesCleaned") {
          tableRows.push({ text: tubewellDataList.numSewerageLinesCleaned, alignment: 'left' });
        } else if (element === "numDrainageLinesCleaned") {
          tableRows.push({ text: tubewellDataList.numDrainageLinesCleaned, alignment: 'left' });
        } else if (element === "numDistributionLinesRepaired") {
          tableRows.push({ text: tubewellDataList.numDistributionLinesRepaired, alignment: 'left' });
        } else if (element === "numDistributionLinesCleaned") {
          tableRows.push({ text: tubewellDataList.numDistributionLinesCleaned, alignment: 'left' });
        } else if (element === "numSewerageLineDiscelitation") {
          tableRows.push({ text: tubewellDataList.numSewerageLineDiscelitation, alignment: 'left' });
        } else if (element === "numDrainageLinesDiscelitation") {
          tableRows.push({ text: tubewellDataList.numDrainageLinesDiscelitation, alignment: 'left' });
        } else if (element === "numDistributionLinesDiscelitation") {
          tableRows.push({ text: tubewellDataList.numDistributionLinesDiscelitation, alignment: 'left' });
        } 
      });
      this.tubewellReportDTOList.push(tableRows)
    });
    debugger;
    ...
    const documentDefinition = {
      ...
      content: [
        ...
        },
        {
          ...
          table: {
            headerRows: 1,
            body: this.tubewellReportDTOList
          }
        },
      ],
      styles: {
    ...
    pdfMake.createPdf(documentDefinition).download('General Operation List.pdf');
  }
  ...
  tableColumn: string[] = [
    ...
    "unionCouncil",
    "txtMonth",
    "numOfPrimaryMainholes",
    "numOfSecondaryMainholes",
    "numOfTeriaryMainholes",
    "numSewerageLinesRepaired",
    "numPriMhCleaned",
    "numSecMhCleaned",
    "numTerMhCleaned",
    "numSewerageLinesCleaned",
    "numDrainageLinesCleaned",
    "numDrainageLinesRepaired",
    "numDistributionLinesRepaired",
    "numDistributionLinesCleaned",
    "numSewerageLineDiscelitation", 
    "numDrainageLinesDiscelitation", 
    "numDistributionLinesDiscelitation"
  ...
  onReset(){
    this.generalOperationForm.controls.serBranchId.reset('')
    this.generalOperationForm.controls.serLocationId.reset('')
    this.generalOperationForm.controls.txtBranchName.reset('')
    this.generalOperationForm.controls.txtLocationName.reset('')       
    this.onSubmit();
  }
