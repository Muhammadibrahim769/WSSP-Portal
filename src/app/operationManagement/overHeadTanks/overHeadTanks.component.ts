import { Component, AfterViewInit, OnInit, ViewChild,ChangeDetectorRef  } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { Router } from '@angular/router';
import { RejectModalComponent } from '@app/modals/rejectModal.component';
import { MomentDateAdapter, MAT_MOMENT_DATE_ADAPTER_OPTIONS } from '@angular/material-moment-adapter';
import { DateAdapter, MAT_DATE_FORMATS, MAT_DATE_LOCALE } from '@angular/material/core';
import * as _moment from 'moment';
import { EditModalComponent } from '@app/modals/editModal.component';
import { BillingService } from '@app/services/billing.service';
import { UtilsService } from '@app/_services/utils.service';
import { EditOverHeadTankComponent } from '@app/modals/editOverHeadTank.component';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
const moment = _moment;
export const MY_FORMATS = {
  parse: {
    dateInput: 'LL',
  },
  display: {
    dateInput: 'LL',
    monthYearLabel: 'MMM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MMMM YYYY',
  },
};

@Component({
  selector: 'app-dashboard',
  templateUrl:'./overHeadTanks.component.html',
  providers: [
    // `MomentDateAdapter` can be automatically provided by importing `MomentDateModule` in your
    // application's root module. We provide it at the component level here, due to limitations of
    // our example generation script.
    {
      provide: DateAdapter,
      useClass: MomentDateAdapter,
      deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS]
    },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class OverHeadTanksComponent implements AfterViewInit {
  operationBasicInfoForm: FormGroup
  step = 0;
  user = "";
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  show = false;
  searchOverHeadData: any[] = [];
  isLoading = true;
  dataSource = null;
  updatedTWBasicinfo:any [] = [];
  responseList:any [] = [];
  date = new FormControl(moment());
  pageSizeOptions: number[] = [10, 20, 50];
  operationBasicInfoColumns: string[] = ['txt_branch_name','txt_location_name','txtProductCodeOH', 'txtProductNameOH', 'blnIsFunctional', 'Actions'];
  operationBasicInfo = new MatTableDataSource();
  @ViewChild("operationBasicInfoSort", { static: true }) operationBasicInfoSort: MatSort;
  @ViewChild("operationBasicInfoPaginator", { static: true }) operationBasicInfoPaginator: MatPaginator;
  ngAfterViewInit() {
  }
  constructor(private formBuilder: FormBuilder, private utilService: UtilsService,private changeDetectorRefs: ChangeDetectorRef, public dialog: MatDialog, private billingService: BillingService, private router: Router) { }
  unionCouncilList: any[] = [];
  ngOnInit(): void {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.operationBasicInfoForm = this.formBuilder.group({
      txtBranch: [""],
      txtUnionCouncil: [''],  
      txtProductCode: [''],  
      txtProductName:[''],  
      blnIsFunctional: [''],
      serBranchId:[''],
      serLocationId:['']
    })       
    this.onGet();
    this.refresh();
    this.getBranchList();
    this.operationBasicInfoForm.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
  
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);
    debugger;
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {
      debugger;
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  onSearch() {
    let blnIsFunctional = this.operationBasicInfoForm.controls.blnIsFunctional.value;
    let serBranchId = this.operationBasicInfoForm.controls.serBranchId.value;
    let serLocationId = this.operationBasicInfoForm.controls.serLocationId.value;
    let txtProductCode = this.operationBasicInfoForm.controls.txtProductCode.value;
    let txtProductName = this.operationBasicInfoForm.controls.txtProductName.value;

    this.billingService.getOverHeadTankServiceByParams(blnIsFunctional,serBranchId, serLocationId, txtProductCode, txtProductName).subscribe(Response => {
      this.searchOverHeadData = Response["data"];
      console.log("Response");
      console.log(Response);
      this.isLoading = false;
      this.dataSource = Response["data"];
      // this.selectkpiList = [];
      this.operationBasicInfo = new MatTableDataSource(Response["data"]);
      this.operationBasicInfo.sort = this.operationBasicInfoSort;
      this.operationBasicInfo.paginator = this.operationBasicInfoPaginator;
      this.pdfImplementation();
    })
  }
  onGet(){
    this.billingService.getOverHeadTankService().subscribe(response => {
      console.log("operationBasicInfo")
      console.log(response.data);
      debugger;
      this.isLoading = false;
      this.dataSource = response.data;
      this.operationBasicInfo = new MatTableDataSource(response.data);
      this.operationBasicInfo.sort = this.operationBasicInfoSort;
      this.operationBasicInfo.paginator = this.operationBasicInfoPaginator;
      this.pdfImplementation();
      // this.changeDetectorRefs.detectChanges();
    })
  }
  refresh() {
    
      this.changeDetectorRefs.detectChanges();
 
  }
  openEditModal(id: any) {
    const EditData = this.dialog.open(EditOverHeadTankComponent, {
      data: id
    })
    EditData.afterClosed().subscribe(() => {
      this.onGet();
     });
  }
  
  openSubmitDialog() {
   
    const document = this.dialog.open(RejectModalComponent, {
      data: {
        message: 'Are you sure want to Submit ?',
        buttonText: {
          ok: 'Yes',
          cancel: 'No'
        }
      }
    });
    document.afterClosed().subscribe((confirmed: boolean) => {
      debugger;
      if (confirmed) {
        this.show = true;
      }
    });
  }
  showColumn = false;
  setStep(index: number) {
    this.step = index;
  }
  overheadDataList: any[] = [];
  overheadReportDTOList: any[] = [];
  pdfImplementation() {
   debugger;
    this.overheadDataList = this.dataSource;
    console.log("daownload data ")
    console.log(this.overheadDataList)
    debugger;
    this.overheadReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Tubewell Number', bold: true },
      { text: 'Tubewell Name', bold: true },     
      { text: 'Functional', bold: true }         
    ]);
    this.overheadDataList.forEach((overheadDataList, i) => {
      debugger;
      this.overheadReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: overheadDataList.txtBranchName, alignment: 'left' },
        { text: overheadDataList.txtLocationName, alignment: 'left' },
        { text: overheadDataList.txtProductCode, alignment: 'left' },
        { text: overheadDataList.txtProductName, alignment: 'left' },       
        { text: overheadDataList.blnIsFunctional, alignment: 'right' }
      
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.overheadReportDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txt_branch_name") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txt_location_name") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtProductCodeOH") {
        tableHeader.push({ text: 'OverHead Number', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtProductNameOH") {
        tableHeader.push({ text: 'OverHead Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "blnIsFunctional") {
        tableHeader.push({ text: 'Functional', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
    });
    this.overheadReportDTOList.push(tableHeader);
    console.log(this.overheadReportDTOList);
    this.overheadDataList.forEach((overheadDataList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txt_branch_name") {
          tableRows.push({ text: overheadDataList.txtBranchName, alignment: 'left' });
        } else if (element === "txt_location_name") {
          tableRows.push({ text: overheadDataList.txtLocationName, alignment: 'left' });
        } else if (element === "txtProductCodeOH") {
          tableRows.push({ text: overheadDataList.txtProductCode, alignment: 'left' });
        } else if (element === "txtProductNameOH") {
          tableRows.push({ text: overheadDataList.txtProductName, alignment: 'left' });
        }else if (element === "blnIsFunctional") {
          tableRows.push({ text: overheadDataList.blnIsFunctional, alignment: 'left' });
        }
      });
      this.overheadReportDTOList.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'OverHead Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.overheadReportDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Overhead List.pdf');
  } 
  tableColumn: string[] = [
    "sr",
    "txt_branch_name",
    "txt_location_name",
    "txtProductCodeOH",
    "txtProductNameOH",    
    "blnIsFunctional" 
  ];
  selection = new FormControl([
    "sr",
    "txt_branch_name",
    "txt_location_name",
    "txtProductCodeOH",
    "txtProductNameOH",   
    "blnIsFunctional"    
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
  onReset(){
    this.operationBasicInfoForm.controls.serBranchId.reset('')
    this.operationBasicInfoForm.controls.serLocationId.reset('')
    this.operationBasicInfoForm.controls.txtBranch.reset('')
    this.operationBasicInfoForm.controls.txtUnionCouncil.reset('')
    this.operationBasicInfoForm.controls.txtProductCode.reset('')
    this.operationBasicInfoForm.controls.txtProductName.reset('')
    this.operationBasicInfoForm.controls.blnIsFunctional.reset('')     
    this.onGet();
  }
}
