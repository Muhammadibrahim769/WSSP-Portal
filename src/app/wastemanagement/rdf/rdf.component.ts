import { Component, OnInit, ViewChild } from "@angular/core";
import { FormBuilder, FormControl, FormGroup } from "@angular/forms";
import { MatPaginator } from "@angular/material/paginator";
import { MatSort } from "@angular/material/sort";
import { MatTableDataSource } from "@angular/material/table";
import { BillingService } from "@app/services/billing.service";
import { DateFormatPipe } from "@app/_helpers/date-format.pipe";
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-rdf',
  templateUrl: './rdf.component.html'
})

export class RdfComponent implements OnInit {

  searchform: FormGroup;
  branchList: any;
  unionCouncilList: any;
  ucList: any;
  getUC = [];
  rdfList: any = "";
  rdfDTO: any[] = [];
  step = 0;
  user: any;
  rdfDataColumn: string[] = ['txtBranchName', 'txtLocationName', 'txtSegregatedRecieved', 'txtPalletProduced', 'dteRdf', 'txtRdfSold', 'txtRate', 'txtVendor', 'numAmount', 'numRdfBalance', 'Actions'];
  rdfData = new MatTableDataSource();
  @ViewChild("stpDataSort", { static: true }) stpDataSort: MatSort;
  @ViewChild("rdfDataPaginator", { static: true }) rdfDataPaginator: MatPaginator;

  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtSegregatedRecieved",
    "txtPalletProduced",
    "dteRdf",
    "txtRdfSold",
    "txtRate",
    "txtVendor",
    "numAmount",
    "numRdfBalance"
  ]);
  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtSegregatedRecieved",
    "txtPalletProduced",
    "dteRdf",
    "txtRdfSold",
    "txtRate",
    "txtVendor",
    "numAmount",
    "numRdfBalance"
  ];
  constructor(private formBuilder: FormBuilder, private billingService: BillingService, private dateFormat: DateFormatPipe) { }

  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.searchform = this.formBuilder.group({
      branch: [''],
      unionCouncil: [''],
      dteRdf: [''],
      txtSegregatedRecieved: [''],
      txtVendor: ['']
    })
    this.billingService.getAllCustomerTemplateService().subscribe(response => {
      debugger;
      console.log(response)
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];
    })

    this.searchform.controls['branch'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getUC = [];
        for (let getBranch of this.ucList) {
          this.getUC.push(getBranch);
        }
        console.log("this.getUC");
        console.log(this.getUC);
      })
    })
    
    this.onReset();
  }

  onSearch() {
    debugger;
    console.log("onSearch");
    console.log(this.searchform.value);
    let bracnhId = +this.searchform.controls.branch.value;
    let ucID = +this.searchform.controls.unionCouncil.value;
    let txtSegregatedRecieved = this.searchform.controls.txtSegregatedRecieved.value;
    let txtVendor = this.searchform.controls.txtVendor.value;
    let dteRdf = '';
    if (this.searchform.controls.dteRdf.value != "") {
      dteRdf = this.dateFormat.transformOperation(this.searchform.controls.dteRdf.value);
    }

    this.billingService.findRdfService(bracnhId, ucID, dteRdf, txtSegregatedRecieved, txtVendor,).subscribe(response => {
      console.log("findRdfService");
      console.log(response);
      if (response.data.length == 0) {
        alert('No record Found');
        this.rdfList = [];
        this.rdfData = new MatTableDataSource([]);
      }
      else {
        this.rdfList = response.data;
        this.rdfData = new MatTableDataSource(response.data);
      }
    });
  }

  clearTo() {
    this.searchform.controls.dteRdf.reset('');
  }

  setStep(index: number) {
    this.step = index;
  }

  onReset() {
    this.billingService.findAllStp().subscribe(response => {
      response;
      this.rdfList = response.data;
      this.rdfData = new MatTableDataSource(response.data);
    })
  }

  downloadPDF() {
    this.rdfDTO = [];
    let tableHeader: any[] = [];
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtSegregatedRecieved") {
        tableHeader.push({ text: 'Segregated Recieved', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtPalletProduced") {
        tableHeader.push({ text: 'Pallet Produced', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "dteRdf") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtRdfSold") {
        tableHeader.push({ text: 'Rdf Sold', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRate") {
        tableHeader.push({ text: 'Rate', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtVendor") {
        tableHeader.push({ text: 'Vendor', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numAmount") {
        tableHeader.push({ text: 'Amount', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numRdfBalance") {
        tableHeader.push({ text: 'Rdf Balance', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    })

    this.rdfDTO.push(tableHeader);
    this.rdfList.forEach((data, i) => {
      let tableRows: any[] = [];
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: data.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: data.txtLocationName, alignment: 'left' });
        } else if (element === "txtSegregatedRecieved") {
          tableRows.push({ text: data.txtSegregatedRecieved, alignment: 'left' });
        } else if (element === "txtPalletProduced") {
          tableRows.push({ text: data.txtPalletProduced, alignment: 'left' });
        } else if (element === "dteRdf") {
          tableRows.push({ text: data.dteRdf, alignment: 'left' });
        } else if (element === "txtRdfSold") {
          tableRows.push({ text: data.txtRdfSold, alignment: 'left' });
        } else if (element === "txtRate") {
          tableRows.push({ text: data.txtRate, alignment: 'left' });
        } else if (element === "txtVendor") {
          tableRows.push({ text: data.txtVendor, alignment: 'left' });
        } else if (element === "numAmount") {
          tableRows.push({ text: data.numAmount, alignment: 'left' });
        } else if (element === "numRdfBalance") {
          tableRows.push({ text: data.numRdfBalance, alignment: 'left' });
        }
      })
      this.rdfDTO.push(tableRows);
    })

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'RDF', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.rdfDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('RDF List.pdf');
  }

  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}