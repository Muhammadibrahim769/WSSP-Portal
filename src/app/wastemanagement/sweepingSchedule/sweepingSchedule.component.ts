import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './sweepingSchedule.component.html'
})
export class SweepingScheduleComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  wasteCollectionColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName', 'txtDate','txtShift', 'txtTypeOfArea', 'txtSweepingMode', 'numLength', 'txtScheduleSweeping', 'txtStartPointCoord', 'txtFinishPointCoord', 'numOfRequiredDays', 'txtStaffDeputed',
    'Actions'];
    wasteCollectionData = new MatTableDataSource();
  @ViewChild("wasteCollectionSort", { static: true }) wasteCollectionSort: MatSort;
  @ViewChild("wasteCollectionPaginator", { static: true }) wasteCollectionPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe, ) {
  }
  clearWEFD() {
    this.form.controls.dteDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  clearDateServicePerformed() {
    this.form.controls.txtDate.reset('');
  }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtDate: [''],
      txtShift:['']
    })
    this.getBranchList();
    this.getSweepingScheduleFindAll();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
      })
    })
    this.user = JSON.parse(sessionStorage.getItem('user'));
  }
  getBranchList() {   
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {  
      this.branchList = response
    })
  }
  getSweepingScheduleFindAll() {
    debugger;
    this.billingService.getSweepingScheduleFindAll().subscribe(response => {
      this.isLoading = false;
      this.dataSource = response["data"];
      this.wasteCollectionList = response["data"];
      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.wasteCollectionSort;
      this.wasteCollectionData.paginator = this.wasteCollectionPaginator;
    })
  }
  onSearch() {
    debugger;
    
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtShift = this.form.controls.txtShift.value;
    // let txtDate = this.dateFormat.transformOperation(this.form.controls.txtDate.value);
    let txtDate = '';
    if (this.form.controls.txtDate.value != "") {
      txtDate = this.dateFormat.transformOperation(this.form.controls.txtDate.value);
    }
    this.billingService.getSweepingScheduleFindByParams(serBranchId, serLocationId, txtDate,txtShift).subscribe(response => {
      debugger;
      this.wasteCollectionList = response["data"];
      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.wasteCollectionSort;
      this.wasteCollectionData.paginator = this.wasteCollectionPaginator;
    })
  }
  wasteCollectionList: any = "";
  // wasteCollectionList: any[] = [];
  wasteCollectionDTO: any[] = [];
  downloadPDF() {
    this.wasteCollectionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtDate") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
      else if (element === "txtShift") {
        tableHeader.push({ text: 'Shift', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtTypeOfArea") {
        tableHeader.push({ text: 'Type Of Area', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtSweepingMode") {
        tableHeader.push({ text: 'Mode Od Sweeping', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numLength") {
        tableHeader.push({ text: 'Length (km)', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtScheduleSweeping") {
        tableHeader.push({ text: 'Schedule & Frequency of Sweeping', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtStartPointCoord") {
        tableHeader.push({ text: 'Start Point Coord', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtFinishPointCoord") {
        tableHeader.push({ text: 'Finish Point Coord', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numOfRequiredDays") {
        tableHeader.push({ text: 'No of Required Days', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtStaffDeputed") {
        tableHeader.push({ text: 'Staff Deputed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }

    });
    this.wasteCollectionDTO.push(tableHeader);
    this.wasteCollectionList.forEach((wasteCollectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: wasteCollectionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: wasteCollectionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtDate") {
          tableRows.push({ text: this.dateFormat.transformOperation(wasteCollectionList.txtDate.toString()), alignment: 'left' });
        } else if (element === "txtShift") {
          tableRows.push({ text: wasteCollectionList.txtShift, alignment: 'left' });
        }else if (element === "txtTypeOfArea") {
          tableRows.push({ text: wasteCollectionList.txtTypeOfArea, alignment: 'left' });
        } else if (element === "txtSweepingMode") {
          tableRows.push({ text: wasteCollectionList.txtSweepingMode, alignment: 'left' });
        } else if (element === "numLength") {
          tableRows.push({ text: wasteCollectionList.numLength, alignment: 'left' });
        } else if (element === "txtScheduleSweeping") {
          tableRows.push({ text: wasteCollectionList.txtScheduleSweeping, alignment: 'left' });
        } else if (element === "txtStartPointCoord") {
          tableRows.push({ text: wasteCollectionList.txtStartPointCoord, alignment: 'left' });
        }
        else if (element === "txtFinishPointCoord") {
          tableRows.push({ text: wasteCollectionList.txtFinishPointCoord, alignment: 'left' });
        } else if (element === "numOfRequiredDays") {
          tableRows.push({ text: wasteCollectionList.numOfRequiredDays, alignment: 'left' });
        } else if (element === "txtStaffDeputed") {
          tableRows.push({ text: wasteCollectionList.txtStaffDeputed, alignment: 'left' });
        }
      });
      this.wasteCollectionDTO.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Sweeping Schedule', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.wasteCollectionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Sweeping Schedule List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDate",
    "txtShift",
    "txtTypeOfArea",
    "txtSweepingMode",
    "numLength",
    "txtScheduleSweeping",
    "txtStartPointCoord",
    "numOfRequiredDays",
    "txtFinishPointCoord",
    "txtStaffDeputed",
    "txtFrequencyWasteCollection",
    "numTotalWasteCollected"
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDate",
    "txtShift",
    "txtTypeOfArea",
    "txtSweepingMode",
    "numLength",
    "txtScheduleSweeping",
    "txtStartPointCoord",
    "numOfRequiredDays",
    "txtFinishPointCoord",
    "txtStaffDeputed",
    "txtFrequencyWasteCollection",
    "numTotalWasteCollected"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
