import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-dashboard',
  templateUrl: './desiltationActivity.component.html'
})
export class DesiltationActivityComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  wasteCollectionColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName', 'txtDateServicePerformed', 'txtCategory', 'txtStreetName','txtModality',
    'numStreetId', 'txtStartPointCoord', 'txtFinishPointCoord', 'numHouses', 'txtModeOfCollection', 'txtFrequencyWasteCollection', 'numTotalWasteCollected',
    'Actions'];
  wasteCollectionData = new MatTableDataSource();
  @ViewChild("desiltationActivitySort", { static: true }) desiltationActivitySort: MatSort;
  @ViewChild("desiltationActivityPaginator", { static: true }) desiltationActivityPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe,) {
  }
  clearWEFD() {
    this.form.controls.dteDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  clearDateServicePerformed() {
    this.form.controls.txtDateServicePerformed.reset('');
  }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtDateServicePerformed: [''],
      txtDateServicePerformedEnd: [''],
      txtCategory:[''],
      txtActivity: ['Desiltation']
    })
    this.getBranchList();
    this.getDesiltationActivity();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {

      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.user = JSON.parse(sessionStorage.getItem('user'));

    console.log(this.user);
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);

    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {

      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  getDesiltationActivity() {
    let txtActivity = this.form.controls.txtActivity.value;
    this.billingService.getRecordByActivity(txtActivity).subscribe(response => {

      this.tripDataList = response["data"];
      this.wasteCollectionList = response["data"];
      console.log("Data Repport")
      console.log(this.wasteCollectionList)


      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.desiltationActivitySort;
      this.wasteCollectionData.paginator = this.desiltationActivityPaginator;
    })
  }
  onSearch() {


    let serBranchId = this.form.controls.serBranchId.value;
    let txtCategory = this.form.controls.txtCategory.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtActivity = this.form.controls.txtActivity.value;
    let txtDateServicePerformed = '';
    if (this.form.controls.txtDateServicePerformed.value != "") {
      txtDateServicePerformed = this.dateFormat.transformWithTimeZero(this.form.controls.txtDateServicePerformed.value);
    }
    let txtDateServicePerformedEnd = '';
    if (this.form.controls.txtDateServicePerformedEnd.value != "") {
      txtDateServicePerformedEnd = this.dateFormat.transformWithTimeHour(this.form.controls.txtDateServicePerformedEnd.value);
    }
    this.billingService.getAllRecordForDesiltationByParam(serBranchId, serLocationId, txtDateServicePerformed, txtDateServicePerformedEnd, txtActivity,txtCategory).subscribe(response => {

      this.tripDataList = response["data"];
      this.wasteCollectionList = response["data"];

      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.desiltationActivitySort;
      this.wasteCollectionData.paginator = this.desiltationActivityPaginator;
    })
  }
  wasteCollectionList: any = "";
  // wasteCollectionList: any[] = [];
  wasteCollectionDTO: any[] = [];
  downloadPDF() {

    console.log(this.selection);
    this.wasteCollectionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtDateServicePerformed") {
        tableHeader.push({ text: 'Date Time of service performed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCategory") {
        tableHeader.push({ text: 'Category', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtStreetName") {
        tableHeader.push({ text: 'Street Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numStreetId") {
        tableHeader.push({ text: 'Street Id / Container Id', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtModality") {
        tableHeader.push({ text: 'Modality', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtStartPointCoord") {
        tableHeader.push({ text: 'Start Point Coord', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtFinishPointCoord") {
        tableHeader.push({ text: 'Finish Point Coord', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numHouses") {
        tableHeader.push({ text: 'No of Houses', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtModeOfCollection") {
        tableHeader.push({ text: 'Mode Of Collection', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtFrequencyWasteCollection") {
        tableHeader.push({ text: 'Frequency of Waste Collection', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numTotalWasteCollected") {
        tableHeader.push({ text: 'Total Waste Collected', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.wasteCollectionDTO.push(tableHeader);
    console.log(this.wasteCollectionDTO);
    this.wasteCollectionList.forEach((wasteCollectionList, i) => {

      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: wasteCollectionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: wasteCollectionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtDateServicePerformed") {
          tableRows.push({ text: this.dateFormat.transformDateTimeFormat(wasteCollectionList.txtDateServicePerformed.toString()), alignment: 'left' });
        } else if (element === "txtCategory") {
          tableRows.push({ text: wasteCollectionList.txtCategory, alignment: 'left' });
        } else if (element === "txtStreetName") {
          tableRows.push({ text: wasteCollectionList.txtStreetName, alignment: 'left' });
        } else if (element === "numStreetId") {
          tableRows.push({ text: wasteCollectionList.numStreetId, alignment: 'left' });
        }
        else if (element === "txtModality") {
          tableRows.push({ text: wasteCollectionList.txtModality, alignment: 'left' });
        } 
        else if (element === "txtStartPointCoord") {
          tableRows.push({ text: wasteCollectionList.txtStartPointCoord, alignment: 'left' });
        }
        else if (element === "txtFinishPointCoord") {
          tableRows.push({ text: wasteCollectionList.txtFinishPointCoord, alignment: 'left' });
        } else if (element === "numHouses") {
          tableRows.push({ text: wasteCollectionList.numHouses, alignment: 'left' });
        } else if (element === "txtModeOfCollection") {
          tableRows.push({ text: wasteCollectionList.txtModeOfCollection, alignment: 'left' });
        } else if (element === "txtFrequencyWasteCollection") {
          tableRows.push({ text: wasteCollectionList.txtFrequencyWasteCollection, alignment: 'left' });
        } else if (element === "numTotalWasteCollected") {
          tableRows.push({ text: wasteCollectionList.numTotalWasteCollected, alignment: 'left' });
        }
      });
      this.wasteCollectionDTO.push(tableRows)
    });


    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Desiltation Activities', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.wasteCollectionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Desiltation Activities List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDateServicePerformed",
    "txtCategory",
    "txtStreetName",
    "numStreetId",
    "txtModality",
    "txtStartPointCoord",
    "numHouses",
    "txtFinishPointCoord",
    "txtModeOfCollection",
    "txtFrequencyWasteCollection",
    "numTotalWasteCollected"
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDateServicePerformed",
    "txtCategory",
    "txtStreetName",
    "numStreetId",
    "txtModality",
    "txtStartPointCoord",
    "numHouses",
    "txtFinishPointCoord",
    "txtModeOfCollection",
    "txtFrequencyWasteCollection",
    "numTotalWasteCollected"
  ]);
  selectingColumns() {

    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}


