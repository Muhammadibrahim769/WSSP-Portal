// gasCollectionSheet
import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './gasCollectionSheet.component.html'
})
export class GasCollectionSheetComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  gasCollectionColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName', 'txtDate', 'txtVentNo', 'txtCellNo', 'txtGasCollection', 'numGasConsumed','numBalance', 'numRate', 'numTotal', 'Actions'];
  gasCollectionData = new MatTableDataSource();
  @ViewChild("gasCollectionSort", { static: true }) gasCollectionSort: MatSort;
  @ViewChild("gasCollectionPaginator", { static: true }) gasCollectionPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe,) {
  }
  clearTxtDate() {
    this.form.controls.txtDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  clearDateServicePerformed() {
    this.form.controls.txtDate.reset('');
  }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtDate: ['']
    })
    this.getBranchList();
    this.getGasCollectionFindAll();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {

      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }

      })
    })
  }
  getBranchList() {
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {

      this.branchList = response
    })
  }
  getGasCollectionFindAll() {

    this.billingService.getGasCollectionFindAll().subscribe(response => {
      this.isLoading = false;
      this.dataSource = response["data"];
      this.gasCollectionList = response["data"];
      this.gasCollectionData = new MatTableDataSource(response.data);
      this.gasCollectionData.sort = this.gasCollectionSort;
      this.gasCollectionData.paginator = this.gasCollectionPaginator;
    })

  }
  onSearch() {
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtDate = '';
    if (this.form.controls.txtDate.value != "") {
      txtDate = this.dateFormat.transformOperation(this.form.controls.txtDate.value);
    }
    this.billingService.getGasCollectionFindByParams(serBranchId, serLocationId, txtDate).subscribe(response => {

      this.gasCollectionList = response["data"];
      this.gasCollectionData = new MatTableDataSource(response.data);
      this.gasCollectionData.sort = this.gasCollectionSort;
      this.gasCollectionData.paginator = this.gasCollectionPaginator;
    })
  }
  gasCollectionList: any = "";
  // gasCollectionList: any[] = [];
  gasCollectionDTO: any[] = [];
  downloadPDF() {


    this.gasCollectionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtDate") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtVentNo") {
        tableHeader.push({ text: 'Vent No', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCellNo") {
        tableHeader.push({ text: 'Cell No', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtGasCollection") {
        tableHeader.push({ text: 'Gas Collection', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numGasConsumed") {
        tableHeader.push({ text: 'Gas Consumed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "numBalance") {
        tableHeader.push({ text: 'Balance', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numRate") {
        tableHeader.push({ text: 'Rate', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numTotal") {
        tableHeader.push({ text: 'Total', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.gasCollectionDTO.push(tableHeader);

    this.gasCollectionList.forEach((gasCollectionList, i) => {

      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: gasCollectionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: gasCollectionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtDate") {
          tableRows.push({ text: this.dateFormat.transformOperation(gasCollectionList.txtDate.toString()), alignment: 'left' });
        } else if (element === "txtVentNo") {
          tableRows.push({ text: gasCollectionList.txtVentNo, alignment: 'left' });
        } else if (element === "txtCellNo") {
          tableRows.push({ text: gasCollectionList.txtCellNo, alignment: 'left' });
        } else if (element === "txtGasCollection") {
          tableRows.push({ text: gasCollectionList.txtGasCollection, alignment: 'left' });
        } else if (element === "numGasConsumed") {
          tableRows.push({ text: gasCollectionList.numGasConsumed, alignment: 'left' });
        }else if (element === "numBalance") {
          tableRows.push({ text: gasCollectionList.numBalance, alignment: 'left' });
        }  else if (element === "numRate") {
          tableRows.push({ text: gasCollectionList.numRate, alignment: 'left' });
        } else if (element === "numTotal") {
          tableRows.push({ text: gasCollectionList.numTotal, alignment: 'left' });
        }
      });
      this.gasCollectionDTO.push(tableRows)
    });


    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Gas Collection', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.gasCollectionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Gas collection List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDate",
    "txtVentNo",
    "txtCellNo",
    "txtGasCollection",
    "numGasConsumed",
    "numBalance",
    "numRate",
    "numTotal"
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDate",
    "txtVentNo",
    "txtCellNo",
    "txtGasCollection",
    "numGasConsumed",
    "numBalance",
    "numRate",
    "numTotal"
  ]);
  selectingColumns() {

    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
