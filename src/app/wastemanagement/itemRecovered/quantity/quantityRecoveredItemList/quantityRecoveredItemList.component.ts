import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './quantityRecoveredItemList.component.html'
})
export class QuantityRecoveredItemListComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  gasCollectionColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName','txtRouteType','txtItemRecovered', 'txtRecoveredDate', 'numTotalWasteRecieved', 'numTotalRecovered','Actions'];
    gasCollectionData = new MatTableDataSource();
  @ViewChild("itemRecoveredSort", { static: true }) itemRecoveredSort: MatSort;
  @ViewChild("itemRecoveredPaginator", { static: true }) itemRecoveredPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe, ) {
  }
  clearTxtDate() {
    this.form.controls.txtRecoveredDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  clearDateServicePerformed() {
    this.form.controls.txtRecoveredDate.reset('');
  }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtRecoveredDate: [''],
      txtRouteType:['']
    })
    this.getBranchList();
    this.getGasCollectionFindAll();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.user = JSON.parse(sessionStorage.getItem('user'));
    debugger;
    console.log(this.user);
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);    
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {  
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  getGasCollectionFindAll() {
    debugger;
    this.billingService.getRecoveredItemFindAll().subscribe(response => {
      debugger;
      console.log(response["data"])
      this.gasCollectionList = response["data"];
      this.gasCollectionData = new MatTableDataSource(response.data);
      this.gasCollectionData.sort = this.itemRecoveredSort;
      this.gasCollectionData.paginator = this.itemRecoveredPaginator;
    })

  }
  onSearch() {
    debugger;
    // let txtRecoveredDate = this.dateFormat.transformOperation(this.form.controls.txtRecoveredDate.value);
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtRouteType = this.form.controls.txtRouteType.value;
    let txtRecoveredDate = '';
    if (this.form.controls.txtRecoveredDate.value != "") {
      txtRecoveredDate = this.dateFormat.transformOperation(this.form.controls.txtRecoveredDate.value);
    }
    this.billingService.getRecoveredItemByParams(serBranchId, serLocationId, txtRecoveredDate,txtRouteType).subscribe(response => {
      debugger;
      this.gasCollectionList = response["data"];
      this.gasCollectionData = new MatTableDataSource(response.data);
      this.gasCollectionData.sort = this.itemRecoveredSort;
      this.gasCollectionData.paginator = this.itemRecoveredPaginator;
    })
  }
  gasCollectionList: any = "";
  // gasCollectionList: any[] = [];
  gasCollectionDTO: any[] = [];
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.gasCollectionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRouteType") {
        tableHeader.push({ text: 'Route Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRecoveredDate") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numTotalWasteRecieved") {
        tableHeader.push({ text: 'Total Amount Recovered', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numTotalRecovered") {
        tableHeader.push({ text: 'Amount of Waste Recieved (ton)', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtItemRecovered") {
        tableHeader.push({ text: 'Item Recovered', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }     
    });
    this.gasCollectionDTO.push(tableHeader);
    console.log(this.gasCollectionDTO);
    this.gasCollectionList.forEach((gasCollectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: gasCollectionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: gasCollectionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtRouteType") {
          tableRows.push({ text: gasCollectionList.txtRouteType, alignment: 'left' });
        } else if (element === "txtRecoveredDate") {
          tableRows.push({ text: this.dateFormat.transformOperation(gasCollectionList.txtRecoveredDate.toString()), alignment: 'left' });
        } else if (element === "numTotalWasteRecieved") {
          tableRows.push({ text: gasCollectionList.numTotalWasteRecieved, alignment: 'left' });
        } else if (element === "numTotalRecovered") {
          tableRows.push({ text: gasCollectionList.numTotalRecovered, alignment: 'left' });
        } else if (element === "txtItemRecovered") {
          tableRows.push({ text: gasCollectionList.txtItemRecovered, alignment: 'left' });
        }
      });
      this.gasCollectionDTO.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Recovered Item', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.gasCollectionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Recovered Item List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtRouteType",
    "txtRecoveredDate",
    "numTotalWasteRecieved",
    "numTotalRecovered",
    "txtItemRecovered"   
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtRouteType",
    "txtRecoveredDate",
    "numTotalWasteRecieved",
    "numTotalRecovered",
    "txtItemRecovered"
  
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
