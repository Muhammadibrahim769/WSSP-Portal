import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './saleRecoveredItemList.component.html'
})
export class SaleRecoveredItemListComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  SaleRecoveredItemColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName','txtSaleItem','txtSoldDate','txtVendor',  'numRate', 'numAmount','Actions'];
    SaleRecoveredItemData = new MatTableDataSource();
  @ViewChild("SaleRecoveredItemSort", { static: true }) SaleRecoveredItemSort: MatSort;
  @ViewChild("SaleRecoveredItemPaginator", { static: true }) SaleRecoveredItemPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe, ) {
  }
  clearTxtDate() {
    this.form.controls.txtSoldDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtSoldDate: ['']
    })
    this.getBranchList();
    this.getSaleRecoveredItemFindAll();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.user = JSON.parse(sessionStorage.getItem('user'));
    debugger;
    console.log(this.user);
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);    
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {  
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  getSaleRecoveredItemFindAll() {
    debugger;
    this.billingService.getSaleRecoveredItemFindAll().subscribe(response => {
      debugger;
      console.log(response["data"])
      this.SaleRecoveredItemList = response["data"];
      this.SaleRecoveredItemData = new MatTableDataSource(response.data);
      this.SaleRecoveredItemData.sort = this.SaleRecoveredItemSort;
      this.SaleRecoveredItemData.paginator = this.SaleRecoveredItemPaginator;
    })

  }
  onSearch() {
    debugger;
   
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtSoldDate = '';
    if (this.form.controls.txtSoldDate.value != "") {
      txtSoldDate = this.dateFormat.transformOperation(this.form.controls.txtSoldDate.value);
    }
    // let txtSoldDate = this.dateFormat.transformOperation(this.form.controls.txtSoldDate.value);
    this.billingService.getSaleRecoveredItemByParams(serBranchId, serLocationId, txtSoldDate).subscribe(response => {
      debugger;
      this.SaleRecoveredItemList = response["data"];
      this.SaleRecoveredItemData = new MatTableDataSource(response.data);
      this.SaleRecoveredItemData.sort = this.SaleRecoveredItemSort;
      this.SaleRecoveredItemData.paginator = this.SaleRecoveredItemPaginator;
    })
  }
  SaleRecoveredItemList: any = "";
  // SaleRecoveredItemList: any[] = [];
  SaleRecoveredItemDTO: any[] = [];
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.SaleRecoveredItemDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtSaleItem") {
        tableHeader.push({ text: 'Sale Item', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtSoldDate") {
        tableHeader.push({ text: 'Sold Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numRate") {
        tableHeader.push({ text: 'Rate', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numAmount") {
        tableHeader.push({ text: 'Amount', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtVendor") {
        tableHeader.push({ text: 'Vender', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }    
       
    });
    this.SaleRecoveredItemDTO.push(tableHeader);
    console.log(this.SaleRecoveredItemDTO);
    this.SaleRecoveredItemList.forEach((SaleRecoveredItemList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: SaleRecoveredItemList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: SaleRecoveredItemList.txtLocationName, alignment: 'left' });
        } else if (element === "txtSaleItem") {
          tableRows.push({ text: SaleRecoveredItemList.txtSaleItem, alignment: 'left' });
        }else if (element === "txtSoldDate") {
          tableRows.push({ text: this.dateFormat.transformOperation(SaleRecoveredItemList.txtSoldDate.toString()), alignment: 'left' });
        } else if (element === "numRate") {
          tableRows.push({ text: SaleRecoveredItemList.numRate, alignment: 'left' });
        } else if (element === "numAmount") {
          tableRows.push({ text: SaleRecoveredItemList.numAmount, alignment: 'left' });
        } else if (element === "txtVendor") {
          tableRows.push({ text: SaleRecoveredItemList.txtVendor, alignment: 'left' });
        }
        
      });
      this.SaleRecoveredItemDTO.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Sale Item', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.SaleRecoveredItemDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Sale Item List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtSoldDate",
    "numRate",
    "numAmount",
    "txtVendor",
    "txtSaleItem"
   
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtSoldDate",
    "numRate",
    "numAmount",
    "txtVendor",
    "txtSaleItem"
  
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
