...
export class TripSheetComponent implements OnInit {
  ...
  getBatchNo: any;
  pageSize = 5;
  tripSheetColumn: string[] = ['sNo', 'txtBranchName', 'txtLocationName', 'date', 'asset_no', 'txtDriverName',
    'txtRouteNo', 'txtRouteName', 'route_distance', 'txtRouteType', 'distance_covered', 'trip_no', 'time_out', 'odometer_out', 'time_in',
    'odometer_in', 'total_distance', 'total_load', 'Actions'];
  tripSheetData = new MatTableDataSource();
  @ViewChild("tripSheetDataSort", { static: true }) tripSheetDataSort: MatSort;
  @ViewChild("tripSheetDataPaginator", { static: true }) tripSheetDataPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe, ) {
  }
  clearWEFD() {
    this.form.controls.dteDate.reset('');
  }
  open(content) {
  ...
  onSearch() {
    ...
    let dteDateStart = this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateStart.value);
    let dteDateEnd = this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateEnd.value);
    console.log(dteDateStart);
    // this.form.controls.dteDate.patchValue(dteDateStart)
    let dteDateStartTrip = dteDateStart;
    ...
    let txtRouteNo = this.form.controls.txtRouteNo.value;
    let txtRouteName = this.form.controls.txtRouteName.value;
    let txtRouteType = this.form.controls.txtRouteType.value;
    let dteDateEndTrip = dteDateEnd
    this.billingService.getAllRecordForTripSheet(serBranchId, serLocationId, txtRouteNo, txtBatchNo, dteDateStartTrip, txtRouteName,txtRouteType, dteDateEndTrip).subscribe(response => {
      this.tripDataList = response["data"];
      this.selectStatusList = [];
      debugger;
      this.tripSheetData = new MatTableDataSource(response.data);
      this.tripSheetData.sort = this.tripSheetDataSort;
      this.tripSheetData.paginator = this.tripSheetDataPaginator;
  ...
  tripSheetListServiceData() {
    this.billingService.getAllRecordForTripSheetFindAll().subscribe(Response => {
      debugger;
  ...
  tripData: any;
  onEditTrip(element) {
    this.tripData = element;
    this.shared.changeMessage(this.tripData);
    this.router.navigate(['/fleet/tripSheet/viewTripSheet']);
  }
  // ---------------------------------------------Download PDF -----------------------------------------------
  ...
  pdfImplementation() {
    ...
    debugger;
    this.reportDtoList.push([
      { text: 'Sr#', bold: true },
      { text: 'Branch Name', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Date', bold: true },
      { text: 'Asset/Vehicle No', bold: true },
      { text: 'Asset/Vehicle Name', bold: true },
      { text: 'Driver Name', bold: true },
      { text: 'Route No', bold: true },
      { text: 'Route Name', bold: true },
      { text: 'Route Distance', bold: true },
      { text: 'Route Type', bold: true },
      { text: 'Distance Covered', bold: true },
      { text: 'Trip No', bold: true },
      { text: 'Time Out', bold: true },
      { text: 'Odo Meter Out', bold: true },
      { text: 'Time In', bold: true },
      { text: 'Odo Meter In', bold: true },
      { text: 'Total Distance', bold: true },
      { text: 'Total Garbage Disposed', bold: true },
    ]);
    this.tripDataList.forEach((tripDataList, i) => {
      debugger;
      this.reportDtoList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tripDataList.txt_branch_name, alignment: 'left' },
        { text: tripDataList.txt_location_name, alignment: 'left' },
        { text: this.dateFormat.transform(tripDataList.dteDate.toString()), alignment: 'left' },
        { text: tripDataList.serBatchNo, alignment: 'left' },
        { text: tripDataList.txt_batch_no, alignment: 'left' },
        { text: tripDataList.txtDriverName, alignment: 'left' },
        { text: tripDataList.txtRouteNo, alignment: 'left' },
        { text: tripDataList.txtRouteName, alignment: 'left' },
        { text: tripDataList.numRouteDistance, alignment: 'left' },
        { text: tripDataList.txtRouteType, alignment: 'left' },
        { text: tripDataList.distance_covered, alignment: 'left' },
        { text: tripDataList.trip_no, alignment: 'left' },
        { text: tripDataList.dteTimeout, alignment: 'left' },
        { text: tripDataList.numOdometerOut, alignment: 'left' },
        { text: tripDataList.dteTimein, alignment: 'left' },
        { text: tripDataList.numOdometerIn, alignment: 'left' },
        { text: tripDataList.numGarbageDisposed, alignment: 'left' }
      ]);
    });
  }
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      if (element === "sr") {
        ...
        tableHeader.push({ text: 'Branch Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "date") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "asset_no") {
        tableHeader.push({ text: 'Asset/Vehicle No', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "vehicle_name") {
        tableHeader.push({ text: 'Asset/Vehicle Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtDriverName") {
        tableHeader.push({ text: 'Driver Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRouteNo") {
        tableHeader.push({ text: 'Route No', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRouteName") {
        tableHeader.push({ text: 'Route Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "route_distance") {
        tableHeader.push({ text: 'Distance', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRouteType") {
        tableHeader.push({ text: 'Route Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "distance_covered") {
        tableHeader.push({ text: 'Covered Distance:', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "trip_no") {
        tableHeader.push({ text: 'Trip No:', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "time_out") {
        tableHeader.push({ text: 'Time Out', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "odometer_out") {
        tableHeader.push({
          text: 'Odo Meter Out', bold: true, fillColor: '#dddddd',
          alignment: 'center'
        });
      } else if (element === "time_in") {
        tableHeader.push({ text: 'Time In', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "odometer_in") {
        tableHeader.push({ text: 'Odo Meter In', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "total_distance") {
        tableHeader.push({ text: 'Distance', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "total_load") {
        tableHeader.push({ text: 'Total Garbage Disposed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.reportDtoList.push(tableHeader);
    console.log(this.reportDtoList);
    this.tripDataList.forEach((tripDataList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: tripDataList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: tripDataList.txtLocationName, alignment: 'left' });
        } else if (element === "date") {
          tableRows.push({ text: this.dateFormat.transform(tripDataList.dteDate.toString()), alignment: 'left' });
        } else if (element === "asset_no") {
          tableRows.push({ text: tripDataList.asset_no, alignment: 'left' });
        } else if (element === "vehicle_name") {
          tableRows.push({ text: tripDataList.vehicle_name, alignment: 'right' });
        } else if (element === "txtDriverName") {
          tableRows.push({ text: tripDataList.txtDriverName, alignment: 'right' });
        } else if (element === "txtRouteNo") {
          tableRows.push({ text: tripDataList.txtRouteNo, alignment: 'right' });
        } else if (element === "txtRouteName") {
          tableRows.push({ text: tripDataList.txtRouteName, alignment: 'left' });
        } else if (element === "route_distance") {
          tableRows.push({ text: tripDataList.numRouteDistance, alignment: 'right' });
        }
        else if (element === "txtRouteType") {
          tableRows.push({ text: tripDataList.txtRouteType, alignment: 'left' });
        } else if (element === "distance_covered") {
          tableRows.push({ text: tripDataList.distance_covered, alignment: 'left' });
        } else if (element === "trip_no") {
          tableRows.push({ text: tripDataList.trip_no, alignment: 'left' });
        } else if (element === "time_out") {
          tableRows.push({ text: tripDataList.time_out, alignment: 'left' });
        } else if (element === "odometer_out") {
          tableRows.push({ text: tripDataList.numOdometerOut, alignment: 'right' });
        } else if (element === "odometer_in") {
          tableRows.push({ text: tripDataList.numOdometerIn, alignment: 'right' });
        } else if (element === "time_in") {
          tableRows.push({ text: tripDataList.dteTimein, alignment: 'left' });
        } else if (element === "total_distance") {
          tableRows.push({ text: tripDataList.numRouteDistance, alignment: 'right' });
        } else if (element === "total_load") {
          tableRows.push({ text: tripDataList.numGarbageDisposed, alignment: 'left' });
        }
      });
      this.reportDtoList.push(tableRows)
    });
    debugger;
      ...
      content: [
        {
          ...
          table: {
            ...
            body: [
              ...
              }, [{ text: 'Trip Sheet Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
    ...
    pdfMake.createPdf(documentDefinition).download('TripSheet List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "txtBranchName",
