
import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './wasteDiversionSheet.component.html'
})
export class WasteDiversionSheetComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  wasteDiversionColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName', 'txtDate', 'numTotalAmountRecieved', 'numWasteCompostPlant', 'numWasteSentToRdf', 'numWasteSentSegregation', 'numWasteSentToCell', 'numTotalBalanace',
    'Actions'];
  wasteDiversionData = new MatTableDataSource();
  @ViewChild("wasteDiversionSort", { static: true }) wasteDiversionSort: MatSort;
  @ViewChild("wasteDiversionPaginator", { static: true }) wasteDiversionPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe, ) {
  }
  clearTxtDate() {
    this.form.controls.txtDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  clearDateServicePerformed() {
    this.form.controls.txtDate.reset('');
  }

  /*
     
      numTotalAmountRecieved:[''],
      numWasteCompostPlant:[''],
      numWasteSentToRdf:[''],
      numWasteSentSegregation:[''],
      numWasteSentToCell:[''],
      numTotalBalanace:['']
  */
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtDate: ['']
    })
    this.getBranchList();
    this.getwasteDiversionFindAll();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.user = JSON.parse(sessionStorage.getItem('user'));
    debugger;
    console.log(this.user);
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  getwasteDiversionFindAll() {
    debugger;
    this.billingService.getwasteDiversionFindAll().subscribe(response => {
      this.isLoading = false;
      this.dataSource = response["data"];
      this.wasteDiversionList = response["data"];
      this.wasteDiversionData = new MatTableDataSource(response.data);
      this.wasteDiversionData.sort = this.wasteDiversionSort;
      this.wasteDiversionData.paginator = this.wasteDiversionPaginator;
    })

  }
  onSearch() {
    debugger;
    // let txtDate = this.dateFormat.transformOperation(this.form.controls.txtDate.value);
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtDate = '';
    if (this.form.controls.txtDate.value != "") {
      txtDate = this.dateFormat.transformOperation(this.form.controls.txtDate.value);
    }
    this.billingService.getwasteDiversionFindByParams(serBranchId, serLocationId, txtDate).subscribe(response => {
      debugger;
      this.wasteDiversionList = response["data"];
      this.wasteDiversionData = new MatTableDataSource(response.data);
      this.wasteDiversionData.sort = this.wasteDiversionSort;
      this.wasteDiversionData.paginator = this.wasteDiversionPaginator;
    })
  }
  wasteDiversionList: any = "";
  // wasteDiversionList: any[] = [];
  wasteDiversionDTO: any[] = [];
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.wasteDiversionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtDate") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numTotalAmountRecieved") {
        tableHeader.push({ text: 'Total Amount Recieved ', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numWasteCompostPlant") {
        tableHeader.push({ text: 'Waste Sent to Compost Plant', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numWasteSentToRdf") {
        tableHeader.push({ text: 'Waste Sent To RDF', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numWasteSentSegregation") {
        tableHeader.push({ text: 'Waste Sent to Segregation', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numWasteSentToCell") {
        tableHeader.push({ text: 'Waste Sent To Cell', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numTotalBalanace") {
        tableHeader.push({ text: 'Total Balanace', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.wasteDiversionDTO.push(tableHeader);
    console.log(this.wasteDiversionDTO);
    this.wasteDiversionList.forEach((wasteDiversionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: wasteDiversionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: wasteDiversionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtDate") {
          tableRows.push({ text: this.dateFormat.transformOperation(wasteDiversionList.txtDate.toString()), alignment: 'left' });
        } else if (element === "numTotalAmountRecieved") {
          tableRows.push({ text: wasteDiversionList.numTotalAmountRecieved, alignment: 'right' });
        } else if (element === "numWasteCompostPlant") {
          tableRows.push({ text: wasteDiversionList.numWasteCompostPlant, alignment: 'right' });
        } else if (element === "numWasteSentToRdf") {
          tableRows.push({ text: wasteDiversionList.numWasteSentToRdf, alignment: 'right' });
        } else if (element === "numWasteSentSegregation") {
          tableRows.push({ text: wasteDiversionList.numWasteSentSegregation, alignment: 'right' });
        } else if (element === "numWasteSentToCell") {
          tableRows.push({ text: wasteDiversionList.numWasteSentToCell, alignment: 'right' });
        } else if (element === "numTotalBalanace") {
          tableRows.push({ text: wasteDiversionList.numTotalBalanace, alignment: 'right' });
        }
      });
      this.wasteDiversionDTO.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Waste Diversion', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.wasteDiversionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Waste Diversion List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDate",
    "numTotalAmountRecieved",
    "numWasteCompostPlant",
    "numWasteSentToRdf",
    "numWasteSentSegregation",
    "numWasteSentToCell",
    "numTotalBalanace"
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtDate",
    "numTotalAmountRecieved",
    "numWasteCompostPlant",
    "numWasteSentToRdf",
    "numWasteSentSegregation",
    "numWasteSentToCell",
    "numTotalBalanace"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
