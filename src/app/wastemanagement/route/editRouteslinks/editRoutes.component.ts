...
export class EditRoutesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.routesform.controls['routeDetails'].valueChanges.subscribe((value) => {
      ...
      value.forEach((currentValue) => {
        this.total = this.total + currentValue.num_distance;
        if (this.total > 0) {
          debugger;
          this.routesform.patchValue({ distance: this.total.toString() });
        }
        console.log(this.findInvalidControls());
        console.log(this.routesform.controls.distance.errors);
      });
    });
  ...
  this.billingService.getrouteServiceById(this.Id).subscribe(response =>{
      ...
      this.editRoutes.routeDetails.forEach(element => {
        ...
        this.routeDetails.push(
          this.formBuilder.group({
            ...
            num_distance: element.num_distance,
      ...
      this.routesform.controls.serBranchId.patchValue(this.editRoutes.txtBranchName);
      this.routesform.controls.name.patchValue(this.editRoutes.name);
      this.routesform.controls.distance.patchValue(this.editRoutes.distance);     
  })
  }
  getRouteList() {
    debugger;
    // let txt_name = this.form.controls.txt_name;
    // let txt_code = this.form.controls.txt_code;
