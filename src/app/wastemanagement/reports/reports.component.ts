import { AfterViewInit, Component, OnInit, ViewChild } from '@angular/core';
import { NgbDateStruct } from '@ng-bootstrap/ng-bootstrap';
import { FormBuilder, FormGroup, Validators, FormControl } from '@angular/forms';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { MatPaginator } from '@angular/material/paginator';
import { BillingService } from '@app/services/billing.service';

import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
import * as _moment from 'moment';
import { Moment } from 'moment';
import { MAT_DATE_FORMATS, MAT_DATE_LOCALE, DateAdapter } from '@angular/material/core';
import { MAT_MOMENT_DATE_ADAPTER_OPTIONS, MomentDateAdapter } from '@angular/material-moment-adapter';
import { MatDatepicker } from '@angular/material/datepicker';
import { element } from 'protractor';
const moment = _moment;
export const MY_FORMATS = {
  parse: {
    dateInput: 'MMM-y',
  },
  display: {
    dateInput: 'MMM-y',
    monthYearLabel: 'MMM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MMMM YYYY',
  },
};
@Component({
  selector: 'app-dashboard',
  templateUrl: './reports.component.html',
  providers: [
    {
      provide: DateAdapter,
      useClass: MomentDateAdapter,
      deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS]
    },

    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class ReportsComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  selectStatusList: any[] = [];
  responseList: any[] = [];
  branchList: any;
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  panelOpenState = true;
  getBatchNolist = [];

  page = 1;
  pageSize = 5;
  tripSheetColumn: string[] = ['sNo', 'txtBranchName', 'txtLocationName', 'tripDate', 'txtBatchNo', 'txtRouteType', 'txtDriverName', 'txtRouteType', 'txtRouteName', 'total_garbage_disposed'];
  tripSheetData = new MatTableDataSource();
  @ViewChild("tripSheetDataSort", { static: true }) tripSheetDataSort: MatSort;
  @ViewChild("tripSheetDataPaginator", { static: true }) tripSheetDataPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private dateFormat: DateFormatPipe, private billingService: BillingService) {

  }
  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue = this.form.controls.tripDate.value;
    ctrlValue.year(normalizedYear.year());
    this.form.controls.tripDate.setValue(ctrlValue);
  }

  chosenMonthHandler(normalizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
    const ctrlValue = this.form.controls.tripDate.value;
    ctrlValue.month(normalizedMonth.month());
    this.form.controls.tripDate.setValue(ctrlValue);
    console.log("ghjsdkasd");
    console.log(this.form.controls.tripDate);
    datepicker.close();
  }
  routesLIst: any[] = [];
  getBranchId = [];
  ucList: any;

  isLoading = true;
  dataSource = null;
  totalGarbage = false;
  clearWEFD() {
    this.form.controls.txtDate.reset('');
  }
  open(content) {
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title', size: 'xl' })
  }
  step = 0;

  setStep(index: number) {
    this.step = index;
  }
  OnlyNumbersAllowed(event): boolean {

    const charcode = (event.which) ? event.which : event.keycode;

    if (charcode > 31 && (charcode < 48 || charcode > 57)) {
      return false;

    }
    return true;
  }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [''],
      serBranchId: [''],
      serLocationId: [''],
      dteDateStart: [""],
      dteDateEnd: [''],
      txtVehicleId: [''],
      txtDriverName: [''],
      txtVehicleName: [''],
      txtBatchNo: [''],
      txtRouteType: [''],
      txtRouteName: [''],
      numGarbageDisposed: [null],
    })
    this.getBranchList();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.onGetReport();
    this.pdfImplementation();
    this.getBatchList();
    this.getRouteList();

  }
  onGetReport() {
    this.billingService.getAllRecordReport().subscribe(response => {
      this.tripSheetData = new MatTableDataSource(response.data);
      this.tripSheetData.sort = this.tripSheetDataSort;
      this.tripSheetData.paginator = this.tripSheetDataPaginator;
    })
  }


  searchTripData: any;
  tripDataList: any[] = [];
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {
      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  total: any = 0;
  onSearch() {
    if ((this.total && this.form.controls.numGarbageDisposed.value) > 0) {
      this.total = 0;
      this.form.controls.numGarbageDisposed.patchValue('');
    }
    this.totalGarbage = true;
    // this.total=0;
    console.log(this.form.value)
    let dteDateStart = this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateStart.value);
    let dteDateEnd = this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateEnd.value);
    console.log(dteDateStart);
    let dteDateStartTrip = dteDateStart;
    let dteDateEndTrip = dteDateEnd;
    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtBatchNo = this.form.controls.txtBatchNo.value
    let txtRouteType = this.form.controls.txtRouteType.value;
    let txtRouteName = this.form.controls.txtRouteName.value;
    this.billingService.getAllRecordForReportByParam(serBranchId, serLocationId, txtBatchNo, dteDateStartTrip, dteDateEndTrip, txtRouteType, txtRouteName).subscribe(response => {
      this.tripDataList = response["data"];
      this.selectStatusList = [];
      debugger;
      this.tripSheetData = new MatTableDataSource(response.data);
      this.tripSheetData.sort = this.tripSheetDataSort;
      this.tripSheetData.paginator = this.tripSheetDataPaginator;
      console.log("response")
      console.log(response)

      this.tripDataList.forEach(element => {
        debugger;
        // this.form.controls.numGarbageDisposed.value = +  this.form.controls.numGarbageDisposed.value
        this.total = this.total + element.numGarbageDisposed
      })
      this.form.controls.numGarbageDisposed.patchValue(this.total);

    })
    this.pdfImplementation();
  }
  onSave(addForm: any) {
    console.log(this.addForm.value);
  }
  getBatchNo: any;

  getBatchList() {
    this.billingService.getFleetService("", "").subscribe(response => {
      console.log("getFleetService");
      console.log(response);
      this.getBatchNo = response
      this.getBatchNo.forEach(element => {
        this.getBatchNolist.push(
          [element["txtBatchNo"]],
        )
      })
    })
  }
  getRouteList() {
    this.billingService.getAllRouteslistOfCodeAndName().subscribe(response => {
      console.log(response["routesLIst"]);
      this.routesLIst = response["data"];
      for (let getTxtNameCode of this.routesLIst) {
        this.getAllNameCode.push(getTxtNameCode);
      }
    })
  }
  reportDtoList: any = [];
  pdfImplementation() {
    this.reportDtoList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Month', bold: true },
      { text: 'Asset/Vehicle No', bold: true },
      { text: 'Driver Name', bold: true },
      { text: 'Route Type', bold: true },
      { text: 'Route Name', bold: true },
      { text: 'Total Garbage Disposed', bold: true },
    ]);
    this.tripDataList.forEach((tripDataList, i) => {
      this.reportDtoList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tripDataList.txtBranchName, alignment: 'left' },
        { text: tripDataList.txtLocationName, alignment: 'left' },
        { text: this.dateFormat.transform(tripDataList.dteDate.toString()), alignment: 'left' },
        { text: tripDataList.txtBatchNo, alignment: 'left' },
        { text: tripDataList.txtDriverName, alignment: 'left' },
        { text: tripDataList.txtRouteType, alignment: 'left' },
        { text: tripDataList.txtRouteName, alignment: 'left' },
        { text: tripDataList.numGarbageDisposed, alignment: 'left' }
      ]);
    });
  }
  downloadPDF() {
    console.log(this.selection);
    this.reportDtoList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Branch Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "tripDate") {
        tableHeader.push({ text: 'Trip Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBatchNo") {
        tableHeader.push({ text: 'Asset/Vehicle No', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtDriverName") {
        tableHeader.push({ text: 'DriverName', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRouteType") {
        tableHeader.push({ text: 'Route Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtRouteName") {
        tableHeader.push({ text: 'Route Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "total_garbage_disposed") {
        tableHeader.push({ text: 'Total Garbage Disposed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.reportDtoList.push(tableHeader);
    console.log(this.reportDtoList);
    this.tripDataList.forEach((tripDataList, i) => {
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: tripDataList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: tripDataList.txtLocationName, alignment: 'left' });
        } else if (element === "tripDate") {
          tableRows.push({ text: this.dateFormat.transformToStandardDateTime(tripDataList.dteDate.toString()), alignment: 'left' });
        } else if (element === "txtBatchNo") {
          tableRows.push({ text: tripDataList.txtBatchNo, alignment: 'left' });
        }
        else if (element === "txtDriverName") {
          tableRows.push({ text: tripDataList.txtDriverName, alignment: 'left' });
        } else if (element === "txtRouteType") {
          tableRows.push({ text: tripDataList.txtRouteType, alignment: 'left' });
        } else if (element === "txtRouteName") {
          tableRows.push({ text: tripDataList.txtRouteName, alignment: 'left' });
        } else if (element === "total_garbage_disposed") {
          tableRows.push({ text: tripDataList.numGarbageDisposed, alignment: 'right' });
        }
      });
      this.reportDtoList.push(tableRows)
    });
    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{
                text: 'Trip Garbage Report'
                //  dteDateStart        dteDateEnd
                , style: 'dateHeader', alignment: 'center', margin: [130, 60, 150, 0], bold: true, fontSize: 16
              }],                            
              {
                image: 'data:image/png;base64,', width: 80,
                height: 80,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          
          layout: 'noBorders'
        },  
        {
          columns: [
            {
              text: [{ text: 'Trip Report : ', bold: true }, this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateStart.value),' ','To',' ',
              this.dateFormat.transformToStandardDateTime(this.form.controls.dteDateEnd.value)  ]
            }        
          ]
        },      
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.reportDtoList
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: ''
             
            },
            {
              text:''
            },
            {
              text: [{ text: 'Total Garbage Disposed (Ton): ', bold: true }, this.form.controls.numGarbageDisposed.value]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('TripSheet List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "tripDate",
    "txtBatchNo",
    "txtRouteType",
    "total_garbage_disposed",
    "txtDriverName",
    "txtRouteType",
    "txtRouteName"
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "tripDate",

    "txtBatchNo",
    "txtRouteType",
    "total_garbage_disposed",
    "txtDriverName",
    "txtRouteType",
    "txtRouteName"


  ]);
  selectingColumns() {
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }





}
