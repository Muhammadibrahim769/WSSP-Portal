import { Component, OnInit } from '@angular/core';
import { CrudService } from '@app/_services/crud.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './view-general-operation.component.html'
})
export class ViewGeneralOperationComponent implements OnInit {

  generalOperationDetails: any
  viewData: any[] = [];
  constructor(private shared: CrudService) { }

  ngOnInit(): void {
    this.shared.currentMessage.subscribe(message => {
      this.viewData.push(message);
      console.log('hjghjgk');
      console.log(this.viewData)
    })
    debugger;
  }
  downloadPDF() {
    debugger;
    console.log("this.tripDataList")
    console.log(this.viewData)
    const documentDefinition = {
      pageOrientation: 'A4',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: [
              [{
                image: 'data:image/png;base64,',
                width: 80,
                height: 80,
                alignment: 'left'
              },
              [{
                text: 'GeneralOperationReport', style: 'tableHeaderValue', alignment: 'center', headerRows: 1,
                margin: [100, 40, 150, 0], bold: true, fontSize: 16
              }],
              {
                image: 'data:image/png;base64,', width: 70,
                height: 70,
                alignment: 'right', margin: [5, 5, 55, 0],
              }],


            ]
          },
          layout: 'noBorders'
        },

        { text: 'Sewerage:', fontSize: 14, bold: true, margin: [0, 50, 0, 0] },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [110, 145, 110, 145],
            body: [

              [
                { text: 'Zone/Branch:', style: 'tableHeader' },
                { text: this.viewData[0].txtBranchName, style: 'tableHeaderValue' },
                { text: 'Union Council:', style: 'tableHeader' },
                { text: this.viewData[0].txtLocationName, style: 'tableHeaderValue' },
              ],
              [

                { text: 'Period:', style: 'tableHeader' },
                { text: this.viewData[0].txtMonth, style: 'tableHeaderValue' },
                { text: 'Manholes Cleaned:', style: 'tableHeader' },
                { text: this.viewData[0].numPriMhCleaned, style: 'tableHeaderValue', alignment: 'right' }
              ],
              [
                { text: 'RPC Manholes Repair:', style: 'tableHeader' },
                { text: this.viewData[0].numPriMhRepaired, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'RCC Manholes Repair:', style: 'tableHeader' },
                { text: this.viewData[0].numSecMhRepaired, style: 'tableHeaderValue', alignment: 'right' }

              ],
              // [
              //   { text: 'IronGrid Manholes Repair:', style: 'tableHeader' },
              //   { text: this.viewData[0].numTerMhRepaired, style: 'tableHeaderValue', alignment: 'right' },
              //   { text: 'RPC Manholes Cleaned(km):', style: 'tableHeader' },
              //   { text: this.viewData[0].numPriMhCleaned, style: 'tableHeaderValue', alignment: 'right' }
              // ],
              // [
              //   { text: 'RCC Manholes Cleaned(km):', style: 'tableHeader' },
              //   { text: this.viewData[0].numSecMhCleaned, style: 'tableHeaderValue', alignment: 'right' },

              //   { text: 'IronGrit Manholes Cleaned(km):', style: 'tableHeader' },
              //   { text: this.viewData[0].numTerMhCleaned, style: 'tableHeaderValue', alignment: 'right' },
              // ],
              [
                { text: 'Primary Sewerage Lines Repair:', style: 'tableHeader' },
                { text: this.viewData[0].numPriSewerageLinesRepaired, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Primary Sewerage Lines Cleaned:', style: 'tableHeader' },
                { text: this.viewData[0].numPriSewerageLinesCleaned, style: 'tableHeaderValue', alignment: 'right' }
              ],
              [
                { text: 'Primary Sewerage Lines Desilation(km):', style: 'tableHeader' },
                { text: this.viewData[0].numPriSewerageLineDiscelitation, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Secondary Sewerage Lines Repaired:', style: 'tableHeader' },
                { text: this.viewData[0].numSecSewerageLineRepaired, style: 'tableHeaderValue', alignment: 'right' },
              ],
              // [
              //   { text: 'Primary Drainage Lines Repair(km):', style: 'tableHeader' },
              //   { text: this.viewData[0].numPriDrainageLinesRepaired, style: 'tableHeaderValue', alignment: 'right' },
              //   { text: 'Secondary Drainage Lines Desilation:', style: 'tableHeader' },
              //   { text: this.viewData[0].numSecDrainageLinesDiscelitation, style: 'tableHeaderValue', alignment: 'right' }

              // ],
              [
                { text: 'Secondary Sewerage Lines Cleaned', style: 'tableHeader' },
                { text: this.viewData[0].numSecSewerageLineCleaned, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Secondary Sewerage Lines Desilation(km)', style: 'tableHeader' },
                { text: this.viewData[0].numSecSewerageLineDiscelitation, style: 'tableHeaderValue', alignment: 'right' }                
              ]             
            ],
            layout: 'noBorders'
          }
        },

        { text: 'Drainage:', fontSize: 14, bold: true, margin: [0, 10, 0, 0] },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [110, 145, 110, 145],
            body: [

              // [
              //   { text: 'Distribution Lines Repair:', style: 'tableHeader' },
              //   { text: this.viewData[0].numDistributionLinesRepaired, style: 'tableHeaderValue', alignment: 'right' },
              //   { text: 'Distribution Lines Cleaned:', style: 'tableHeader' },
              //   { text: this.viewData[0].numDistributionLinesCleaned, style: 'tableHeaderValue', alignment: 'right' },

              // ],



              [

                { text: 'Drainage Lines Repair:', style: 'tableHeader' },
                { text: this.viewData[0].numPriDrainageLinesRepaired, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Drainage Lines Cleaned:', style: 'tableHeader' },
                { text: this.viewData[0].numPriDrainageLinesCleaned, style: 'tableHeaderValue', alignment: 'right' }
              ],
              [
                { text: 'Primary Drainage Lines Desilation(km):', style: 'tableHeader' },
                { text: this.viewData[0].numPriDrainageLinesDiscelitation, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Secondary Drainage Lines Repair:', style: 'tableHeader' },
                { text: this.viewData[0].numSecDrainageLinesRepaired, style: 'tableHeaderValue', alignment: 'right' }

              ],
              [
                { text: 'Secondary Drainage Lines Cleaned:', style: 'tableHeader' },
                { text: this.viewData[0].numSecDrainageLinesCleaned, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Secondary Drainage Lines Desilation:', style: 'tableHeader' },
                { text: this.viewData[0].numSecDrainageLinesDiscelitation, style: 'tableHeaderValue', alignment: 'right' }

              ],
              [
                { text: 'Street Drain Repair:', style: 'tableHeader' },
                { text: this.viewData[0].numPriDistributionLinesRepaired, style: 'tableHeaderValue', alignment: 'right' },
                { text: 'Street Drain Cleaned(km)', style: 'tableHeader' },
                { text: this.viewData[0].numPriDistributionLinesCleaned, style: 'tableHeaderValue', alignment: 'right' }
              ],
              [
                { text: 'Street Drain Desilation:', style: 'tableHeader' },
                { text: this.viewData[0].numPriDistributionLinesDiscelitation, style: 'tableHeaderValue', alignment: 'right' },
                { text: '' },
                { text: '' }
              ]         
             
            ]
          },
          // layout: 'noBorders'
        },
      ],
      styles: {
        header: {
          fontSize: 16,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
          padding: [50, 0, 0, 0]
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
          padding: [50, 0, 0, 0]
        }

      },
      defaultStyle: {
      }
    };
    pdfMake.createPdf(documentDefinition).download('General Operation Detail.pdf');
  }
}
