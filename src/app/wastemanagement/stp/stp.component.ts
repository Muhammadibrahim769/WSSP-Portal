import { Component, OnInit, ViewChild } from "@angular/core";
import { FormGroup, FormBuilder, FormControl } from "@angular/forms";
import { MatPaginator } from "@angular/material/paginator";
import { MatSort } from "@angular/material/sort";
import { MatTableDataSource } from "@angular/material/table";
import { BillingService } from "@app/services/billing.service";
import { DateFormatPipe } from "@app/_helpers/date-format.pipe";
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-stp',
  templateUrl: './stp.component.html'
})

export class StpComponent implements OnInit {

  searchform: FormGroup;
  branchList: any;
  unionCouncilList: any;
  ucList: any;
  getUC = [];
  step = 0;
  user: any;
  stpList: any = "";
  stpDTO: any[] = [];
  waterQualityList = ['BoD', 'CoD', 'Phosphorous', 'Nitrogen', 'TDS', 'Ammonia', 'Coliform', 'E-Coli', 'F-Coli']

  stpDataColumn: string[] = ['txtBranchName', 'txtLocationName', 'txtInflowQuantity', 'txtWaterQuality', 'txtOutflowQuantity', 'txtSlitRemoved', 'dteStp', 'Actions'];
  stpData = new MatTableDataSource();
  @ViewChild("stpDataSort", { static: true }) stpDataSort: MatSort;
  @ViewChild("stpDataPaginator", { static: true }) stpDataPaginator: MatPaginator;

  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtInflowQuantity",
    "txtOutflowQuantity",
    "txtWaterQuality",
    "txtSlitRemoved",
    "dteStp"
  ]);
  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtInflowQuantity",
    "txtOutflowQuantity",
    "txtWaterQuality",
    "txtSlitRemoved",
    "dteStp"
  ];

  constructor(private formBuilder: FormBuilder, private billingService: BillingService, private dateFormat: DateFormatPipe) { }

  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.searchform = this.formBuilder.group({
      branch: [''],
      unionCouncil: [''],
      txtWaterQuality: [''],
      dteStp: ['']
    })
    this.billingService.getAllCustomerTemplateService().subscribe(response => {
      debugger;
      console.log(response)
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];
    })
    this.searchform.controls['branch'].valueChanges.subscribe(element => {
      debugger;
      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getUC = [];
        for (let getBranch of this.ucList) {
          this.getUC.push(getBranch);
        }
        console.log("this.getUC");
        console.log(this.getUC);
      })
    })

    this.onReset();
  }

  setStep(index: number) {
    this.step = index;
  }

  clearTo() {
    this.searchform.controls.dteStp.reset('');
  }

  onSearch() {
    debugger;
    console.log("onSearch")
    console.log(this.searchform.value);
    let bracnhId = +this.searchform.controls.branch.value;
    let ucID = +this.searchform.controls.unionCouncil.value;
    let txtWaterQuality = this.searchform.controls.txtWaterQuality.value;
    let dteStp = '';
    if (this.searchform.controls.dteStp.value != "") {
      dteStp = this.dateFormat.transformOperation(this.searchform.controls.dteStp.value);
    }

    this.billingService.findStpService(bracnhId, ucID, dteStp, txtWaterQuality).subscribe(response => {
      debugger;
      console.log(response)
      if (response.data.length == 0) {
        alert('No record Found');
        this.stpList = [];
        this.stpData = new MatTableDataSource([]);
      }
      else {
        this.stpList = response.data;
        this.stpData = new MatTableDataSource(response.data);
        this.stpData.paginator = this.stpDataPaginator;
        this.stpData.sort = this.stpDataSort;
      }
    });
  }
  
  downloadPDF() {
    this.stpDTO = [];
    let tableHeader: any[] = [];
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtInflowQuantity") {
        tableHeader.push({ text: 'Inflow Quantity', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtOutflowQuantity") {
        tableHeader.push({ text: 'Outflow Quantity', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtWaterQuality") {
        tableHeader.push({ text: 'Water Quality', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtSlitRemoved") {
        tableHeader.push({ text: 'Slit Removed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "dteStp") {
        tableHeader.push({ text: 'Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    })

    this.stpDTO.push(tableHeader);
    this.stpList.forEach((data, i) => {
      let tableRows: any[] = [];
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: data.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: data.txtLocationName, alignment: 'left' });
        } else if (element === "txtInflowQuantity") {
          tableRows.push({ text: data.txtInflowQuantity, alignment: 'left' });
        } else if (element === "txtOutflowQuantity") {
          tableRows.push({ text: data.txtOutflowQuantity, alignment: 'left' });
        } else if (element === "txtWaterQuality") {
          tableRows.push({ text: data.txtWaterQuality, alignment: 'left' });
        } else if (element === "txtSlitRemoved") {
          tableRows.push({ text: data.txtSlitRemoved, alignment: 'left' });
        } else if (element === "dteStp") {
          tableRows.push({ text: data.dteStp, alignment: 'left' });
        }
      })
      this.stpDTO.push(tableRows);
    })

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'STP', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.stpDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('STP List.pdf');
  }

  onReset() {
    this.billingService.findStpService(+'', +'', '', '').subscribe(response => {
      console.log("findStpService");
      console.log(response);
      this.stpList = response.data;
      this.stpData = new MatTableDataSource(response.data);
      this.stpData.paginator = this.stpDataPaginator;
      this.stpData.sort = this.stpDataSort;
    });
  }

  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}