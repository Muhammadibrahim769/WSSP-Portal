...
export class WasteCollectionComponent implements OnInit {
  ...
  getBatchNo: any;
  pageSize = 5;
  wasteCollectionColumn: string[] = ['sNo', 'txtBranchName', 'txtLocationName', 'txtDateServicePerformed', 'txtCollectionType', 'txtStreetName','txtModality',
    'numStreetId',  'txtStartPointCoord', 'txtFinishPointCoord', 'numHouses', 'txtModeOfCollection', 'txtFrequencyWasteCollection', 'numTotalWasteCollected',
    'Actions'];
  wasteCollectionData = new MatTableDataSource();
  @ViewChild("wasteCollectionSort", { static: true }) wasteCollectionSort: MatSort;
  @ViewChild("wasteCollectionPaginator", { static: true }) wasteCollectionPaginator: MatPaginator;
  ...
  isLoading = true;
  dataSource = null;
  clearDateServicePerformed(){
    this.form.controls.txtDateServicePerformed.reset('');
  }
  ngOnInit() {
  ...
  getDesiltationActivity(){
    let txtActivity =this.form.controls.txtActivity.value;
    this.billingService.getRecordByActivity(txtActivity).subscribe(response => {
      debugger;
      this.wasteCollectionList = response["data"];
      this.tripDataList = response["data"];   
      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.wasteCollectionSort;
      this.wasteCollectionData.paginator = this.wasteCollectionPaginator;
    })
  }
  onSearch() {
    debugger;
    ...
    // let txtDateServicePerformed = this.form.controls.txtDateServicePerformed.value;
    // let txtDateServicePerformedEnd = this.form.controls.txtDateServicePerformedEnd.value;
    ...
    let txtDateServicePerformed = '';
    if (this.form.controls.txtDateServicePerformed.value != "") {
      txtDateServicePerformed = this.dateFormat.transformWithTimeZero(this.form.controls.txtDateServicePerformed.value);
    }
    let txtDateServicePerformedEnd = '';
    if (this.form.controls.txtDateServicePerformedEnd.value != "") {
      txtDateServicePerformedEnd = this.dateFormat.transformWithTimeHour(this.form.controls.txtDateServicePerformedEnd.value);
    }
    this.billingService.getAllRecordForWasteCollection(serBranchId, serLocationId, txtDateServicePerformed,txtDateServicePerformedEnd,txtActivity).subscribe(response => {
      debugger;
      this.wasteCollectionList = response["data"];
      this.tripDataList = response["data"];   
      this.wasteCollectionData = new MatTableDataSource(response.data);
  ...
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      ...
      } else if (element === "txtDateServicePerformed") {
        tableHeader.push({ text: 'Date Time of service performed', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCollectionType") {
        tableHeader.push({ text: 'Collection Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtStreetName") {
        tableHeader.push({ text: 'Street Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numStreetId") {
        tableHeader.push({ text: 'Street Id / Container Id', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtModality") {
        tableHeader.push({ text: 'Modality', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtStartPointCoord") {
        tableHeader.push({ text: 'Start Point Coord', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
      else if (element === "txtFinishPointCoord") {
        tableHeader.push({ text: 'Finish Point Coord', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numHouses") {
        tableHeader.push({ text: 'No of Houses', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtModeOfCollection") {
        tableHeader.push({ text: 'Mode Of Collection', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtFrequencyWasteCollection") {
        tableHeader.push({ text: 'Frequency of Waste Collection', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numTotalWasteCollected") {
        tableHeader.push({ text: 'Total Waste Collected', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.wasteCollectionDTO.push(tableHeader);
    console.log(this.wasteCollectionDTO);
    this.wasteCollectionList.forEach((wasteCollectionList, i) => {
      ...
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: wasteCollectionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: wasteCollectionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtDateServicePerformed") {
          tableRows.push({ text: this.dateFormat.transformDateTimeFormat( wasteCollectionList.txtDateServicePerformed.toString()),alignment: 'left' });
        } else if (element === "txtCollectionType") {
          tableRows.push({ text: wasteCollectionList.txtCollectionType, alignment: 'left' });
        } else if (element === "txtStreetName") {
          tableRows.push({ text: wasteCollectionList.txtStreetName, alignment: 'left' });
        } else if (element === "numStreetId") {
          tableRows.push({ text: wasteCollectionList.numStreetId, alignment: 'left' });
        }
         else if (element === "txtModality") {
          tableRows.push({ text: wasteCollectionList.txtModality, alignment: 'left' });
        }
         else if (element === "txtStartPointCoord") {
          tableRows.push({ text: wasteCollectionList.txtStartPointCoord, alignment: 'left' });
        }
        else if (element === "txtFinishPointCoord") {
          tableRows.push({ text: wasteCollectionList.txtFinishPointCoord, alignment: 'left' });
        } else if (element === "numHouses") {
          tableRows.push({ text: wasteCollectionList.numHouses, alignment: 'left' });
        } else if (element === "txtModeOfCollection") {
          tableRows.push({ text: wasteCollectionList.txtModeOfCollection, alignment: 'left' });
        } else if (element === "txtFrequencyWasteCollection") {
          tableRows.push({ text: wasteCollectionList.txtFrequencyWasteCollection, alignment: 'left' });
        } else if (element === "numTotalWasteCollected") {
          tableRows.push({ text: wasteCollectionList.numTotalWasteCollected, alignment: 'left' });
        }
      });
      ...
      content: [
        {
          ...
          table: {
            ...
            body: [
              ...
              }, [{ text: 'Waste Collection', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
    ...
    pdfMake.createPdf(documentDefinition).download('Waste Collection List.pdf');
  }
