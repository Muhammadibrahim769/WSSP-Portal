import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDatepicker } from '@angular/material/datepicker';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ActivatedRoute, Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { UtilsFunctions } from '@app/utils/utilsFunctions';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { CrudService } from '@app/_services/crud.service';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-dashboard',
  templateUrl: './compost.component.html'
})
export class CompostComponent implements OnInit {

  form: FormGroup;
  addForm: FormGroup;
  user: any = "";
  getAllNameCode = [];
  responseList: any[] = [];
  model: NgbDateStruct;
  search: boolean = true;
  add: boolean = false;
  reportDtoList: any = [];
  tripDataList: any;
  panelOpenState = true;
  page = 1;
  driverName: any;
  getAllBatchCode = [];
  getBatchCode = [];
  getBatchNo: any;
  pageSize = 5;
  wasteCollectionColumn: string[] = ['sr', 'txtBranchName', 'txtLocationName', 'txtCompostDate',  'txtProduced', 'numRate',
    'txtVendor', 'numAmount', 'numBalance', 'numSold',
    'Actions'];



  wasteCollectionData = new MatTableDataSource();
  @ViewChild("desiltationActivitySort", { static: true }) desiltationActivitySort: MatSort;
  @ViewChild("desiltationActivityPaginator", { static: true }) desiltationActivityPaginator: MatPaginator;
  constructor(private modalService: NgbModal, private formBuilder: FormBuilder, private billingService: BillingService, private utilsFunctions: UtilsFunctions, private shared: CrudService, private route: ActivatedRoute, private router: Router, private dateFormat: DateFormatPipe,) {
  }
  clearWEFD() {
    this.form.controls.dteDate.reset('');
  }
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  routesLIst: any[] = [];
  isLoading = true;
  dataSource = null;
  clearTimeOut() {
    this.form.controls.txtCompostDate.reset('');
  }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.form = this.formBuilder.group({
      serUserId: [""],
      serBranchId: [''],
      serLocationId: [''],
      txtCompostDate: [''],
      txtVendor: [''],
    })
    this.getBranchList();
    this.getCompostFindAll();
    this.form.controls['serBranchId'].valueChanges.subscribe(element => {

      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }
        console.log("this.getBranchId");
        console.log(this.getBranchId);
      })
    })
    this.user = JSON.parse(sessionStorage.getItem('user'));

    console.log(this.user);
  }
  getBranchList() {
    console.log(this.user);
    console.log("user id " + this.user["serUserId"]);

    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {

      console.log("getBranchService");
      console.log(response);
      this.branchList = response
    })
  }
  getCompostFindAll() {

    this.billingService.getCompostFindAll().subscribe(response => {
      console.log(response)
      this.tripDataList = response["data"];
      this.wasteCollectionList = response["data"]
      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.desiltationActivitySort;
      this.wasteCollectionData.paginator = this.desiltationActivityPaginator;
    })
  }
  onSearch() {


    let serBranchId = this.form.controls.serBranchId.value;
    let serLocationId = this.form.controls.serLocationId.value;
    let txtVendor = this.form.controls.txtVendor.value;

    let txtCompostDate = '';
    if (this.form.controls.txtCompostDate.value != "") {
      txtCompostDate = this.dateFormat.transformOperation(this.form.controls.txtCompostDate.value);
    }

    this.billingService.getAllRecordCompost(serBranchId, serLocationId, txtCompostDate, txtVendor).subscribe(response => {

      this.tripDataList = response["data"];
      this.wasteCollectionList = response["data"];

      this.wasteCollectionData = new MatTableDataSource(response.data);
      this.wasteCollectionData.sort = this.desiltationActivitySort;
      this.wasteCollectionData.paginator = this.desiltationActivityPaginator;
    })
  }
  wasteCollectionList: any = "";
  // wasteCollectionList: any[] = [];
  wasteCollectionDTO: any[] = [];
  downloadPDF() {

    console.log(this.selection);
    this.wasteCollectionDTO = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCompostDate") {
        tableHeader.push({ text: 'Compost Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }  else if (element === "txtProduced") {
        tableHeader.push({ text: 'Compost Produced', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtVendor") {
        tableHeader.push({ text: 'Compost Vender', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numRate") {
        tableHeader.push({ text: 'Compost Rate', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numAmount") {
        tableHeader.push({ text: 'CompostAmount', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numBalance") {
        tableHeader.push({ text: 'Compost  Balance', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numSold") {
        tableHeader.push({ text: 'Compost Sold', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }


    });
    this.wasteCollectionDTO.push(tableHeader);
    console.log(this.wasteCollectionDTO);
    this.wasteCollectionList.forEach((wasteCollectionList, i) => {

      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtBranchName") {
          tableRows.push({ text: wasteCollectionList.txtBranchName, alignment: 'left' });
        } else if (element === "txtLocationName") {
          tableRows.push({ text: wasteCollectionList.txtLocationName, alignment: 'left' });
        } else if (element === "txtCompostDate") {
          tableRows.push({ text: this.dateFormat.transformOperation(wasteCollectionList.txtCompostDate.toString()), alignment: 'left' });
        } else if (element === "txtProduced") {
          tableRows.push({ text: wasteCollectionList.txtProduced, alignment: 'left' });
        } else if (element === "txtVendor") {
          tableRows.push({ text: wasteCollectionList.txtVendor, alignment: 'left' });
        }
        else if (element === "numRate") {
          tableRows.push({ text: wasteCollectionList.numRate, alignment: 'left' });
        }
        else if (element === "numAmount") {
          tableRows.push({ text: wasteCollectionList.numAmount, alignment: 'left' });
        }
        else if (element === "numBalance") {
          tableRows.push({ text: wasteCollectionList.numBalance, alignment: 'left' });
        } else if (element === "numSold") {
          tableRows.push({ text: wasteCollectionList.numSold, alignment: 'left' });
        }
      });
      this.wasteCollectionDTO.push(tableRows)
    });


    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto','auto'],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Compost Record', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -25, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.wasteCollectionDTO
          }
        },
        {
          alignment: 'justify',
          columns: [
            {
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
            {
              text: ''
            },
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
          ]
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Compost List.pdf');
  }

  tableColumn: string[] = [
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtCompostDate",
    "txtProduced",
    "txtVendor",
    "numRate",
    "numAmount",
    "numSold",
    "numBalance",
    "numTotalWasteCollected"
  ];
  selection = new FormControl([
    "sr",
    "txtBranchName",
    "txtLocationName",
    "txtCompostDate",
    "txtProduced",
    "txtVendor",
    "numRate",
    "numAmount",
    "numSold",
    "numBalance",
    "numTotalWasteCollected"
  ]);
  selectingColumns() {

    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}


