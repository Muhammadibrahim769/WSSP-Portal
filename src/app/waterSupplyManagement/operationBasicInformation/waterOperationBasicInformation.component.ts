import {
  Component,
  AfterViewInit,
  OnInit,
  ViewChild,
  ChangeDetectorRef,
} from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { Router } from '@angular/router';
import { RejectModalComponent } from '@app/modals/rejectModal.component';
import {
  MomentDateAdapter,
  MAT_MOMENT_DATE_ADAPTER_OPTIONS,
} from '@angular/material-moment-adapter';
import {
  DateAdapter,
  MAT_DATE_FORMATS,
  MAT_DATE_LOCALE,
} from '@angular/material/core';
import * as _moment from 'moment';
import { EditModalComponent } from '@app/modals/editModal.component';
import { BillingService } from '@app/services/billing.service';
import { UtilsService } from '@app/_services/utils.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import { EditTabsModalComponent } from '@app/modals/editTabsModal.component';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
const moment = _moment;
export const MY_FORMATS = {
  parse: {
    dateInput: 'LL',
  },
  display: {
    dateInput: 'LL',
    monthYearLabel: 'MMM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MMMM YYYY',
  },
};
@Component({
  selector: 'app-dashboard',
  templateUrl: './waterOperationBasicInformation.component.html',
  providers: [
    {
      provide: DateAdapter,
      useClass: MomentDateAdapter,
      deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS],
    },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class WaterOperationBasicInformationComponent implements OnInit {
  operationBasicInfoForm: FormGroup;
  step = 0;
  show = false;
  updatedTWBasicinfo: any[] = [];
  responseList: any[] = [];
  searchTubeWellData: any[] = [];
  isLoading = true;
  dataSource = null;
  date = new FormControl(moment());
  pageSizeOptions: number[] = [10, 20, 50];
  operationBasicInfoColumns: string[] = [
    'txt_branch_name',
    'txt_location_name',
    'longitude',
    'lattitude',
    'txtTubewellRefNo',
    'dteInstallationDate',
    'txtPumpingMachineryType',
    'numPumpingPower',
    'numPumpingDepth',
    'numTransformerCapacity',
    'txtProductCode',
    'txtProductName',
    'txtMeterType',
    'numOfConnections',
    'AvgDischarge',
    'blnIsFunctional',
    'blnChamberAvailable',
    'blnIsMetered',
    'dteWarrenty',
    'Actions',
  ];
  operationBasicInfo = new MatTableDataSource();
  @ViewChild('operationBasicInfoSort', { static: true })
  operationBasicInfoSort: MatSort;
  @ViewChild('operationBasicInfoPaginator', { static: true })
  operationBasicInfoPaginator: MatPaginator;
  constructor(
    private formBuilder: FormBuilder,
    private utilService: UtilsService,
    private changeDetectorRefs: ChangeDetectorRef,
    public dialog: MatDialog,
    private billingService: BillingService,
    private router: Router,
    private dateFormat: DateFormatPipe
  ) {}
  ucList: any;
  user = '';
  getBranchId = [];
  getUClist = [];
  branchList: any;
  unionCouncilList: any[] = [];
  ngOnInit(): void {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.operationBasicInfoForm = this.formBuilder.group({
      serBranchId: [''],
      serLocationId: [''],
      txtBranch: [''],
      txtUnionCouncil: [''],
      serNumOfConnection: [''],
      txtAvgDischarge: [''],
      txtProductCode: [''],
      txtProductName: [''],
      dteInstallationDate: [''],
      blnChamberAvailable: [''],
      blnIsMetered: [''],
      txtPumpingMachineryType: [''],
      numTransformerCapacity: [''],
      numPumpingDepth: [''],
      numPumpingPower: [''],
      blnIsFunctional: [''],
      txtRemarks: [''],
      txtTubewellRefNo: [''],
      numAvgDischargeVolume: [''],
    });

    this.onGet();
    this.refresh();
    this.getBranchList();
    this.operationBasicInfoForm.controls['serBranchId'].valueChanges.subscribe(
      (element) => {
        debugger;
        this.billingService.getBranchIdForRoutes(element).subscribe((data) => {
          this.ucList = data;
          this.getBranchId = [];
          for (let getBranch of this.ucList) {
            this.getBranchId.push(getBranch);
          }
          console.log('this.getBranchId');
          console.log(this.getBranchId);
        });
      }
    );
  }
  getBranchList() {
    console.log(this.user);
    console.log('user id ' + this.user['serUserId']);
    debugger;
    this.billingService
      .getBranchService(this.user['serUserId'])
      .subscribe((response) => {
        debugger;
        console.log('getBranchService');
        console.log(response);
        this.branchList = response;
      });
  }
  onSearch() {
    debugger;
    let blnIsFunctional =
      this.operationBasicInfoForm.controls.blnIsFunctional.value;

    let blnIsMetered = this.operationBasicInfoForm.controls.blnIsMetered.value;
    let serBranchId = this.operationBasicInfoForm.controls.serBranchId.value;
    let serLocationId =
      this.operationBasicInfoForm.controls.serLocationId.value;
    let txtProductCode =
      this.operationBasicInfoForm.controls.txtProductCode.value;
    let txtProductName =
      this.operationBasicInfoForm.controls.txtProductName.value;
    let txtTubewellRefNo =
      this.operationBasicInfoForm.controls.txtTubewellRefNo.value;
    this.billingService
      .getOperationBasicInfoServiceByParams(
        txtTubewellRefNo,
        blnIsFunctional,
        blnIsMetered,
        serBranchId,
        serLocationId,
        txtProductCode,
        txtProductName
      )
      .subscribe((Response) => {
        debugger;
        this.searchTubeWellData = Response['data'];
        console.log('Response');
        console.log(Response);
        this.isLoading = false;
        this.dataSource = Response['data'];
        // this.selecttubewellList = [];
        this.operationBasicInfo = new MatTableDataSource(Response['data']);
        this.operationBasicInfo.sort = this.operationBasicInfoSort;
        this.operationBasicInfo.paginator = this.operationBasicInfoPaginator;
        this.pdfImplementation();
      });
  }
  onGet() {
    debugger;
    this.billingService.getOperationBasicInfoService().subscribe((response) => {
      console.log('operationBasicInfo');
      console.log(response.data);
      this.isLoading = false;

      this.dataSource = response.data;
      this.operationBasicInfo = new MatTableDataSource(response.data);
      this.operationBasicInfo.sort = this.operationBasicInfoSort;
      this.operationBasicInfo.paginator = this.operationBasicInfoPaginator;
      this.pdfImplementation();
      // this.changeDetectorRefs.detectChanges();
    });
  }
  refresh() {
    this.changeDetectorRefs.detectChanges();
  }
  openEditModal(id: any) {
    const EditData = this.dialog.open(EditModalComponent, {
      data: id,
    });
    EditData.afterClosed().subscribe(() => {
      this.onSearch();
    });
  }
  openEditTabsModal(id: any) {
    debugger;
    const EditData = this.dialog.open(EditTabsModalComponent, {
      data: id,
    });
    EditData.afterClosed().subscribe(() => {
      this.onSearch();
    });
  }
  openSubmitDialog() {
    // this.dialog.open(SubmitModalComponent);
    const document = this.dialog.open(RejectModalComponent, {
      data: {
        message: 'Are you sure want to Submit ?',
        buttonText: {
          ok: 'Yes',
          cancel: 'No',
        },
      },
    });
    document.afterClosed().subscribe((confirmed: boolean) => {
      debugger;
      if (confirmed) {
        this.show = true;
      }
    });
  }
  showColumn = false;
  setStep(index: number) {
    this.step = index;
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList = this.dataSource;
    console.log('daownload data ');
    console.log(this.tubewellDataList);
    debugger;
    this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Tubewell Ref No', bold: true },
      { text: 'Tubewell Number', bold: true },
      { text: 'Tubewell Name', bold: true },
      { text: 'Meter Type', bold: true },
      { text: 'No of Connections', bold: true },
      { text: 'Avg Discharge m3/hr', bold: true },
      { text: 'UOM', bold: true },
      { text: 'blnChamberAvailable', bold: true },
      { text: 'Functional', bold: true },
      { text: 'Metered', bold: true },
      { text: 'Warrenty', bold: true },
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      this.tubewellReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tubewellDataList.txtBranchName, alignment: 'left' },
        { text: tubewellDataList.txtLocationName, alignment: 'left' },
        { text: tubewellDataList.txtTubewellRefNo, alignment: 'left' },
        { text: tubewellDataList.txtProductCode, alignment: 'left' },
        { text: tubewellDataList.txtProductName, alignment: 'left' },
        { text: tubewellDataList.txtMeterType, alignment: 'right' },
        { text: tubewellDataList.numOfConnections, alignment: 'right' },
        { text: tubewellDataList.numAvgDischargeVolume, alignment: 'right' },
        { text: tubewellDataList.numAvgDischargeVolum, alignment: 'right' },

        { text: tubewellDataList.blnIsFunctional, alignment: 'right' },
        { text: tubewellDataList.blnChamberAvailable, alignment: 'right' },
        { text: tubewellDataList.blnIsMetered, alignment: 'right' },
        {
          text: this.dateFormat.transformOperation(
            tubewellDataList.dteWarrenty
          ),
          alignment: 'right',
        },
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.tubewellReportDTOList = [];
    let tableHeader: any[] = [];
    this.selection.value.forEach((element) => {
      if (element === 'sr') {
        tableHeader.push({
          text: 'Sr#',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txt_branch_name') {
        tableHeader.push({
          text: 'Zone / Branch',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txt_location_name') {
        tableHeader.push({
          text: 'Union Council',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtTubewellRefNo') {
        tableHeader.push({
          text: 'Tubewell Ref No',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtProductCode') {
        tableHeader.push({
          text: 'Tubewell Number',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtProductName') {
        tableHeader.push({
          text: 'Tubewell Name',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtMeterType') {
        tableHeader.push({
          text: 'Meter Type',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numOfConnections') {
        tableHeader.push({
          text: 'No of Connections',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'AvgDischarge') {
        tableHeader.push({
          text: 'Avg Discharge m3/hr',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'blnIsFunctional') {
        tableHeader.push({
          text: 'Functional',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'blnChamberAvailable') {
        tableHeader.push({
          text: 'Chamber',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'blnIsMetered') {
        tableHeader.push({
          text: 'Metered',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'dteWarrenty') {
        tableHeader.push({
          text: 'Warrenty',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
    });
    this.tubewellReportDTOList.push(tableHeader);
    console.log(this.tubewellReportDTOList);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      let tableRows: any[] = [];
      this.selection.value.forEach((element) => {
        if (element === 'sr') {
          tableRows.push({ text: i + 1, alignment: 'right' });
        } else if (element === 'txt_branch_name') {
          tableRows.push({
            text: tubewellDataList.txtBranchName,
            alignment: 'left',
          });
        } else if (element === 'txt_location_name') {
          tableRows.push({
            text: tubewellDataList.txtLocationName,
            alignment: 'left',
          });
        } else if (element === 'txtTubewellRefNo') {
          tableRows.push({
            text: tubewellDataList.txtTubewellRefNo,
            alignment: 'left',
          });
        } else if (element === 'txtProductCode') {
          tableRows.push({
            text: tubewellDataList.txtProductCode,
            alignment: 'left',
          });
        } else if (element === 'txtProductName') {
          tableRows.push({
            text: tubewellDataList.txtProductName,
            alignment: 'left',
          });
        } else if (element === 'txtMeterType') {
          tableRows.push({
            text: tubewellDataList.txtMeterType,
            alignment: 'left',
          });
        } else if (element === 'numOfConnections') {
          tableRows.push({
            text: tubewellDataList.numOfConnections,
            alignment: 'right',
          });
        } else if (element === 'AvgDischarge') {
          tableRows.push({
            text: tubewellDataList.numAvgDischargeVolume,
            alignment: 'right',
          });
        } else if (element === 'blnIsFunctional') {
          tableRows.push({
            text: tubewellDataList.blnIsFunctional,
            alignment: 'left',
          });
        } else if (element === 'blnChamberAvailable') {
          tableRows.push({
            text: tubewellDataList.blnChamberAvailable,
            alignment: 'right',
          });
        } else if (element === 'blnIsMetered') {
          tableRows.push({
            text: tubewellDataList.blnIsMetered,
            alignment: 'left',
          });
        } else if (element === 'dteWarrenty') {
          tableRows.push({
            text: this.dateFormat.transformOperation(
              tubewellDataList.dteWarrenty
            ),
            alignment: 'left',
          });
        }
      });
      this.tubewellReportDTOList.push(tableRows);
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [
                {
                  image:
                    'data:image/png;base64,',
                },
                [
                  {
                    text: 'Tubewell Report',
                    style: 'dateHeader',
                    alignment: 'center',
                    margin: [200, 60, 150, 0],
                    bold: true,
                    fontSize: 16,
                  },
                ],
                {
                  image:
                    'data:image/png;base64,',
                  width: 100,
                  height: 100,
                  alignment: 'right',
                  margin: [0, 0, -80, 0],
                },
              ],
            ],
          },
          layout: 'noBorders',
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.tubewellReportDTOList,
          },
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10],
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5],
        },
        tableExample: {
          margin: [0, 5, 0, 15],
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
        },
      },
      defaultStyle: {
        // alignment: 'justify'
      },
    };

    pdfMake.createPdf(documentDefinition).download('TubeWell List.pdf');
  }
  tableColumn: string[] = [
    'sr',
    'txt_branch_name',
    'txt_location_name',
    'txtTubewellRefNo',
    'dteInstallationDate',
    'txtPumpingMachineryType',
    'numPumpingPower',
    'numPumpingDepth',
    'numTransformerCapacity',
    'txtProductCode',
    'txtProductName',

    'txtMeterType',
    'numOfConnections',
    'AvgDischarge',
    // "numAvgDischargeVolume",

    'blnIsFunctional',
    'blnChamberAvailable',
    'blnIsMetered',
    'dteWarrenty',
  ];
  selection = new FormControl([
    'sr',
    'txt_branch_name',
    'txt_location_name',
    'txtTubewellRefNo',
    'dteInstallationDate',
    'txtPumpingMachineryType',
    'numPumpingPower',
    'numPumpingDepth',
    'numTransformerCapacity',
    'txtProductCode',
    'txtProductName',
    'txtMeterType',

    'numOfConnections',
    'AvgDischarge',
    // "numAvgDischargeVolume",
    'blnIsFunctional',
    'blnChamberAvailable',
    'blnIsMetered',
    'dteWarrenty',
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach((singleColumn) => {
        let display = '';
        if (singleColumn !== 'sr') {
          let columnSelected = this.selection.value.findIndex(
            (x) => x === singleColumn
          );
          if (columnSelected >= 0) {
            display = 'table-cell';
          } else {
            display = 'none';
          }
          const slides = document.getElementsByClassName(
            'mat-column-' + singleColumn
          );
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
  onReset() {
    this.operationBasicInfoForm.controls.serBranchId.reset('');
    this.operationBasicInfoForm.controls.serLocationId.reset('');
    this.operationBasicInfoForm.controls.txtBranch.reset('');
    this.operationBasicInfoForm.controls.txtUnionCouncil.reset('');
    this.operationBasicInfoForm.controls.txtProductCode.reset('');
    this.operationBasicInfoForm.controls.txtProductName.reset('');
    this.operationBasicInfoForm.controls.blnIsMetered.reset('');
    this.operationBasicInfoForm.controls.blnIsFunctional.reset('');
    this.onGet();
  }
}
