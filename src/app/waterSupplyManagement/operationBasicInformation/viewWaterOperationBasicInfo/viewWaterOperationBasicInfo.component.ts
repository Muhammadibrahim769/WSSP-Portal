import { Component, OnInit } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { ActivatedRoute } from '@angular/router';
import { BillingService } from '@app/services/billing.service';

import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './viewWaterOperationBasicInfo.component.html',
})
export class ViewWaterOperationBasicInfoComponent implements OnInit {
  Id: any = "";
  basicInfo: any;
  user="";
  // blnIsFunctional:any;
  // blnIsMetered:any;
  constructor(private dialog: MatDialog, private route: ActivatedRoute, private billingService: BillingService, private dateFormat: DateFormatPipe) { }
  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.route.paramMap.subscribe((params) => {
      this.Id = +params.get("id");
    });
    this.billingService.getOperationBasicInfoListServiceByID(this.Id).subscribe(response => {

      console.log(response)

      this.basicInfo = response;
    })
  }
  downloadPDF() {
    const documentDefinition = {
      pageOrientation: 'A4',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: [
              [{
                image: 'data:image/png;base64,',
                width: 80,
                height: 80,
                alignment: 'left'
              }, 
              [{ text: 'TubeWellReport', style: 'tableHeaderValue', alignment: 'center',          headerRows: 1,
              margin: [100, 40, 150, 0], bold: true, fontSize: 16
            }],
              {
                image: 'data:image/png;base64,', width: 70,
                height: 70,
                alignment: 'right', margin: [20, 5, 25, 0],
              }],
  
  
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            widths: [110, 145, 110, 145],
            // widths: ['auto', 'auto', 'auto', 'auto'],
  
            body: [
              [
                { text: 'Zone/Branch:', style: 'tableHeader' },
                { text: this.basicInfo.txtBranchName, style: 'tableHeaderValue' },
                { text: 'Union Council:', style: 'tableHeader' },
                { text: this.basicInfo.txtLocationName, style: 'tableHeaderValue' },
              ],
              [
                { text: 'TubeWell Number:', style: 'tableHeader' },
                { text: this.basicInfo.txtProductCode, style: 'tableHeaderValue' },
                { text: 'TubeWell Name:', style: 'tableHeader' },
                { text: this.basicInfo.txtProductName, style: 'tableHeaderValue' }

              ],
              [
                { text: 'TubeWell Reference NO:', style: 'tableHeader' },
                { text: this.basicInfo.txtTubewellRefNo, style: 'tableHeaderValue' },
                { text: 'Avg Discharge m3/hr:', style: 'tableHeader' },
                { text: this.basicInfo.numAvgDischargeVolume, style: 'tableHeaderValue' }
              ],
              [
                { text: 'Functional:', style: 'tableHeader' },
                { text:( this.basicInfo.blnIsFunctional === true) ? 'Functional' : 'Non-Functional' 
                  , style: 'tableHeaderValue' },
                { text: 'Connection:', style: 'tableHeader' },
                { text: ( this.basicInfo.blnIsMetered === true) ? 'Metered' : 'Un-Metered'
                  , style: 'tableHeaderValue' }
              ],
              [
                { text: 'Meter Type', style: 'tableHeader' },
                { text: this.basicInfo.txtMeterType, style: 'tableHeaderValue' },
                
                { text: 'No Of Connection :', style: 'tableHeader' },
                { text: this.basicInfo.numOfConnections, style: 'tableHeaderValue' },
              ],
              [
                { text: 'Chamber:', style: 'tableHeader' },
                { text:( this.basicInfo.blnChamberAvailable === true) ? 'Available' : 'Not Available' 
                  , style: 'tableHeaderValue' },
                { text: 'Water Table History(FT)', style: 'tableHeader' },
                { text: this.basicInfo.numPumpingDepth, style: 'tableHeaderValue' },
             
              ],
              [
                { text: 'Warrenty Date', style: 'tableHeader' },
                { text: this.dateFormat.transformOperation(this.basicInfo.dteWarrenty), style: 'tableHeaderValue' },
                { text: 'Installation Date', style: 'tableHeader' },
                { text: this.dateFormat.transformOperation(this.basicInfo.dteInstallationDate), style: 'tableHeaderValue' },
              ],
              [
                { text: 'Pumping Machineri(CHINERY)HP', style: 'tableHeader' },
                { text: this.basicInfo.numPumpingPower, style: 'tableHeaderValue' },
                { text: 'Type of Pumping Machineri', style: 'tableHeader' },
                { text: this.basicInfo.txtPumpingMachineryType, style: 'tableHeaderValue' },
              ],
              [
                { text: 'longitude', style: 'tableHeader' },
                { text: this.basicInfo.longitude, style: 'tableHeaderValue' },
                { text: 'Latitude', style: 'tableHeader' },
                { text: this.basicInfo.lattitude, style: 'tableHeaderValue' },
              ],
              [
                { text: 'Transforme Capicity(KVA)', style: 'tableHeader' },
                { text: this.basicInfo.numTransformerCapacity, style: 'tableHeaderValue' },
                { text: '', style: 'tableHeader' },
                { text: '', style: 'tableHeaderValue' },
              
              ],
            ]


          },
          layout: 'noBorders'
          
        },
        {
          alignment: 'justify',
        
          columns: [
         
            
            {
              text: [{ text: ' ', bold: true,
            },
            ]
            },
            
            {
              text: [{ text: ' ', bold: true,
            },
            ]
            },
            {
              text: [{ text: ' ', bold: true,
            },
            ]
            },
             
          ]
        },
        {
          alignment: 'justify',
        
          columns: [
         
            
            {
              text: [{ text: ' ', bold: true,
            },
            ]
            },
            
            {
              text: [{ text: ' ', bold: true,
            },
            ]
            },
            {
              text: [{ text: ' ', bold: true,
            },
            ]
            },
             
          ]
        },
        
        {
          
          alignment: 'justify',
        
          columns: [
                
            {
          
              
              text: [{ text: 'Printed Date: ', bold: true }, this.dateFormat.transformDateTime(new Date())],
            },
              
            {
              text: [{ text: 'Printed By: ', bold: true }, this.user["txtUserName"]]
            }
             
          ]
        },
       
        {
          alignment: 'justify',
        
          columns: [
         
          
             
          ]
        },
       
      ],
      styles: {
        header: {
          fontSize: 16,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
          padding: [50, 0, 0, 0]
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
          padding: [50, 0, 0, 0]
        }
      },
      defaultStyle: {
      }
    };
    pdfMake.createPdf(documentDefinition).download('TubeWell Detail.pdf');
  }
}
