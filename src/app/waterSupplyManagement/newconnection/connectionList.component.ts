import { Component, AfterViewInit, ViewChild, OnInit } from '@angular/core';
import { MatTableDataSource } from '@angular/material/table';
import { SelectionModel } from '@angular/cdk/collections';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { UtilsService } from '@app/_services/utils.service';
import { MatDialog } from '@angular/material/dialog';
import { ApprovedModalComponent } from '@app/modals/approvedModal.component';
import { MatPaginator } from '@angular/material/paginator';
import { RejectModalComponent } from '@app/modals/rejectModal.component';
import { MatSort } from '@angular/material/sort';
import { BillingService } from '@app/services/billing.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
// ------------------------------ Data Source For Table --------------------------
export interface PeriodicElement {
  branch: string;
  // @input data
  unionCouncil: string;
  connectionType: string;
  consumerId: number;
  consumerName: string;
  category: string;
}

const ELEMENT_DATA: PeriodicElement[] = [
  {
    branch: 'A',
    unionCouncil: 'A',
    connectionType: 'Commercial',
    consumerId: 1,
    consumerName: 'ABC',
    category: 'QWE',
  },
  {
    branch: 'B',
    unionCouncil: 'A',
    connectionType: 'Domestic',
    consumerId: 2,
    consumerName: 'XYZ',
    category: 'TYP',
  },
  {
    branch: 'C',
    unionCouncil: 'A',
    connectionType: 'Aquifer',
    consumerId: 3,
    consumerName: 'ABC',
    category: 'MAN',
  },
  {
    branch: 'D',
    unionCouncil: 'A',
    connectionType: 'Commercial',
    consumerId: 4,
    consumerName: 'XYZ',
    category: 'QWE',
  },
];

//---------------------------  Copmonent Body Starts --------------------------------

@Component({
  selector: 'app-dashboard',
  templateUrl: './connectionList.component.html',
})
export class ConnectionListComponent implements OnInit {
  step = 0;
  searchConnectionForm: FormGroup;
  latitude: number;
  longitude: number;
  zoom: number;
  tabIndex: number;

  newConnectionRequestColumn: string[] = [
    'txtCustomerCode',
    'branch',
    'unionCouncil',
    'txtConnectionType',
    'txtBusinessName',
    'category',
    'Actions',
  ];
  allConnectionColumn: string[] = [
    'txtCustomerCode',
    'branch',
    'unionCouncil',
    'txtConnectionType',
    'txtBusinessName',
    'category',
    'txtStatus',
    'Actions',
  ];
  newConnection = new MatTableDataSource();
  allConnection = new MatTableDataSource();
  @ViewChild('newConnectionSort', { static: true }) newConnectionSort: MatSort;
  @ViewChild('newConnectionPaginator', { static: true })
  newConnectionPaginator: MatPaginator;
  @ViewChild('allConnectionSort', { static: true }) allConnectionSort: MatSort;
  @ViewChild('allConnectionPaginator', { static: true })
  allConnectionPaginator: MatPaginator;
  selection = new SelectionModel<PeriodicElement>(true, []);
  branchList: any[] = [];
  unionCouncilList: any[] = [];
  connectionTypeList: any[] = [];
  categoryList: any[] = [];
  statusList: any[] = [];
  selectStatusList: any[] = [];
  responseList: any[] = [];

  ngAfterViewInit() {}
  constructor(
    private formBuilder: FormBuilder,
    private utilService: UtilsService,
    public dialog: MatDialog,
    private billingService: BillingService
  ) {}

  ngOnInit(): void {
    this.searchConnectionForm = this.formBuilder.group({
      branch: [''],
      unionCouncil: [''],
      txtConnectionType: [''],
      txtCustomerCode: [''],
      txtStatus: [''],
    });
    this.pdfImplementation();
    this.pdfImplementationAll();
    this.billingService
      .getAllCustomerTemplateService()
      .subscribe((response) => {
        debugger;
        console.log(response);
        console.log(response['branchList']);
        this.branchList = response['branchList'];

        console.log(response['unionConcilList']);
        this.unionCouncilList = response['unionConcilList'];

        console.log(response['connectionTypeList']);
        this.connectionTypeList = response['connectionTypeList'];

        console.log(response['categoryList']);
        this.categoryList = response['categoryList'];

        console.log(response['statusList']);
        this.statusList = response['statusList'];
      });

    this.getAllConnection();
    this.getNewConnection();
    this.onValChange(value);
    this.tabIndex = 0;
  }
  dataArray = [];
  allConnectionData: any[] = [];
  getNewConnection() {
    this.billingService.getAllDumpCustomerService().subscribe((response) => {
      console.log('newDumpCutomer');
      console.log(response.data);
      debugger;
      this.dataArray = [];
      response.data.forEach((element) => {
        if (element.txtStatus == 'PENDING') {
          this.dataArray.push(element);
        }
      });
      this.newConnection = new MatTableDataSource(this.dataArray);
      this.newConnection.sort = this.newConnectionSort;
      this.newConnection.paginator = this.newConnectionPaginator;
      this.pdfImplementation();
    });
  }

  getAllConnection() {
    this.billingService.getAllCutomerService().subscribe((response) => {
      console.log('allCutomer');
      console.log(response.data);
      debugger;
      this.allConnectionData = response['data'];
      this.allConnection = new MatTableDataSource(response.data);
      this.allConnection.sort = this.allConnectionSort;
      this.allConnection.paginator = this.allConnectionPaginator;
      this.pdfImplementationAll();
    });
  }
  onSubmit() {
    debugger;
    console.log(this.searchConnectionForm.value);
    let formValue = this.searchConnectionForm.value;
    formValue.branch = { id: +this.searchConnectionForm.controls.branch.value };
    formValue.unionCouncil = {
      id: +this.searchConnectionForm.controls.unionCouncil.value,
    };
    let code = this.searchConnectionForm.controls.txtCustomerCode.value;
    let brachID = this.searchConnectionForm.controls.branch.value;
    let UcID = this.searchConnectionForm.controls.unionCouncil.value;
    let connType = this.searchConnectionForm.controls.txtConnectionType.value;
    let status = this.searchConnectionForm.controls.txtStatus.value;

    if (this.tabIndex == 0) {
      this.billingService
        .searchConnectionCustomerDumpListServiceByCode(
          code,
          brachID,
          UcID,
          connType,
          status,
          ''
        )
        .subscribe((response) => {
          debugger;
          this.selectStatusList = [];
          console.log('response');
          console.log(response);
          if (response['count'] == 0) {
            this.selectStatusList = [];
            this.newConnection = new MatTableDataSource(this.selectStatusList);
            alert('No Record found');
          } else {
            this.responseList = response['data'];
            this.responseList.forEach((element) => {
              if (element.txtStatus === 'PENDING') {
                this.selectStatusList.push(element);
              }
              if (this.selectStatusList.length == 0) {
                alert('NO Record Found');
              }
              this.newConnection = new MatTableDataSource(
                this.selectStatusList
              );
              this.newConnection.sort = this.newConnectionSort;
              this.newConnection.paginator = this.newConnectionPaginator;
            });
          }
        });
    } else if (this.tabIndex == 1) {
      this.billingService
        .searchConnectionCustomerListServiceByCode(
          code,
          brachID,
          UcID,
          connType,
          status,
          ''
        )
        .subscribe((response) => {
          debugger;
          this.selectStatusList = [];
          console.log('response');
          console.log(response);

          if (response['count'] == 0) {
            this.selectStatusList = [];
            this.allConnection = new MatTableDataSource(this.selectStatusList);
            alert('No Record found');
          } else {
            this.responseList = response['data'];
            this.responseList.forEach((element) => {
              debugger;
              this.selectStatusList.push(element);
              // if(element.txtStatus === "ACTIVE" || element.txtStatus === "SUSPENDED" || element.txtStatus === "SUSPENSION"){
              //   this.selectStatusList.push(element)
              // }
              this.allConnection = new MatTableDataSource(
                this.selectStatusList
              );
              this.allConnection.sort = this.allConnectionSort;
              this.allConnection.paginator = this.allConnectionPaginator;
              this.pdfImplementationAll();
            });
          }
        });
    }
  }

  onReset() {
    this.getNewConnection();
    this.getAllConnection();
  }

  onTabChanged(event) {
    console.log('onTabChanged event');
    console.log(event);
    this.tabIndex = event.index;
    console.log(this.tabIndex);
  }

  openApprovedDialog(element) {
    const document = this.dialog.open(ApprovedModalComponent, {
      data: {
        element,
        buttonText: {
          ok: 'Approve',
          cancel: 'Cancel',
        },
      },
    });
    document.afterClosed().subscribe(() => {
      console.log('openApprovedDialog closed');
      this.getNewConnection();
      this.getAllConnection();
    });
  }

  openRejectDialog(element) {
    console.log(element);
    const document = this.dialog.open(RejectModalComponent, {
      data: {
        element,
        message: 'Are you sure  you want to reject?',
        buttonText: {
          ok: 'Yes',
          cancel: 'No',
        },
      },
    });
    document.afterClosed().subscribe(() => {
      console.log('openRejectDialog closed');
      this.getNewConnection();
      this.getAllConnection();
    });
    //   const rejectData = this.dialog.open(RejectModalComponent, {
    //     data:element
    // }
  }

  openOTBill(element) {
    debugger;
    console.log(element);
    this.billingService
      .getCurrentOTBillService(element.txtCustomerCode)
      .subscribe((response) => {
        console.log(response);
      });
  }
  setStep(index: number) {
    this.step = index;
  }

  /** Whether the number of selected elements matches the total number of rows. */
  isAllSelected() {
    const numSelected = this.selection.selected.length;
    const numRows = this.newConnection.data.length;
    return numSelected === numRows;
  }

  /** Selects all rows if they are not all selected; otherwise clear selection. */
  onValChange(value) {
    debugger;
    console.log(value);
  }
  newConnectionDataList: any[] = [];
  newConnectionReportDtoList: any[] = [];
  pdfImplementation() {
    debugger;
    this.newConnectionDataList = this.dataArray;
    console.log('daownload data ');
    console.log(this.newConnectionDataList);
    debugger;
    this.newConnectionReportDtoList.push([
      { text: 'Sr#', bold: true },
      { text: 'Consumer Id', bold: true },
      { text: 'Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Connection Type', bold: true },
      { text: 'Business Name', bold: true },
      { text: 'Category', bold: true },
    ]);
    this.newConnectionDataList.forEach((newConnectionDataList, i) => {
      debugger;
      this.newConnectionReportDtoList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: newConnectionDataList.txtCustomerCode, alignment: 'left' },
        { text: newConnectionDataList.branch.name, alignment: 'left' },
        { text: newConnectionDataList.unionCouncil.name, alignment: 'left' },
        { text: newConnectionDataList.txtConnectionType, alignment: 'left' },
        { text: newConnectionDataList.txtBusinessName, alignment: 'right' },
        { text: newConnectionDataList.category, alignment: 'right' },
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selectionColumn);
    this.newConnectionReportDtoList = [];
    let tableHeader: any[] = [];
    this.selectionColumn.value.forEach((element) => {
      if (element === 'sr') {
        tableHeader.push({
          text: 'Sr#',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtCustomerCode') {
        tableHeader.push({
          text: 'Consumer Id',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'branch') {
        tableHeader.push({
          text: 'Branch',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'unionCouncil') {
        tableHeader.push({
          text: 'Union Council',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtConnectionType') {
        tableHeader.push({
          text: 'Connection Type',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtBusinessName') {
        tableHeader.push({
          text: 'Business Name',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'Category') {
        tableHeader.push({
          text: 'Category',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
    });
    this.newConnectionReportDtoList.push(tableHeader);
    console.log(this.newConnectionReportDtoList);
    this.newConnectionDataList.forEach((newConnectionDataList, i) => {
      debugger;
      let tableRows: any[] = [];
      this.selectionColumn.value.forEach((element) => {
        if (element === 'sr') {
          tableRows.push({ text: i + 1, alignment: 'right' });
        } else if (element === 'txtCustomerCode') {
          tableRows.push({
            text: newConnectionDataList.txtCustomerCode,
            alignment: 'left',
          });
        } else if (element === 'branch') {
          tableRows.push({
            text: newConnectionDataList.branch.name,
            alignment: 'left',
          });
        } else if (element === 'unionCouncil') {
          tableRows.push({
            text: newConnectionDataList.unionCouncil.name,
            alignment: 'left',
          });
        } else if (element === 'txtConnectionType') {
          tableRows.push({
            text: newConnectionDataList.txtConnectionType,
            alignment: 'left',
          });
        } else if (element === 'txtBusinessName') {
          tableRows.push({
            text: newConnectionDataList.txtBusinessName,
            alignment: 'left',
          });
        } else if (element === 'Category') {
          tableRows.push({
            text: newConnectionDataList.category.name,
            alignment: 'left',
          });
        }
      });
      this.newConnectionReportDtoList.push(tableRows);
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [
                {
                  image:
                    'data:image/png;base64,',
                },
                [
                  {
                    text: 'New Connections',
                    style: 'dateHeader',
                    alignment: 'center',
                    margin: [200, 60, 150, 0],
                    bold: true,
                    fontSize: 16,
                  },
                ],
                {
                  image:
                    'data:image/png;base64,',
                  width: 100,
                  height: 100,
                  alignment: 'right',
                  margin: [0, 0, -80, 0],
                },
              ],
            ],
          },
          layout: 'noBorders',
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.newConnectionReportDtoList,
          },
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10],
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5],
        },
        tableExample: {
          margin: [0, 5, 0, 15],
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
        },
      },
      defaultStyle: {
        // alignment: 'justify'
      },
    };

    pdfMake.createPdf(documentDefinition).download('New Connections List.pdf');
  }
  /* New Connection Column Selection */
  tableColumn: string[] = [
    'sr',
    'txtCustomerCode',
    'branch',
    'unionCouncil',
    'txtConnectionType',
    'txtBusinessName',
    'Category',
  ];
  selectionColumn = new FormControl([
    'sr',
    'txtCustomerCode',
    'branch',
    'unionCouncil',
    'txtConnectionType',
    'txtBusinessName',
    'Category',
  ]);
  selectingColumns() {
    debugger;
    if (this.selectionColumn.value.length > 0) {
      this.tableColumn.forEach((singleColumn) => {
        let display = '';
        if (singleColumn !== 'sr') {
          let columnSelected = this.selectionColumn.value.findIndex(
            (x) => x === singleColumn
          );
          if (columnSelected >= 0) {
            display = 'table-cell';
          } else {
            display = 'none';
          }
          const slides = document.getElementsByClassName(
            'mat-column-' + singleColumn
          );
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
  /* New Connection Column Selection */
  allConnectionReportDtoList: any[] = [];
  allConnectionDataList: any[] = [];
  pdfImplementationAll() {
    this.allConnectionDataList = this.selectStatusList;
    console.log('daownload data ');
    console.log(this.allConnectionDataList);
    debugger;
    this.allConnectionReportDtoList.push([
      { text: 'Sr#', bold: true },
      { text: 'Consumer Id', bold: true },
      { text: 'Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Connection Type', bold: true },
      { text: 'Business Name', bold: true },
      { text: 'Category', bold: true },
    ]);
    this.allConnectionDataList.forEach((allConnectionDataList, i) => {
      debugger;
      this.allConnectionReportDtoList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: allConnectionDataList.txtCustomerCode, alignment: 'left' },
        { text: allConnectionDataList.branch.name, alignment: 'left' },
        { text: allConnectionDataList.unionCouncil.name, alignment: 'left' },
        { text: allConnectionDataList.txtConnectionType, alignment: 'left' },
        { text: allConnectionDataList.txtBusinessName, alignment: 'right' },
        { text: allConnectionDataList.category, alignment: 'right' },
      ]);
    });
  }
  downloadAllPDF() {
    debugger;
    console.log(this.allSelectionColumn);
    this.allConnectionReportDtoList = [];
    let tableHeader: any[] = [];
    this.allSelectionColumn.value.forEach((element) => {
      if (element === 'sr') {
        tableHeader.push({
          text: 'Sr#',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtCustomerCode') {
        tableHeader.push({
          text: 'Consumer Id',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'branch') {
        tableHeader.push({
          text: 'Branch',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'unionCouncil') {
        tableHeader.push({
          text: 'Union Council',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtConnectionType') {
        tableHeader.push({
          text: 'Connection Type',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtBusinessName') {
        tableHeader.push({
          text: 'Business Name',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'Category') {
        tableHeader.push({
          text: 'Category',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
    });
    this.allConnectionReportDtoList.push(tableHeader);
    console.log(this.allConnectionReportDtoList);
    this.allConnectionDataList.forEach((allConnectionDataList, i) => {
      debugger;
      let tableRows: any[] = [];
      this.allSelectionColumn.value.forEach((element) => {
        if (element === 'sr') {
          tableRows.push({ text: i + 1, alignment: 'right' });
        } else if (element === 'txtCustomerCode') {
          tableRows.push({
            text: allConnectionDataList.txtCustomerCode,
            alignment: 'left',
          });
        } else if (element === 'branch') {
          tableRows.push({
            text: allConnectionDataList.branch.name,
            alignment: 'left',
          });
        } else if (element === 'unionCouncil') {
          tableRows.push({
            text: allConnectionDataList.unionCouncil.name,
            alignment: 'left',
          });
        } else if (element === 'txtConnectionType') {
          tableRows.push({
            text: allConnectionDataList.txtConnectionType,
            alignment: 'left',
          });
        } else if (element === 'txtBusinessName') {
          tableRows.push({
            text: allConnectionDataList.txtBusinessName,
            alignment: 'left',
          });
        } else if (element === 'Category') {
          tableRows.push({
            text: allConnectionDataList.category.name,
            alignment: 'left',
          });
        }
      });
      this.allConnectionReportDtoList.push(tableRows);
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [
                {
                  image:
                    'data:image/png;base64,',
                },
                [
                  {
                    text: 'All Connections',
                    style: 'dateHeader',
                    alignment: 'center',
                    margin: [200, 60, 150, 0],
                    bold: true,
                    fontSize: 16,
                  },
                ],
                {
                  image:
                    'data:image/png;base64,',
                  width: 100,
                  height: 100,
                  alignment: 'right',
                  margin: [0, 0, -80, 0],
                },
              ],
            ],
          },
          layout: 'noBorders',
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.allConnectionReportDtoList,
          },
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10],
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5],
        },
        tableExample: {
          margin: [0, 5, 0, 15],
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
        },
      },
      defaultStyle: {
        // alignment: 'justify'
      },
    };

    pdfMake.createPdf(documentDefinition).download('All Connections List.pdf');
  }
  allTableColumn: string[] = [
    'sr',
    'txtCustomerCode',
    'branch',
    'unionCouncil',
    'txtConnectionType',
    'txtBusinessName',
    'Category',
  ];
  allSelectionColumn = new FormControl([
    'sr',
    'txtCustomerCode',
    'branch',
    'unionCouncil',
    'txtConnectionType',
    'txtBusinessName',
    'Category',
  ]);
  allSelectingColumns() {
    debugger;
    if (this.allSelectionColumn.value.length > 0) {
      this.tableColumn.forEach((singleColumn) => {
        let display = '';
        if (singleColumn !== 'sr') {
          let columnSelected = this.allSelectionColumn.value.findIndex(
            (x) => x === singleColumn
          );
          if (columnSelected >= 0) {
            display = 'table-cell';
          } else {
            display = 'none';
          }
          const slides = document.getElementsByClassName(
            'mat-column-' + singleColumn
          );
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
function value(value: any) {
  throw new Error('Function not implemented.');
}
