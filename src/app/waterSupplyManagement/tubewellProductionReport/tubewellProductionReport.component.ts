import { Component, OnInit, ViewChild } from '@angular/core';
import {
  FormBuilder,
  FormControl,
  FormGroup,
  Validators,
} from '@angular/forms';
import { MatSort } from '@angular/material/sort';
import { MatPaginator } from '@angular/material/paginator';
import { MatTableDataSource } from '@angular/material/table';

import { MatDatepicker } from '@angular/material/datepicker';
import {
  MomentDateAdapter,
  MAT_MOMENT_DATE_ADAPTER_OPTIONS,
} from '@angular/material-moment-adapter';
import {
  DateAdapter,
  MAT_DATE_FORMATS,
  MAT_DATE_LOCALE,
} from '@angular/material/core';
import * as _moment from 'moment';
import { Moment } from 'moment';
import { MatDialog } from '@angular/material/dialog';
import { Router, ActivatedRoute } from '@angular/router';
import { RejectModalComponent } from '@app/modals/rejectModal.component';
import { EditWaterProductionModalComponent } from '@app/modals/editWaterProductionModal.component';
import { BillingService } from '@app/services/billing.service';
import { element } from 'protractor';
import { CrudService } from '@app/_services/crud.service';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { CustomFormatterPipe } from '@app/_helpers/custom-formatter.pipe';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
const moment = _moment;
export const MY_FORMATS = {
  parse: {
    dateInput: 'MMM-y',
  },
  display: {
    dateInput: 'MMM-y',
    monthYearLabel: 'MMM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MMMM YYYY',
  },
};

@Component({
  selector: 'app-dashboard',
  templateUrl: './tubewellProductionReport.component.html',
  providers: [
    {
      provide: DateAdapter,
      useClass: MomentDateAdapter,
      deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS],
    },

    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class TubewellProductionReportComponent implements OnInit {
  searchWaterRunningForm: FormGroup;
  selectStatusList: any[] = [];
  user = '';
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  totalMonthlyBillCalcu = 0;
  sumTotalRunningHour = 0;
  AvgDischargeperHR = 0;
  TotalProducedCubic = 0;
  TotalRunningHour = 0;
  avgDischarge = 0;
  totalproduced = 0;
  Total = 0;

  //  sumTotalRunningHour=0;
  totalRunning: String;
  form: any;
  searchFrom: any;

  constructor(
    private formBuilder: FormBuilder,
    private numberFormat: CustomFormatterPipe,
    private dateFormat: DateFormatPipe,
    public dialog: MatDialog,
    private shared: CrudService,
    private route: ActivatedRoute,
    private router: Router,
    private billingService: BillingService
  ) {}
  public myDates: any = {};
  @ViewChild('picker') datePickerElement = MatDatepicker;
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  // txtMonth = new FormControl(moment());
  // date2= new FormControl(moment());
  //  txtMonth = this.dateFormat.transform(this.date);

  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue = this.searchWaterRunningForm.controls.txtMonth.value;
    ctrlValue.year(normalizedYear.year());
    this.searchWaterRunningForm.controls.txtMonth.setValue(ctrlValue);
  }

  chosenMonthHandler(
    normalizedMonth: Moment,
    datepicker: MatDatepicker<Moment>
  ) {
    const ctrlValue = this.searchWaterRunningForm.controls.txtMonth.value;
    ctrlValue.month(normalizedMonth.month());
    this.searchWaterRunningForm.controls.txtMonth.setValue(ctrlValue);
    console.log('ghjsdkasd');
    console.log(this.searchWaterRunningForm.controls.txtMonth);
    datepicker.close();
  }
  clearDate() {
    this.searchWaterRunningForm.controls.txtMonth.reset('');
  }
  blnIsActive: boolean;
  // @ViewChild(MatPaginator) paginator: MatPaginator;
  // @ViewChild(MatSort) sort: MatSort;
  ngAfterViewInit() {}
  OnlyNumbersAllowed(event): boolean {
    const charcode = event.which ? event.which : event.keycode;
    if (charcode > 31 && (charcode < 48 || charcode > 57)) {
      return false;
    }
    return true;
  }
  clearAvgDischarge() {
    this.searchWaterRunningForm.controls.dateAvgDischarge.reset('');
  }
  clearPeriod() {
    this.searchWaterRunningForm.controls.txtMonth.reset('');
  }
  unionCouncilList: any[] = [];

  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));

    this.searchWaterRunningForm = this.formBuilder.group({
      serBranchId: [''],
      serLocationId: [''],
      txtItemType: ['TW'],
      txtMonth: [moment()],

      dteDateTo: [moment()],
      dteDatefrom: [moment()],
      txtProductCode: [''],
      txtProductName: [''],
      txtTubewellRefNo: [''],
    });
    this.getServerDateTime();
    this.getBranchList();
    this.searchWaterRunningForm.controls['serBranchId'].valueChanges.subscribe(
      (element) => {
        debugger;
        this.billingService.getBranchIdForRoutes(element).subscribe((data) => {
          this.ucList = data;
          this.getBranchId = [];
          for (let getBranch of this.ucList) {
            this.getBranchId.push(getBranch);
          }
          console.log('this.getBranchId');
          console.log(this.getBranchId);
        });
      }
    );
  }
  waterProductionColumn: string[] = [
    'txtBranchName',
    'txtLocationName',
    'txtTubewellRefNo',
    'txtMonth',
    'txtProductCode',
    'txtProductName',
    'numWaterProducedInUnit',
    'TotalRunningUnit',
    'txtAverageDischargePerUnit',
    'numWaterProducedInUnit',
    'numMonthlyBill',
    'blnIsMetered',
  ];
  waterSourceProduction = new MatTableDataSource();
  @ViewChild('waterSourceProductionSort', { static: true })
  waterSourceProductionSort: MatSort;
  @ViewChild('waterSourceProductionPaginator', { static: true })
  waterSourceProductionPaginator: MatPaginator;
  responseList: any[] = [];
  numTotalRunningHours = 0;
  getBranchList() {
    console.log(this.user);
    console.log('user id ' + this.user['serUserId']);
    debugger;
    this.billingService
      .getBranchService(this.user['serUserId'])
      .subscribe((response) => {
        debugger;
        console.log('getBranchService');
        console.log(response);
        this.branchList = response;
      });
  }
  onSubmit() {
    debugger;
    let dteDateTo = this.dateFormat.transform(
      this.searchWaterRunningForm.controls.dteDateTo.value
    );
    let dteDatefrom = this.dateFormat.transform(
      this.searchWaterRunningForm.controls.dteDatefrom.value
    );
    let serBranchId = this.searchWaterRunningForm.controls.serBranchId.value;
    let serLocationId =
      this.searchWaterRunningForm.controls.serLocationId.value;
    let txtItemType = this.searchWaterRunningForm.controls.txtItemType.value;
    let txtProductCode =
      this.searchWaterRunningForm.controls.txtProductCode.value;
    let txtProductName =
      this.searchWaterRunningForm.controls.txtProductName.value;
    let txtTubewellRefNo =
      this.searchWaterRunningForm.controls.txtTubewellRefNo.value;
    let txtMonth = this.dateFormat.transform(
      this.searchWaterRunningForm.controls.txtMonth.value
    );
    console.log(txtMonth);
    this.billingService
      .WaterTubeWellMonthlyReport(
        dteDateTo,
        dteDatefrom,
        txtItemType,
        txtProductCode,
        txtProductName,
        txtTubewellRefNo,
        txtMonth,
        serBranchId,
        serLocationId
      )
      .subscribe((response) => {
        debugger;
        this.selectStatusList = [];
        console.log(response);
        this.responseList = response['data'];

        // this.sumTotalRunningHour= 0;
        this.totalMonthlyBillCalcu = 0;
        this.sumTotalRunningHour = 0;
        this.AvgDischargeperHR = 0;
        this.TotalProducedCubic = 0;
        this.TotalRunningHour = 0;
        this.avgDischarge = 0;
        this.totalproduced = 0;
        this.Total = 0;
        this.responseList.forEach((element) => {
          debugger;
          if (
            element.numMonthlyBill != NaN &&
            element.numMonthlyBill != undefined
          ) {
            this.totalMonthlyBillCalcu =
              this.totalMonthlyBillCalcu + element.numMonthlyBill;
          }

          if (
            element.numWaterProducedInUnit != NaN &&
            element.numWaterProducedInUnit != undefined &&
            element.blnIsMetered == true
          ) {
            this.TotalProducedCubic =
              this.TotalProducedCubic + element.numWaterProducedInUnit;
          }

          if (
            element.numAvgDischargeVolume != NaN &&
            element.numAvgDischargeVolume != undefined
          ) {
            this.TotalRunningHour =
              this.TotalRunningHour + element.numAvgDischargeVolume;
          }

          if (
            element.numAvgDischargeVolume != NaN &&
            element.numAvgDischargeVolume != undefined &&
            element.blnIsMetered == false
          ) {
            this.AvgDischargeperHR =
              this.AvgDischargeperHR + element.numTotalRunningHours;
          }
          this.avgDischarge =
            (this.TotalProducedCubic + this.AvgDischargeperHR) /
            this.responseList.length;
          if (element.blnIsMetered == true) {
            if (
              element.numUnitConsumed != NaN &&
              element.numUnitConsumed != undefined
            ) {
              this.totalproduced = this.totalproduced + element.numUnitConsumed;
            }
          } else if (element.blnIsMetered == false) {
            this.Total = this.totalproduced + element.numWaterProducedInHour;
          }
          this.selectStatusList.push(element);
          this.waterSourceProduction = new MatTableDataSource(
            this.responseList
          );
          this.waterSourceProduction.sort = this.waterSourceProductionSort;
          this.waterSourceProduction.paginator =
            this.waterSourceProductionPaginator;
          this.pdfImplementation();
        });
      });
  }
  getServerDateTime() {
    this.billingService.getServerDatetimeService().subscribe((response) => {
      debugger;
      this.searchWaterRunningForm.controls.txtMonth = response['data'];
      console.log('response');
      console.log(this.searchWaterRunningForm.controls.txtMonth.value);
    });
  }
  handleYearSelected(normalizedYear: Moment) {
    console.log('normalizedYear: ', normalizedYear.toDate());
  }
  handleMonthSelected(normalizedMonth: Moment) {
    console.log('normalizedMonth: ', normalizedMonth.toDate());
  }
  viewData: any[] = [];
  viewdetail: any;
  onView(element) {
    debugger;
    this.viewData = element;
    console.log(this.viewData);
    this.shared.changeMessage(this.viewData);
    this.router.navigate([
      '/waterSupplyManagement/waterProduction/viewwaterProduction',
    ]);
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList = this.selectStatusList;
    this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'TubeWell Number', bold: true },
      { text: 'TubeWell Name', bold: true },
      // { text: 'TubeWell Name', bold: true },
      { text: 'Total Reference No', bold: true },
      // { text: 'Average Discharge per hour', bold: true },
      // { text: 'Water Produced In hour', bold: true },
      // { text: 'Total Running Units', bold: true },
      { text: 'Average Discharge per Hour', bold: true },
      { text: 'Total Produced cubic/Month', bold: true },
      { text: 'Monthly bill', bold: true },
      // { text: 'Functional', bold: true },
      { text: 'Metered', bold: true },
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      this.tubewellReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tubewellDataList.txtBranchName, alignment: 'left' },
        { text: tubewellDataList.txtLocationName, alignment: 'left' },
        { text: tubewellDataList.txtProductCode, alignment: 'left' },
        { text: tubewellDataList.txtProductName, alignment: 'left' },
        { text: tubewellDataList.txtTubewellRefNo, alignment: 'left' },
        { text: tubewellDataList.numTotalRunningHours, alignment: 'right' },
        {
          text: tubewellDataList.txtAverageDischargePerHour,
          alignment: 'right',
        },
        { text: tubewellDataList.numWaterProducedInHour, alignment: 'right' },
        { text: tubewellDataList.TotalRunningUnit, alignment: 'right' },
        {
          text: tubewellDataList.txtAverageDischargePerUnit,
          alignment: 'right',
        },
        { text: tubewellDataList.numWaterProducedInUnit, alignment: 'right' },
        { text: tubewellDataList.numMonthlyBill, alignment: 'right' },
        // { text: tubewellDataList.blnIsFunctional, alignment: 'right' },
        { text: tubewellDataList.Metered, alignment: 'right' },
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.tubewellReportDTOList = [];
    let tableHeader: any[] = [];
    this.selection.value.forEach((element) => {
      if (element === 'sr') {
        tableHeader.push({
          text: 'Sr#',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtBranchName') {
        tableHeader.push({
          text: 'Zone / Branch',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtLocationName') {
        tableHeader.push({
          text: 'Union Council',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtProductCode') {
        tableHeader.push({
          text: 'TubeWell Number',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtTubewellRefNo') {
        tableHeader.push({
          text: 'TubeWell Reference No',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
        // } else if (element === "numTotalRunningHours") {
        //   tableHeader.push({ text: 'Total Running Hours', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === 'txtProductName') {
        tableHeader.push({
          text: 'TubeWell Name',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
      // else if (element === "txtAverageDischargePerHour") {
      //   tableHeader.push({ text: 'Average Discharge m3/hour', bold: true, fillColor: '#dddddd', alignment: 'center' });
      // }
      // else if (element === "numWaterProducedInHour") {
      //   tableHeader.push({ text: 'Water Produced In hour', bold: true, fillColor: '#dddddd', alignment: 'center' });
      else if (element === 'TotalRunningUnit') {
        tableHeader.push({
          text: 'Total Running Hour',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtAverageDischargePerUnit') {
        tableHeader.push({
          text: 'Avg Discharge m3/hr',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numWaterProducedInUnit') {
        tableHeader.push({
          text: 'Total Produced Cubic/Month',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numMonthlyBill') {
        tableHeader.push({
          text: 'Monthly Bill',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
        // } else if (element === "blnIsFunctional") {
        //   tableHeader.push({ text: 'Functional', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === 'blnIsMetered') {
        tableHeader.push({
          text: 'Metered',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
    });
    this.tubewellReportDTOList.push(tableHeader);
    console.log(this.tubewellReportDTOList);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      let tableRows: any[] = [];
      this.selection.value.forEach((element) => {
        if (element === 'sr') {
          tableRows.push({ text: i + 1, alignment: 'right' });
        } else if (element === 'txtBranchName') {
          tableRows.push({
            text: tubewellDataList.txtBranchName,
            alignment: 'left',
          });
        } else if (element === 'txtLocationName') {
          tableRows.push({
            text: tubewellDataList.txtLocationName,
            alignment: 'left',
          });
        } else if (element === 'txtProductCode') {
          tableRows.push({
            text: tubewellDataList.txtProductCode,
            alignment: 'left',
          });
        } else if (element === 'txtProductName') {
          tableRows.push({
            text: tubewellDataList.txtProductName,
            alignment: 'left',
          });
          // } else if (element === "numTotalRunningHours") {
          //   tableRows.push({ text: tubewellDataList.numTotalRunningHours, alignment: 'left' });
          // } else if (element === "txtAverageDischargePerHour") {
          //   tableRows.push({ text: tubewellDataList.txtAverageDischargePerHour, alignment: 'left' });

          // } else if (element === "numWaterProducedInHour") {
          //   tableRows.push({ text: tubewellDataList.numWaterProducedInHour, alignment: 'left' });
        } else if (element === 'txtTubewellRefNo') {
          tableRows.push({
            text: tubewellDataList.txtTubewellRefNo,
            alignment: 'left',
          });
        } else if (element === 'TotalRunningUnit') {
          tableRows.push({
            text: tubewellDataList.numAvgDischargeVolume,
            alignment: 'left',
          });
        } else if (
          element === 'txtAverageDischargePerUnit' &&
          tubewellDataList.blnIsMetered == true
        ) {
          tableRows.push({
            text: tubewellDataList.numWaterProducedInUnit,
            alignment: 'left',
          });
        } else if (
          element === 'txtAverageDischargePerUnit' &&
          tubewellDataList.blnIsMetered == false
        ) {
          tableRows.push({
            text: tubewellDataList.numTotalRunningHours,
            alignment: 'left',
          });
        } else if (
          element === 'numWaterProducedInUnit' &&
          tubewellDataList.blnIsMetered == true
        ) {
          tableRows.push({
            text: tubewellDataList.numUnitConsumed,
            alignment: 'left',
          });
        } else if (
          element === 'numWaterProducedInUnit' &&
          tubewellDataList.blnIsMetered == false
        ) {
          tableRows.push({
            text: tubewellDataList.numWaterProducedInHour,
            alignment: 'left',
          });
        } else if (element === 'numMonthlyBill') {
          tableRows.push({
            text: tubewellDataList.numMonthlyBill,
            alignment: 'left',
          });
          // } else if (element === "blnIsFunctional") {
          //   tableRows.push({ text: tubewellDataList.blnIsFunctional, alignment: 'left' });
        } else if (element === 'blnIsMetered') {
          tableRows.push({
            text: tubewellDataList.blnIsMetered,
            alignment: 'left',
          });
        }
      });
      this.tubewellReportDTOList.push(tableRows);
    });
    debugger;
    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [
                {
                  image:
                    'data:image/png;base64,',
                },
                [
                  {
                    text: 'Water Production Report',
                    style: 'dateHeader',
                    alignment: 'center',
                    margin: [200, 60, 150, 0],
                    bold: true,
                    fontSize: 14,
                  },
                ],
                {
                  image:
                    'data:image/png;base64,',
                  width: 90,
                  height: 90,
                  alignment: 'right',
                  margin: [0, 0, -30, 0],
                },
              ],
            ],
          },
          layout: 'noBorders',
        },

        {
          text: [
            { text: 'Date: ', bold: true },
            this.dateFormat.transform(
              this.searchWaterRunningForm.controls.dteDateTo.value
            ),
            ' ',
            'To',
            ' ',
            this.dateFormat.transform(
              this.searchWaterRunningForm.controls.txtMonth.value
            ),
          ],
        },

        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.tubewellReportDTOList,
          },
        },

        {
          alignment: 'justify',

          columns: [
            {
              text: [
                { text: 'Total Running Hour: ', bold: true },
                this.TotalRunningHour,
              ],
            },
            {
              text: [
                { text: 'Avg Discharge m3/hr ', bold: true },
                this.numberFormat.transform(this.avgDischarge.toString()),
              ],
            },
          ],
        },
        {
          alignment: 'justify',

          columns: [
            {
              text: [{ text: ' ', bold: true }],
            },
          ],
        },
        {
          alignment: 'justify',

          columns: [
            {
              text: [
                { text: 'Total Produced cubic/Month: ', bold: true },
                this.Total,
              ],
            },
            {
              text: [
                { text: 'Monthly Electricity Bill: ', bold: true },
                this.totalMonthlyBillCalcu,
              ],
            },
          ],
        },
        {
          alignment: 'justify',

          columns: [
            {
              text: [{ text: ' ', bold: true }],
            },

            {
              text: [{ text: ' ', bold: true }],
            },
            {
              text: [{ text: ' ', bold: true }],
            },
          ],
        },
        {
          alignment: 'justify',

          columns: [
            {
              text: [
                { text: 'Printed Date: ', bold: true },
                this.dateFormat.transformDateTime(new Date()),
              ],
            },
            {
              text: [
                { text: 'Printed By: ', bold: true },
                this.user['txtUserName'],
              ],
            },
          ],
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10],
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5],
        },
        tableExample: {
          margin: [0, 5, 0, 15],
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
        },
      },
      defaultStyle: {
        // alignment: 'justify'
      },
    };
    pdfMake.createPdf(documentDefinition).download('Water Production List.pdf');
  }
  tableColumn: string[] = [
    'sr',
    'txtBranchName',
    'txtLocationName',
    'txtProductCode',
    'txtTubewellRefNo',
    'txtProductName',

    'numTotalRunningHours',
    'txtAverageDischargePerHour',
    'TotalRunningUnit',
    'numWaterProducedInHour',
    'txtAverageDischargePerUnit',
    'numWaterProducedInUnit',
    'numMonthlyBill',

    // "blnIsFunctional",
    'blnIsMetered',
  ];
  selection = new FormControl([
    'sr',
    'txtBranchName',
    'txtLocationName',
    'txtProductCode',
    'txtTubewellRefNo',
    'txtProductName',

    'numTotalRunningHours',
    'txtAverageDischargePerHour',
    'TotalRunningUnit',
    'numWaterProducedInHour',
    'txtAverageDischargePerUnit',
    'numWaterProducedInUnit',
    'numMonthlyBill',

    // "blnIsFunctional",
    'blnIsMetered',
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach((singleColumn) => {
        let display = '';
        if (singleColumn !== 'sr') {
          let columnSelected = this.selection.value.findIndex(
            (x) => x === singleColumn
          );
          if (columnSelected >= 0) {
            display = 'table-cell';
          } else {
            display = 'none';
          }
          const slides = document.getElementsByClassName(
            'mat-column-' + singleColumn
          );
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
