import { Component, OnInit, ViewChild, AfterViewInit } from '@angular/core';
import { MatTableDataSource } from '@angular/material/table';
import { MatPaginator } from '@angular/material/paginator';
import { FormBuilder, FormGroup, Validators, FormControl } from '@angular/forms';
import { MatSort } from '@angular/material/sort';
import { MatDialog } from '@angular/material/dialog';
import { RejectModalComponent } from '@app/modals/rejectModal.component';
import { Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { ConnectionRejectedComponent } from '@app/modals/connectionRejected.component';
import { ConnectionApprovedComponent } from '@app/modals/connectionAproved.component';
import { ApprovedModalComponent } from '@app/modals/approvedModal.component';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './updateConnectionList.component.html'
})
export class UpdateConnectionListComponent implements OnInit {
  step = 0;
  show = false;
  form: FormGroup;

  branchList: any[] = [];
  unionCouncilList: any[] = [];
  connectionTypeList: any[] = [];
  statusList: any[] = [];
  responseList: any[] = [];

  updateColumns: string[] = ['txtCustomerCode', 'branch', 'unionCouncil', 'txtBusinessName', 'txtConnectionType', 'connetionDate', 'Action'];
  updateListData = new MatTableDataSource();
  @ViewChild("updateSort", { static: true }) updateSort: MatSort;
  @ViewChild("updatePaginator", { static: true }) updatePaginator: MatPaginator;


  ngAfterViewInit() {
  }

  OnlyNumbersAllowed(event): boolean {

    const charcode = (event.which) ? event.which : event.keycode;

    if (charcode > 31 && (charcode < 48 || charcode > 57)) {
      return false;

    }

    return true;
  }
  clearConnectionDate() {
    this.form.controls.txtConnectionDate.reset('');
  }
  onSearch() {
    this.show = true;
  }
  constructor(private formBuilder: FormBuilder, public dialog: MatDialog, private router: Router, private billingService: BillingService) { }

  ngOnInit(): void {
    this.form = this.formBuilder.group({
      branch: [''],
      txtCustomerCode: [''],
      unionCouncil: [''],
      txtConnectionType: [''],
      dteCreatedDate: ['']
    })
    this.pdfImplementation();

    this.billingService.getAllCustomerTemplateService().subscribe(response => {
      debugger;
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];

      console.log(response["connectionTypeList"]);
      this.connectionTypeList = response["connectionTypeList"];
    })

    this.onGetAllCustomer();
  }

  onGetAllCustomer(){
    this.billingService.getAllDumpCustomerService().subscribe(response => {
      debugger;
      console.log(response);
      this.statusList = [];
        response.data.forEach(element => {
          if (element.txtStatus === "UPDATED") {
            this.statusList.push(element)
          }
        });
        this.updateListData = new MatTableDataSource(this.statusList);
        this.updateListData.sort = this.updateSort;
        this.updateListData.paginator = this.updatePaginator;
        this.pdfImplementation();
    })
  }

  onSubmit() {
    debugger;
    console.log(this.form.value)
    let formValue = this.form.value;
    formValue.branch = { "id": + this.form.controls.branch.value }
    formValue.unionCouncil = { "id": +this.form.controls.unionCouncil.value }
    console.log(this.form.value);
    let code = this.form.controls.txtCustomerCode.value;
    let brachID = this.form.controls.branch.value;
    let UcID = this.form.controls.unionCouncil.value;
    let connType = this.form.controls.txtConnectionType.value;
    let connDate = this.form.controls.dteCreatedDate.value;
    this.billingService.searchCustomerDumpListServiceByCode(code, brachID, UcID, connType, connDate).subscribe(response => {
      debugger;
      this.statusList = [];
      console.log("response");
      console.log(response);
      this.responseList = response["data"];
      this.responseList.forEach(element => {
        if (element.txtStatus === "UPDATED") {
          this.statusList.push(element)
        }
      });
      debugger;
      console.log(this.statusList.length);
      if (this.statusList.length == 0) {
        alert('No Record Found')
        this.statusList = [];
        this.updateListData = new MatTableDataSource(this.statusList);
      }
      else {
        this.updateListData = new MatTableDataSource(this.statusList);
        this.updateListData.sort = this.updateSort;
        this.updateListData.paginator = this.updatePaginator;
        this.pdfImplementation();
      }
    })
  }

  onReset(){
    this.onGetAllCustomer();
  }

  openApprovedDialog(element) {
    const document = this.dialog.open(ConnectionApprovedComponent, {
      data: {
        element,
        buttonText: {
          ok: 'Approve',
          cancel: 'Cancel'
        }
      }
    });
    document.afterClosed().subscribe(() => {
      debugger;
        console.log("openApprovedDialog closed");
        this.onGetAllCustomer();
        console.log("openApprovedDialog closed");
    });
  };

  openRejectDialog(element) {
    console.log(element)
    const document = this.dialog.open(ConnectionRejectedComponent, {
      data: {
        element,
        message: 'Are you sure want to reject?',
        buttonText: {
          ok: 'Yes',
          cancel: 'No'
        }
      }
    });
    document.afterClosed().subscribe(() => {
        console.log("openRejectDialog closed");
        this.onGetAllCustomer();
    });
  }
  updateConnectionList: any[] = [];
  updateConnectionDTOList: any[] = [];
  pdfImplementation() {    
    debugger;
    this.updateConnectionList = this.responseList;
    console.log("daownload data ")
    console.log(this.updateConnectionList)
    debugger;
    // 'txtCustomerCode', 'branch', 'unionCouncil', 'txtBusinessName', 'txtConnectionType', 'connetionDate',
    this.updateConnectionDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Consumer Id', bold: true },
      { text: 'Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Business Name', bold: true },
      { text: 'Connection Type', bold: true },      
      { text: 'Connetion Date', bold: true }     
    ]);
    this.updateConnectionList.forEach((updateConnectionList, i) => {
      debugger;
      this.updateConnectionDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: updateConnectionList.txtCustomerCode, alignment: 'left' },
        { text: updateConnectionList.branch.name, alignment: 'left' },
        { text: updateConnectionList.unionCouncil.name, alignment: 'left' },
        { text: updateConnectionList.txtBusinessName, alignment: 'right' },
        { text: updateConnectionList.txtConnectionType, alignment: 'left' },
        { text: updateConnectionList.connetionDate, alignment: 'right' }
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.updateConnectionDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCustomerCode") {
        tableHeader.push({ text: 'Consumer Id', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "branch") {
        tableHeader.push({ text: 'Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBusinessName") {
        tableHeader.push({ text: 'Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtConnectionType") {
        tableHeader.push({ text: 'Connection Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
      else if (element === "connetionDate") {
        tableHeader.push({ text: 'Connetion Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }      
    });
    this.updateConnectionDTOList.push(tableHeader);
    console.log(this.updateConnectionDTOList);
    this.updateConnectionList.forEach((updateConnectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtCustomerCode") {
          tableRows.push({ text: updateConnectionList.txtCustomerCode, alignment: 'left' });
        } else if (element === "branch") {
          tableRows.push({ text: updateConnectionList.branch.name, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: updateConnectionList.unionCouncil.name, alignment: 'left' });
        } else if (element === "txtBusinessName") {
          tableRows.push({ text: updateConnectionList.txtBusinessName, alignment: 'left' });
        }else if (element === "txtConnectionType") {
          tableRows.push({ text: updateConnectionList.txtConnectionType, alignment: 'left' });
        }  else if (element === "connetionDate") {
          tableRows.push({ text: updateConnectionList.connetionDate, alignment: 'left' });
        } 
      });
      this.updateConnectionDTOList.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Update Connection', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.updateConnectionDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Update Connection List.pdf');
  }

   // ['txtperformanceAreaName','txtperformanceAreaCode', 
    // 'numkpiCode', 'txtKpiName', 'numMinValue', 'numMaxValue', 
    // 'txtFormula', 'txtUom', 'txtCostCenter', 
  tableColumn: string[] = [
    "sr",
    "txtCustomerCode",
    "branch",
    "unionCouncil",
    "txtBusinessName",
    "txtConnectionType", 
    "connetionDate"
  ];
  selection = new FormControl([
    "sr",
    "txtCustomerCode",
    "branch",
    "unionCouncil",
    "txtBusinessName",
    "txtConnectionType", 
    "connetionDate"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }

}
