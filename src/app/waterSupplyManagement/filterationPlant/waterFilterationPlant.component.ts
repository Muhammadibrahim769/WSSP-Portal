...
export interface PeriodicElement {
  ...
  TubeWellName: string;
  NoOfConnection: number;
  AvgDischarge: number;
  Status: string;
  connection: string,
  AvgDischargePerUnit: number
}
const ELEMENT_DATA: PeriodicElement[] = [
  { TubeWellNumber: '106788', TubeWellName: 'ABC', NoOfConnection: 3, AvgDischarge: 1000, AvgDischargePerUnit: 700, Status: 'Functional', connection: 'Metered' },
  { TubeWellNumber: '58765', TubeWellName: 'XYZ', NoOfConnection: 4, AvgDischarge: 0, AvgDischargePerUnit: 700, Status: 'Functional', connection: 'Metered' },
  { TubeWellNumber: '35465', TubeWellName: 'KPK', NoOfConnection: 7, AvgDischarge: 1000, AvgDischargePerUnit: 0, Status: 'Functional', connection: 'Metered' },
]
@Component({
...
export class WaterFilterationPlantComponent implements AfterViewInit {
  ...
  operationBasicInfoColumns: string[] = ['txt_branch_name','txt_location_name','txtProductCodeF', 'txtProductNameF','txtMeterType','numOfConnections', 'blnIsFunctional', 'Actions'];
  ...
  constructor(private formBuilder: FormBuilder, private utilService: UtilsService,private changeDetectorRefs: ChangeDetectorRef, public dialog: MatDialog, private billingService: BillingService, private router: Router) { }

  unionCouncilList: any[] = [];
  searchFilterationData: any[] = [];
  isLoading = true;
  dataSource = null;
  ngOnInit(): void {
  ...
  onSearch() {
    ...
    this.billingService.getFiterationServiceByParams(blnIsFunctional,serBranchId, serLocationId, txtProductCode, txtProductName).subscribe(Response => {
      this.searchFilterationData = Response["data"];
      console.log("Response");
      console.log(Response);
  ...
  onGet(){
    this.billingService.getFilterationPlantoService().subscribe(response => {
  ...
  openEditModal(id: any) {
    const EditData = this.dialog.open(EditFilterationPlantModalComponent, {
      data: id
    })
  ...
  overheadDataList: any[] = [];
  overheadReportDTOList: any[] = [];
  pdfImplementation() {
   debugger;
    this.overheadDataList = this.dataSource;
    console.log("daownload data ")
    console.log(this.overheadDataList)
    debugger;
    this.overheadReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Filteration Number', bold: true },
      { text: 'Filteration Name', bold: true },     
      { text: 'Functional', bold: true },
      { text: 'Meter Type', bold: true },     
      { text: 'No of Connections', bold: true }          
    ]);
    this.overheadDataList.forEach((overheadDataList, i) => {
      debugger;
      this.overheadReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: overheadDataList.txtBranchName, alignment: 'left' },
        { text: overheadDataList.txtLocationName, alignment: 'left' },
        { text: overheadDataList.txtProductCode, alignment: 'left' },
        { text: overheadDataList.txtProductName, alignment: 'left' },       
        { text: overheadDataList.blnIsFunctional, alignment: 'right' },
        { text: overheadDataList.txtMeterType, alignment: 'left' },       
        { text: overheadDataList.numOfConnections, alignment: 'right' }
      ]);
    });
  }
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      if (element === "sr") {
        ...
        tableHeader.push({ text: 'Filteration Number', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtProductNameF") {
        tableHeader.push({ text: 'Filteration Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
    ...
    this.overheadReportDTOList.push(tableHeader);
    console.log(this.overheadReportDTOList);
    this.overheadDataList.forEach((overheadDataList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: overheadDataList.txtBranchName, alignment: 'left' });
        } else if (element === "txt_location_name") {
          tableRows.push({ text: overheadDataList.txtLocationName, alignment: 'left' });
        } else if (element === "txtProductCodeF") {
          tableRows.push({ text: overheadDataList.txtProductCode, alignment: 'left' });
        } else if (element === "txtProductNameF") {
          tableRows.push({ text: overheadDataList.txtProductName, alignment: 'left' });
        } else if (element === "txtMeterType") {
          tableRows.push({ text: overheadDataList.txtMeterType, alignment: 'left' });
        } else if (element === "numOfConnections") {
          tableRows.push({ text: overheadDataList.numOfConnections, alignment: 'left' });
        } else if (element === "blnIsFunctional") {
          tableRows.push({ text: overheadDataList.blnIsFunctional, alignment: 'left' });
        }
      });
      this.overheadReportDTOList.push(tableRows)
    });
    debugger;
      ...
      content: [
        {
          ...
          table: {
            ...
            body: [
              ...
              }, [{ text: 'Filteration Plant ', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
        ...
        },
        {
          ...
          table: {
            headerRows: 1,
            body: this.overheadReportDTOList
          }
        },
      ],
      styles: {
    ...
    pdfMake.createPdf(documentDefinition).download('Filteration List.pdf');
  } 
  tableColumn: string[] = [
    "sr",
    "txt_branch_name",
