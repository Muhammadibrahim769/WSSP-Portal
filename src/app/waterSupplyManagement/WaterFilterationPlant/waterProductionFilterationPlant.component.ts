import { Component, OnInit, ViewChild } from '@angular/core';
import {
  FormBuilder,
  FormControl,
  FormGroup,
  Validators,
} from '@angular/forms';
import { MatSort } from '@angular/material/sort';
import { MatPaginator } from '@angular/material/paginator';
import { MatTableDataSource } from '@angular/material/table';
import { MatDatepicker } from '@angular/material/datepicker';
import {
  MomentDateAdapter,
  MAT_MOMENT_DATE_ADAPTER_OPTIONS,
} from '@angular/material-moment-adapter';
import {
  DateAdapter,
  MAT_DATE_FORMATS,
  MAT_DATE_LOCALE,
} from '@angular/material/core';
import * as _moment from 'moment';
import { Moment } from 'moment';
import { MatDialog } from '@angular/material/dialog';
import { Router, ActivatedRoute } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { CrudService } from '@app/_services/crud.service';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { EditWaterFilterationPlantModalComponent } from '@app/modals/editWaterFilterationPlantModal.component';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
const moment = _moment;
export const MY_FORMATS = {
  parse: {
    dateInput: 'MMM-y',
  },
  display: {
    dateInput: 'MMM-y',
    monthYearLabel: 'MMM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MMMM YYYY',
  },
};

@Component({
  selector: 'app-dashboard',
  templateUrl: './waterProductionFilterationPlant.component.html',
  providers: [
    {
      provide: DateAdapter,
      useClass: MomentDateAdapter,
      deps: [MAT_DATE_LOCALE, MAT_MOMENT_DATE_ADAPTER_OPTIONS],
    },

    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class WaterProductionFilterationPlantComponent implements OnInit {
  searchWaterRunningForm: FormGroup;
  selectStatusList: any[] = [];
  user = '';
  ucList: any;
  getBranchId = [];
  getUClist = [];
  branchList: any;
  totalRunning: String;
  constructor(
    private formBuilder: FormBuilder,
    private dateFormat: DateFormatPipe,
    public dialog: MatDialog,
    private shared: CrudService,
    private route: ActivatedRoute,
    private router: Router,
    private billingService: BillingService
  ) {}
  public myDates: any = {};
  @ViewChild('picker') datePickerElement = MatDatepicker;
  step = 0;
  setStep(index: number) {
    this.step = index;
  }
  // txtMonth = new FormControl(moment());
  // date2= new FormControl(moment());
  //  txtMonth = this.dateFormat.transform(this.date);

  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue = this.searchWaterRunningForm.controls.txtMonth.value;
    ctrlValue.year(normalizedYear.year());
    this.searchWaterRunningForm.controls.txtMonth.setValue(ctrlValue);
  }

  chosenMonthHandler(
    normalizedMonth: Moment,
    datepicker: MatDatepicker<Moment>
  ) {
    const ctrlValue = this.searchWaterRunningForm.controls.txtMonth.value;
    ctrlValue.month(normalizedMonth.month());
    this.searchWaterRunningForm.controls.txtMonth.setValue(ctrlValue);
    console.log('ghjsdkasd');
    console.log(this.searchWaterRunningForm.controls.txtMonth);
    datepicker.close();
  }
  clearDate() {
    this.searchWaterRunningForm.controls.txtMonth.reset('');
  }
  blnIsActive: boolean;
  // @ViewChild(MatPaginator) paginator: MatPaginator;
  // @ViewChild(MatSort) sort: MatSort;
  ngAfterViewInit() {}
  OnlyNumbersAllowed(event): boolean {
    const charcode = event.which ? event.which : event.keycode;
    if (charcode > 31 && (charcode < 48 || charcode > 57)) {
      return false;
    }
    return true;
  }
  clearAvgDischarge() {
    this.searchWaterRunningForm.controls.dateAvgDischarge.reset('');
  }
  clearPeriod() {
    this.searchWaterRunningForm.controls.txtMonth.reset('');
  }
  unionCouncilList: any[] = [];

  ngOnInit() {
    this.user = JSON.parse(sessionStorage.getItem('user'));

    this.searchWaterRunningForm = this.formBuilder.group({
      serBranchId: [''],
      serLocationId: [''],
      txtProductCode: [''],
      txtProductName: [''],
      blnIsActive: [],
      txtAverageDischargePerHour: [''],
      blnIsMetered: [''],
      blnIsFunctional: [''],
      txtMonth: [moment()],
      blnStatus: [''],
      txtRemarks: [''],
      txtStatus: [],
      serProductId: [],
      txtItemType: ['FP'],
      dteDateFrom: [''],
      dteDateTo: [''],
    });
    this.getServerDateTime();
    this.getBranchList();
    this.searchWaterRunningForm.controls['serBranchId'].valueChanges.subscribe(
      (element) => {
        debugger;
        this.billingService.getBranchIdForRoutes(element).subscribe((data) => {
          this.ucList = data;
          this.getBranchId = [];
          for (let getBranch of this.ucList) {
            this.getBranchId.push(getBranch);
          }
          console.log('this.getBranchId');
          console.log(this.getBranchId);
        });
      }
    );
  }
  waterProductionColumn: string[] = [
    'txtBranchName',
    'txtLocationName',
    'txtProductCodeF',
    'txtProductNameF',
    'TotalRunningUnit',
    'txtAverageDischargePerUnit',
    'numWaterProducedInUnit',
    'numMonthlyBill',
    'blnIsFunctional',
    'blnIsMetered',
    'Actions',
  ];
  waterSourceProduction = new MatTableDataSource();
  @ViewChild('waterSourceProductionSort', { static: true })
  waterSourceProductionSort: MatSort;
  @ViewChild('waterSourceProductionPaginator', { static: true })
  waterSourceProductionPaginator: MatPaginator;
  responseList: any[] = [];
  getBranchList() {
    console.log(this.user);
    console.log('user id ' + this.user['serUserId']);
    debugger;
    this.billingService
      .getBranchService(this.user['serUserId'])
      .subscribe((response) => {
        debugger;
        console.log('getBranchService');
        console.log(response);
        this.branchList = response;
      });
  }
  onSubmit() {
    debugger;
    console.log(this.searchWaterRunningForm.value);
    let formValue = this.searchWaterRunningForm.value;
    // formValue.serUserId = {"id": + this.searchWaterRunningForm.controls.serUserId.value }
    // formValue.serLocationId = { "id": +this.searchWaterRunningForm.controls.serLocationId.value }
    let txtProductCode =
      this.searchWaterRunningForm.controls.txtProductCode.value;
    let serBranchId = this.searchWaterRunningForm.controls.serBranchId.value;
    let serLocationId =
      this.searchWaterRunningForm.controls.serLocationId.value;
    let txtProductName =
      this.searchWaterRunningForm.controls.txtProductName.value;

    let txtItemType = this.searchWaterRunningForm.controls.txtItemType.value;
    let txtMonth = this.dateFormat.transform(
      this.searchWaterRunningForm.controls.txtMonth.value
    );
    let dteDateTo = this.dateFormat.transformToStandardDateTime(
      this.searchWaterRunningForm.controls.dteDateTo.value
    );
    console.log(txtMonth);
    this.billingService
      .searchWaterProductionListServiceByCode(
        dteDateTo,
        txtItemType,
        txtMonth,
        txtProductCode,
        txtProductName,
        serBranchId,
        serLocationId
      )
      .subscribe((response) => {
        debugger;
        this.selectStatusList = [];
        console.log(response);
        this.responseList = response['data'];
        this.responseList.forEach((element) => {
          debugger;
          this.selectStatusList.push(element);
          this.waterSourceProduction = new MatTableDataSource(
            this.responseList
          );
          this.waterSourceProduction.sort = this.waterSourceProductionSort;
          this.waterSourceProduction.paginator =
            this.waterSourceProductionPaginator;
          this.pdfImplementation();
        });
      });
  }
  getServerDateTime() {
    this.billingService.getServerDatetimeService().subscribe((response) => {
      debugger;
      // this.searchWaterRunningForm.controls['txtMonth'].patchValue(response)
      // console.log(this.searchWaterRunningForm.controls.txtMonth.value)
      this.searchWaterRunningForm.controls.txtMonth = response['data'];
      console.log('response');
      console.log(this.searchWaterRunningForm.controls.txtMonth.value);
    });
  }
  // onGetWaterSourceAll()
  // {
  //   let txtMonth = this.dateFormat.transform(this.searchWaterRunningForm.controls.txtMonth.value);
  //   this.billingService.getAllWSProductionDataService(txtMonth).subscribe(response => {
  //     console.log("waterSourceProduction")
  //     console.log(response.data);
  //     debugger;
  //     this.waterSourceProduction = new MatTableDataSource(response.data);
  //     this.waterSourceProduction.sort = this.waterSourceProductionSort;
  //     this.waterSourceProduction.paginator = this.waterSourceProductionPaginator;
  //   })
  // }

  openEditModel(element) {
    console.log('form value');
    console.log(this.searchWaterRunningForm.value);
    const EditData = this.dialog.open(EditWaterFilterationPlantModalComponent, {
      data: element,
    });
    EditData.afterClosed().subscribe(() => {
      this.onSubmit();
    });
  }
  // UIForm = new FormGroup({
  //   txtMonth: new FormControl(moment()),
  // });
  handleYearSelected(normalizedYear: Moment) {
    console.log('normalizedYear: ', normalizedYear.toDate());
  }
  handleMonthSelected(normalizedMonth: Moment) {
    console.log('normalizedMonth: ', normalizedMonth.toDate());
  }
  viewData: any[] = [];
  viewdetail: any;
  onView(element) {
    debugger;
    this.viewData = element;
    console.log(this.viewData);
    this.shared.changeMessage(this.viewData);
    this.router.navigate([
      '/waterSupplyManagement/waterProduction/viewWaterProdFilterationPlant',
    ]);
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList = this.selectStatusList;
    this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Filteration Plant Number', bold: true },
      { text: 'Filteration Plant Name', bold: true },
      { text: 'Total Running Hours', bold: true },
      { text: 'Average Discharge per hour', bold: true },
      { text: 'Water Produced In hour', bold: true },
      { text: 'Total Running Units', bold: true },
      { text: 'Average Discharge per unit', bold: true },
      { text: 'Water Produced In unit', bold: true },
      { text: 'Monthly bill', bold: true },
      { text: 'Functional', bold: true },
      { text: 'Metered', bold: true },
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      this.tubewellReportDTOList.push([
        { text: (i + 1).toString(), alignment: 'right' },
        { text: tubewellDataList.txtBranchName, alignment: 'left' },
        { text: tubewellDataList.txtLocationName, alignment: 'left' },
        { text: tubewellDataList.txtProductCode, alignment: 'left' },
        { text: tubewellDataList.txtProductName, alignment: 'left' },
        { text: tubewellDataList.numTotalRunningHours, alignment: 'right' },
        {
          text: tubewellDataList.txtAverageDischargePerHour,
          alignment: 'right',
        },
        { text: tubewellDataList.numWaterProducedInHour, alignment: 'right' },
        { text: tubewellDataList.TotalRunningUnit, alignment: 'right' },
        {
          text: tubewellDataList.txtAverageDischargePerUnit,
          alignment: 'right',
        },
        { text: tubewellDataList.numWaterProducedInUnit, alignment: 'right' },
        { text: tubewellDataList.numMonthlyBill, alignment: 'right' },
        { text: tubewellDataList.blnIsFunctional, alignment: 'right' },
        { text: tubewellDataList.Metered, alignment: 'right' },
      ]);
    });
  }
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.tubewellReportDTOList = [];
    let tableHeader: any[] = [];
    this.selection.value.forEach((element) => {
      if (element === 'sr') {
        tableHeader.push({
          text: 'Sr#',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtBranchName') {
        tableHeader.push({
          text: 'Zone / Branch',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtLocationName') {
        tableHeader.push({
          text: 'Union Council',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtProductCodeF') {
        tableHeader.push({
          text: 'Filteration Plant Number',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtProductNameF') {
        tableHeader.push({
          text: 'Filteration Plant Name',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numTotalRunningHours') {
        tableHeader.push({
          text: 'Total Running Hours',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtAverageDischargePerHour') {
        tableHeader.push({
          text: 'Average Discharge per hour',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numWaterProducedInHour') {
        tableHeader.push({
          text: 'Water Produced In hour',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'TotalRunningUnit') {
        tableHeader.push({
          text: 'Total Running Units',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'txtAverageDischargePerUnit') {
        tableHeader.push({
          text: 'Average Discharge per unit',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numWaterProducedInUnit') {
        tableHeader.push({
          text: 'Water Produced In unit',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'numMonthlyBill') {
        tableHeader.push({
          text: 'Monthly Bill',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'blnIsFunctional') {
        tableHeader.push({
          text: 'Functional',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      } else if (element === 'blnIsMetered') {
        tableHeader.push({
          text: 'Metered',
          bold: true,
          fillColor: '#dddddd',
          alignment: 'center',
        });
      }
    });
    this.tubewellReportDTOList.push(tableHeader);
    console.log(this.tubewellReportDTOList);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      debugger;
      let tableRows: any[] = [];
      this.selection.value.forEach((element) => {
        if (element === 'sr') {
          tableRows.push({ text: i + 1, alignment: 'right' });
        } else if (element === 'txtBranchName') {
          tableRows.push({
            text: tubewellDataList.txtBranchName,
            alignment: 'left',
          });
        } else if (element === 'txtLocationName') {
          tableRows.push({
            text: tubewellDataList.txtLocationName,
            alignment: 'left',
          });
        } else if (element === 'txtProductCodeF') {
          tableRows.push({
            text: tubewellDataList.txtProductCode,
            alignment: 'left',
          });
        } else if (element === 'txtProductNameF') {
          tableRows.push({
            text: tubewellDataList.txtProductName,
            alignment: 'left',
          });
        } else if (element === 'numTotalRunningHours') {
          tableRows.push({
            text: tubewellDataList.numTotalRunningHours,
            alignment: 'left',
          });
        } else if (element === 'txtAverageDischargePerHour') {
          tableRows.push({
            text: tubewellDataList.txtAverageDischargePerHour,
            alignment: 'left',
          });
        } else if (element === 'numWaterProducedInHour') {
          tableRows.push({
            text: tubewellDataList.numWaterProducedInHour,
            alignment: 'left',
          });
        } else if (element === 'TotalRunningUnit') {
          tableRows.push({
            text: tubewellDataList.TotalRunningUnit,
            alignment: 'left',
          });
        } else if (element === 'txtAverageDischargePerUnit') {
          tableRows.push({
            text: tubewellDataList.txtAverageDischargePerUnit,
            alignment: 'left',
          });
        } else if (element === 'numWaterProducedInUnit') {
          tableRows.push({
            text: tubewellDataList.numWaterProducedInUnit,
            alignment: 'left',
          });
        } else if (element === 'numMonthlyBill') {
          tableRows.push({
            text: tubewellDataList.numMonthlyBill,
            alignment: 'left',
          });
        } else if (element === 'blnIsFunctional') {
          tableRows.push({
            text: tubewellDataList.blnIsFunctional,
            alignment: 'left',
          });
        } else if (element === 'blnIsMetered') {
          tableRows.push({
            text: tubewellDataList.blnIsMetered,
            alignment: 'left',
          });
        }
      });
      this.tubewellReportDTOList.push(tableRows);
    });
    debugger;
    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [
                {
                  image:
                    'data:image/png;base64,',
                },
                [
                  {
                    text: 'Water Production Report',
                    style: 'dateHeader',
                    alignment: 'center',
                    margin: [200, 60, 150, 0],
                    bold: true,
                    fontSize: 14,
                  },
                ],
                {
                  image:
                    'data:image/png;base64,',
                  width: 90,
                  height: 90,
                  alignment: 'right',
                  margin: [0, 0, -30, 0],
                },
              ],
            ],
          },
          layout: 'noBorders',
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.tubewellReportDTOList,
          },
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10],
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5],
        },
        tableExample: {
          margin: [0, 5, 0, 15],
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black',
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black',
        },
      },
      defaultStyle: {
        // alignment: 'justify'
      },
    };
    pdfMake.createPdf(documentDefinition).download('Water Production List.pdf');
  }
  tableColumn: string[] = [
    'sr',
    'txtBranchName',
    'txtLocationName',
    'txtProductCodeF',
    'txtProductNameF',

    'TotalRunningUnit',

    'txtAverageDischargePerUnit',
    'numWaterProducedInUnit',
    'numMonthlyBill',
    'blnIsFunctional',
    'blnIsMetered',
  ];
  selection = new FormControl([
    'sr',
    'txtBranchName',
    'txtLocationName',
    'txtProductCodeF',
    'txtProductNameF',

    'TotalRunningUnit',

    'txtAverageDischargePerUnit',
    'numWaterProducedInUnit',
    'numMonthlyBill',
    'blnIsFunctional',
    'blnIsMetered',
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach((singleColumn) => {
        let display = '';
        if (singleColumn !== 'sr') {
          let columnSelected = this.selection.value.findIndex(
            (x) => x === singleColumn
          );
          if (columnSelected >= 0) {
            display = 'table-cell';
          } else {
            display = 'none';
          }
          const slides = document.getElementsByClassName(
            'mat-column-' + singleColumn
          );
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
