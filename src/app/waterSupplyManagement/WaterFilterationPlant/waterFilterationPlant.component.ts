...
@Component({
  ...
  templateUrl: './waterFilterationPlant.component.html',
  providers: [
    {
      provide: DateAdapter,
...
export class WaterFilterationPlantComponent implements OnInit {

  searchWaterRunningForm: FormGroup;
  ...
  branchList: any;
  totalRunning: String;
  constructor(private formBuilder: FormBuilder, private dateFormat: DateFormatPipe, public dialog: MatDialog, private shared: CrudService, private route: ActivatedRoute, private router: Router, private billingService: BillingService) { }
  public myDates: any = {};
  ...
  // txtMonth = new FormControl(moment());
  // date2= new FormControl(moment());
  //  txtMonth = this.dateFormat.transform(this.date);

  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue = this.searchWaterRunningForm.controls.txtMonth.value;
    ctrlValue.year(normalizedYear.year());
    this.searchWaterRunningForm.controls.txtMonth.setValue(ctrlValue);
  }
  ...
  chosenMonthHandler(normalizedMonth: Moment, datepicker: MatDatepicker<Moment>) {
    ...
    console.log(this.searchWaterRunningForm.controls.txtMonth);
    datepicker.close();
  }
  clearDate() {
    this.searchWaterRunningForm.controls.txtMonth.reset('');
  }
  blnIsActive: boolean;
  ...
  clearAvgDischarge() {
    this.searchWaterRunningForm.controls.dateAvgDischarge.reset('');
  }
  clearPeriod() {
    this.searchWaterRunningForm.controls.txtMonth.reset('');
  }
  ...
  ngOnInit() {
    ...
    this.searchWaterRunningForm = this.formBuilder.group({
      serBranchId: [''],
      serLocationId: [''],
      txtProductCode: [''],
      txtProductName: [''],
    ...
    this.getBranchList();
    this.searchWaterRunningForm.controls['serBranchId'].valueChanges.subscribe(element => {
      debugger;
  ...
  waterProductionColumn: string[] = ['txtBranchName', 'txtLocationName', 'txtProductCodeF', 'txtProductNameF', 'TotalRunningUnit', 'txtAverageDischargePerUnit',
    'numWaterProducedInUnit', 'numMonthlyBill', 'blnIsFunctional', 'blnIsMetered', 'Actions'];
  waterSourceProduction = new MatTableDataSource();
  @ViewChild("waterSourceProductionSort", { static: true }) waterSourceProductionSort: MatSort;
  @ViewChild("waterSourceProductionPaginator", { static: true }) waterSourceProductionPaginator: MatPaginator;
  // selection = new SelectionModel<operationBasicInfo>(true, []);
  // isAllSelected() {
  //   const numSelected = this.selection.selected.length;
  //   const numRows = this.waterSourceProduction.data.length;
  //   return numSelected === numRows;
  // }
  // masterToggle() {
  //   this.isAllSelected() ?
  //       this.selection.clear() :
  //       this.waterSourceProduction.data.forEach(row => this.selection.select(row));
  // }
  // checkboxLabel(row?: operationBasicInfo): string {
  //   if (!row) {
  //     return `${this.isAllSelected() ? 'select' : 'deselect'} all`;
  //   }
  //   return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.TubeWellNo + 1}`;
  // }
  responseList: any[] = [];
  getBranchList() {
  ...
  onSubmit() {
    debugger;
    console.log(this.searchWaterRunningForm.value)
    let formValue = this.searchWaterRunningForm.value;
    // formValue.serUserId = {"id": + this.searchWaterRunningForm.controls.serUserId.value }
    // formValue.serLocationId = { "id": +this.searchWaterRunningForm.controls.serLocationId.value }
    let txtProductCode = this.searchWaterRunningForm.controls.txtProductCode.value;
    let serBranchId = this.searchWaterRunningForm.controls.serBranchId.value;
    let serLocationId = this.searchWaterRunningForm.controls.serLocationId.value;
    let txtProductName = this.searchWaterRunningForm.controls.txtProductName.value;

    let txtItemType = this.searchWaterRunningForm.controls.txtItemType.value
    let txtMonth = this.dateFormat.transform(this.searchWaterRunningForm.controls.txtMonth.value);
    let dteDateFrom = this.dateFormat.transformToStandardDateTime(this.searchWaterRunningForm.controls.dteDateFrom.value);
    let dteDateTo = this.dateFormat.transformToStandardDateTime(this.searchWaterRunningForm.controls.dteDateTo.value);
    console.log(txtMonth);
    this.billingService.searchWaterProductionListServiceByCode(dteDateFrom, dteDateTo, txtItemType, txtMonth, txtProductCode, txtProductName, serBranchId, serLocationId).subscribe(response => {
      debugger;
      ...
      this.responseList.forEach(element => {
        ...
        this.waterSourceProduction = new MatTableDataSource(this.responseList);
  ...
  getServerDateTime() {
    this.billingService.getServerDatetimeService().subscribe(response => {
      debugger;
      // this.searchWaterRunningForm.controls['txtMonth'].patchValue(response)
      // console.log(this.searchWaterRunningForm.controls.txtMonth.value)
      this.searchWaterRunningForm.controls.txtMonth = response["data"]
      console.log("response");
      console.log(this.searchWaterRunningForm.controls.txtMonth.value);
    })
  }
  ...
  // {
  //   let txtMonth = this.dateFormat.transform(this.searchWaterRunningForm.controls.txtMonth.value);
  //   this.billingService.getAllWSProductionDataService(txtMonth).subscribe(response => {
  //     console.log("waterSourceProduction")
  //     console.log(response.data);
  //     debugger;
  //     this.waterSourceProduction = new MatTableDataSource(response.data);
  //     this.waterSourceProduction.sort = this.waterSourceProductionSort;
  //     this.waterSourceProduction.paginator = this.waterSourceProductionPaginator;
  //   })
  // }
  ...
  openEditModel(element) {
    ...
    console.log(this.searchWaterRunningForm.value)
    const EditData = this.dialog.open(EditWaterFilterationPlantModalComponent, {
      data: element
  ...
  onView(element) {
    ...
    this.router.navigate(['/waterSupplyManagement/waterProduction/viewFilterationPlant']);
  }
  tubewellDataList: any[] = [];
  tubewellReportDTOList: any[] = [];
  pdfImplementation() {
    debugger;
    this.tubewellDataList = this.selectStatusList;
    this.tubewellReportDTOList.push([
      { text: 'Sr#', bold: true },
      { text: 'Zone / Branch', bold: true },
      { text: 'Union Council', bold: true },
      { text: 'Filteration Plant Number', bold: true },
      { text: 'Filteration Plant Name', bold: true },
      { text: 'Total Running Hours', bold: true },
      { text: 'Average Discharge per hour', bold: true },
      { text: 'Water Produced In hour', bold: true },
      { text: 'Total Running Units', bold: true },
      { text: 'Average Discharge per unit', bold: true },
      { text: 'Water Produced In unit', bold: true },
      { text: 'Monthly bill', bold: true },
      { text: 'Functional', bold: true },
      { text: 'Metered', bold: true }
    ]);
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      ...
      this.tubewellReportDTOList.push([
        ...
        { text: tubewellDataList.txtProductCode, alignment: 'left' },
        { text: tubewellDataList.txtProductName, alignment: 'left' },
        { text: tubewellDataList.numTotalRunningHours, alignment: 'right' },
        { text: tubewellDataList.txtAverageDischargePerHour, alignment: 'right' },
        { text: tubewellDataList.numWaterProducedInHour, alignment: 'right' },
        { text: tubewellDataList.TotalRunningUnit, alignment: 'right' },
        { text: tubewellDataList.txtAverageDischargePerUnit, alignment: 'right' },
        { text: tubewellDataList.numWaterProducedInUnit, alignment: 'right' },
        { text: tubewellDataList.numMonthlyBill, alignment: 'right' },
        { text: tubewellDataList.blnIsFunctional, alignment: 'right' },
        { text: tubewellDataList.Metered, alignment: 'right' }
      ]);
    });
  }
  downloadPDF() {
    ...
    this.selection.value.forEach(element => {
      ...
      } else if (element === "txtBranchName") {
        tableHeader.push({ text: 'Zone / Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtLocationName") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtProductCodeF") {
        tableHeader.push({ text: 'Filteration Plant Number', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtProductNameF") {
        tableHeader.push({ text: 'Filteration Plant Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numTotalRunningHours") {
        tableHeader.push({ text: 'Total Running Hours', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "txtAverageDischargePerHour") {
        tableHeader.push({ text: 'Average Discharge per hour', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numWaterProducedInHour") {
        tableHeader.push({ text: 'Water Produced In hour', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "TotalRunningUnit") {
        tableHeader.push({ text: 'Total Running Units', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtAverageDischargePerUnit") {
        tableHeader.push({ text: 'Average Discharge per unit', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "numWaterProducedInUnit") {
        tableHeader.push({ text: 'Water Produced In unit', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "numMonthlyBill") {
        tableHeader.push({ text: 'Monthly Bill', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "blnIsFunctional") {
        tableHeader.push({ text: 'Functional', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "blnIsMetered") {
        tableHeader.push({ text: 'Metered', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    ...
    this.tubewellDataList.forEach((tubewellDataList, i) => {
      ...
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: tubewellDataList.txtProductCode, alignment: 'left' });
        } else if (element === "txtProductNameF") {
          tableRows.push({ text: tubewellDataList.txtProductName, alignment: 'left' });
        } else if (element === "numTotalRunningHours") {
          tableRows.push({ text: tubewellDataList.numTotalRunningHours, alignment: 'left' });
        } else if (element === "txtAverageDischargePerHour") {
          tableRows.push({ text: tubewellDataList.txtAverageDischargePerHour, alignment: 'left' });
        } else if (element === "numWaterProducedInHour") {
          tableRows.push({ text: tubewellDataList.numWaterProducedInHour, alignment: 'left' });
        } else if (element === "TotalRunningUnit") {
          tableRows.push({ text: tubewellDataList.TotalRunningUnit, alignment: 'left' });
        } else if (element === "txtAverageDischargePerUnit") {
          tableRows.push({ text: tubewellDataList.txtAverageDischargePerUnit, alignment: 'left' });
        } else if (element === "numWaterProducedInUnit") {
          tableRows.push({ text: tubewellDataList.numWaterProducedInUnit, alignment: 'left' });
        } else if (element === "numMonthlyBill") {
          tableRows.push({ text: tubewellDataList.numMonthlyBill, alignment: 'left' });
        } else if (element === "blnIsFunctional") {
          tableRows.push({ text: tubewellDataList.blnIsFunctional, alignment: 'left' });
        } else if (element === "blnIsMetered") {
          tableRows.push({ text: tubewellDataList.blnIsMetered, alignment: 'left' });
        }
      });
    ...
    pdfMake.createPdf(documentDefinition).download('Water Production List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "txtBranchName",
