import { Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup, FormBuilder, FormControl } from '@angular/forms';
import { MatTableDataSource } from '@angular/material/table';
import { MatSort } from '@angular/material/sort';
import { MatPaginator } from '@angular/material/paginator';
import { BillingService } from '@app/services/billing.service';
import { MatDialog } from '@angular/material/dialog';
import { CrudService } from '@app/_services/crud.service';
import { Router } from '@angular/router';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { DateFormatPipe } from '@app/_helpers/date-format.pipe';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-dashboard',
  templateUrl: './scheduler.component.html'
})
export class SchedulerComponent implements OnInit {
  schedulerForm: FormGroup;
  step = 0;
  user: any = "";
  ucList: any;
  responseList: any[] = [];
  branchList: any;
  selectStatusList: any[] = [];
  getBranchId = [];
  unionCouncilList: any[] = [];
  constructor(private formBuilder: FormBuilder, private billingService: BillingService,
    public dialog: MatDialog, private shared: CrudService, private router: Router, private dateFormat: DateFormatPipe) { }
  schedulerColumns: string[] = ['branch', 'unionCouncil', 'dteScheduleDate', 'billFrom', 'billTo', 'referenceCodeStart', 'billLastDate', 'dteBillValidityDate', 'latePayment', 'Actions'];
  schedulerListData = new MatTableDataSource();
  @ViewChild("schedulerSort", { static: true }) schedulerSort: MatSort;
  @ViewChild("schedulerPaginator", { static: true }) schedulerPaginator: MatPaginator;

  clearScheduleDate() {
    this.schedulerForm.controls.scheduleDateTime.reset('');
  }
  clearBillLastDate() {
    this.schedulerForm.controls.dteBillLastDate.reset('');
  }
  OnlyNumbersAllowed(event): boolean {

    const charcode = (event.which) ? event.which : event.keycode;

    if (charcode > 31 && (charcode < 48 || charcode > 57)) {
      return false;
    }

    return true;
  }
  ngOnInit(): void {
    this.user = JSON.parse(sessionStorage.getItem('user'));
    this.schedulerForm = this.formBuilder.group({
      serUserId: [""],
      dteScheduleDate: [''],
      serBranchId: [''],
      serLocationId: [''],
      dteBillLastDate: [''],
      referenceCodeEnd: [],
      referenceCodeStart: []
    })
    this.getBranchList();
    
    this.schedulerForm.controls['serBranchId'].valueChanges.subscribe(element => {

      this.billingService.getBranchIdForRoutes(element).subscribe(data => {
        this.ucList = data;
        this.getBranchId = [];
        for (let getBranch of this.ucList) {
          this.getBranchId.push(getBranch);
        }

      })
    })
   
    this.getScheduleData();
  }
  ScheduleData: any = "";
  getScheduleData() {
    this.billingService.getAllSchedulerService().subscribe(response => {
      debugger;
      console.log(response);
      // this.ScheduleData = response["data"]
      this.schedulerListData = new MatTableDataSource(response.data);
      this.schedulerListData.sort = this.schedulerSort;
      this.schedulerListData.paginator = this.schedulerPaginator;
    })
  }
  getBranchList() {
    this.billingService.getBranchService(this.user["serUserId"]).subscribe(response => {

      this.branchList = response
    })
  }
  onReset(){
    this.schedulerForm.reset();
  }
  
  onSearch() {
    debugger
    let serBranchId = this.schedulerForm.controls.serBranchId.value;
    let serLocationId = this.schedulerForm.controls.serLocationId.value;
    // let txtDate = '';
    // if (this.schedulerForm.controls.txtDate.value != "") {
    //   txtDate = this.dateFormat.transformOperation(this.schedulerForm.controls.txtDate.value);
    // }


    this.billingService.getScheduleFindAllByParams(serBranchId,serLocationId).subscribe(response => {
      debugger
      console.log("response Schedule")
      console.log(response)
      this.ScheduleData = response["data"]
      debugger
      this.schedulerListData = new MatTableDataSource(response.data);
      this.schedulerListData.sort = this.schedulerSort;
      this.schedulerListData.paginator = this.schedulerPaginator;
    })
  }
  deleteScheduleRow(serScheduleId: any) {
    debugger;
    // deleteScheduleServiceById
    this.billingService.deleteScheduleServiceById(serScheduleId).subscribe(response => {
      debugger;
      console.log(response);
      this.getScheduleData();
    })
  }
  scheduleDTOList: any[] = [];

 
  // [ 'branch', 'unionCouncil','dteScheduleDate', 'referenceCodeStart', 'billLastDate','latePayment', 'Actions'];
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.scheduleDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "branch") {
        tableHeader.push({ text: 'Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "dteScheduleDate") {
        tableHeader.push({ text: 'Schedule Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "billFrom") {
        tableHeader.push({ text: 'Bill From', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "billTo") {
        tableHeader.push({ text: 'Bill To', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "referenceCodeStart") {
        tableHeader.push({ text: 'Reference Code Start', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "billLastDate") {
        tableHeader.push({ text: 'Bill Last Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
      else if (element === "latePayment") {
        tableHeader.push({ text: 'Late Payment', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }
    });
    this.scheduleDTOList.push(tableHeader);
    console.log(this.scheduleDTOList);
    this.ScheduleData.forEach((ScheduleData, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "branch") {
          tableRows.push({ text: ScheduleData.serBranchId, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: ScheduleData.serLocationId, alignment: 'left' });
        } else if (element === "dteScheduleDate") {
          tableRows.push({ text: this.dateFormat.transformDateTime(ScheduleData.dteScheduleDate.toString()), alignment: 'left' });
        } else if (element === "billFrom") {
          tableRows.push({ text: ScheduleData.billFrom, alignment: 'left' });
        } else if (element === "billTo") {
          tableRows.push({ text: ScheduleData.billTo, alignment: 'left' });
        } else if (element === "referenceCodeStart") {
          tableRows.push({ text: ScheduleData.referenceCodeStart, alignment: 'right' });
        } else if (element === "billLastDate") {
          tableRows.push({ text: this.dateFormat.transformDateTime(ScheduleData.dteBillLastDate.toString()), alignment: 'left' });
        } else if (element === "latePayment") {
          tableRows.push({ text: ScheduleData.numLatePayment, alignment: 'right' });
        }
      });
      this.scheduleDTOList.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Schedule Report', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.scheduleDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };
    pdfMake.createPdf(documentDefinition).download('Schedule Report List.pdf');
  }
  tableColumn: string[] = [
    "sr",
    "branch",
    "unionCouncil",
    "dteScheduleDate",
    "billFrom",
    "billTo",
    "referenceCodeStart",
    "billLastDate",
    "latePayment"
  ];
  selection = new FormControl([
    "sr",
    "branch",
    "unionCouncil",
    "dteScheduleDate",
    "billFrom",
    "billTo",
    "referenceCodeStart",
    "billLastDate",
    "latePayment"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
