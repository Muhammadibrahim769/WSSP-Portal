...
export class NewconnectionComponent implements OnInit {
  ...
  //     this.progressInfos[i] = { value: [i+1], documentName: selectedFiles[i].name };
  //     this.pppp.push(this.progressInfos[i])
  //     this.pppp.push({ value: [i+1], documentName: selectedFiles[i].name });
  //     this.billingService.uploadFileNewConnectionService(i,this.file,this.progressInfos[i].documentName).subscribe(response => {
  //       debugger;
