...
@Component({
  ...
  templateUrl: './viewAllConnectionInformation.component.html'
})
export class ViewAllConnectionInformationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.billingService.getCustomerListServiceByID(this.Id).subscribe(response => {
      ...
      this.documentDatasource = new MatTableDataSource(this.files);
      this.show = true;
    },
      (error) => {
        this.show = false;
        error = 'No Record Found. Only ACTIVE Connection record canbe viewed.';
        alert(error);
      })
