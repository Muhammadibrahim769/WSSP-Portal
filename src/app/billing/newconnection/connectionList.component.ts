...
export class ConnectionListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchConnectionForm.controls['branch'].valueChanges.subscribe(
      (element) => {
    ...
    this.searchConnectionForm.controls['unionCouncil'].valueChanges.subscribe(
      (element) => {
        this.billingService
  ...
  getProductList() {
    this.billingService
      ...
      .subscribe((response) => {
        ...
        this.serviceList = testArray.filter(
          (x: any) =>
            x['txtProductName'].includes('Water') ||
            x['txtProductName'].includes('Conser') ||
            x['txtProductName'].includes('Sew')
        );
      });
  }
  ...
  getAllConnection() {
    this.billingService.getAllCutomerService().subscribe((response) => {
      this.allConnectionDataList = response['data'];
      this.isLoading = false;
  ...
  onSubmit() {
    ...
    let txtProductName =
      this.searchConnectionForm.controls.txtProductName.value;
    this.isLoading = true;
    if (this.tabIndex == 0) {
      this.billingService
        ...
        .subscribe((response) => {
          ...
          if (response['count'] == 0) {
            ...
            this.responseList.forEach((element) => {
              ...
              this.newConnectionDataList = this.selectStatusList;
              this.isLoading = false;
  ...
  onReset() {
    this.searchConnectionForm.reset();
    // this.getNewConnection();
    // this.getAllConnection();
  }
  ...
  clearFrom() {
    this.searchConnectionForm.controls.connectionDateFrom.reset('');
  }
  clearTo() {
    this.searchConnectionForm.controls.connectionDateTo.reset('');
  }
  ...
  openOTBill(element) {
    this.router.navigate(['billing/connectionList/viewBill']);
    // this.billingService.getCurrentOTBillService(element.txtCustomerCode).subscribe(response=>{
    // })
  }
