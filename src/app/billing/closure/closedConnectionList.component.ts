import { Component, OnInit, ViewChild } from '@angular/core';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { Router } from '@angular/router';
import { ApproveCloseModalComponent } from '@app/modals/approveCloseModal.component';
import { RejectCloseModalComponent } from '@app/modals/rejectCloseModal.component';
import { BillingService } from '@app/services/billing.service';
import { CrudService } from '@app/_services/crud.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-closedConnectionList',
  templateUrl: './closedConnectionList.component.html'
})
export class ClosedConnectionListComponent implements OnInit {

  reportDtoList: any = [];
  customer: any = "";

  step = 0;
  searchClosureForm: FormGroup;

  branchList: any[] = [];
  unionCouncilList: any[] = [];
  connectionTypeList: any[] = [];

  basicInfo: any[] = [];
  responseList: any[] = [];


  closureColumns: string[] = ['txtCustomerCode', 'branch', 'unionCouncil', 'txtBusinessName', 'txtConnectionType', 'closingDate', 'Actions'];
  closureListData = new MatTableDataSource();
  @ViewChild("closureSort", { static: true }) closureSort: MatSort;
  @ViewChild("closurePaginator", { static: true }) closurePaginator: MatPaginator;


  constructor(private formBuilder: FormBuilder, private billingService: BillingService,
    public dialog: MatDialog, private shared: CrudService, private router: Router) { }
    
  ngOnInit(): void {
    this.searchClosureForm = this.formBuilder.group({
      txtCustomerCode: [''],
      branch: [''],
      unionCouncil: [''],
      txtConnectionType: [''],
      suspensionDate: ['']
    })

    this.getAllCustomers();
    this.billingService.getAllCustomerTemplateService().subscribe(response => {
      debugger;
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];

      console.log(response["connectionTypeList"]);
      this.connectionTypeList = response["connectionTypeList"];
    })

  }

  ngAfterViewInit() {
  }

  clearSuspenionDate() {
    this.searchClosureForm.controls.suspensionDate.reset('');
  }

  getAllCustomers(){
    this.billingService.getAllCustomerService().subscribe(
      response => {
        debugger;
        this.basicInfo = [];
        console.log("getAllCustomerService")
        console.log(response.data)
        response.data.forEach(element => {
          debugger;
          if (element.txtStatus === "CLOSED") {
            this.basicInfo.push(element)
          }
        });
        this.closureListData = new MatTableDataSource(this.basicInfo);
        this.closureListData.sort = this.closureSort;
        this.closureListData.paginator = this.closurePaginator;
        this.closeConnectionList =this.basicInfo;
      })

  }

  onSubmit() {
    debugger;
    console.log(this.searchClosureForm.value)
    let formValue = this.searchClosureForm.value;
    formValue.branch = { "id": + this.searchClosureForm.controls.branch.value }
    formValue.unionCouncil = { "id": +this.searchClosureForm.controls.unionCouncil.value }
    let code = this.searchClosureForm.controls.txtCustomerCode.value;
    let brachID = this.searchClosureForm.controls.branch.value;
    let UcID = this.searchClosureForm.controls.unionCouncil.value;
    let connType = this.searchClosureForm.controls.txtConnectionType.value;
    let suspendDate = this.searchClosureForm.controls.suspensionDate.value;

    this.billingService.searchSuspendCustomerListServiceByCode(code, brachID, UcID, connType, suspendDate).subscribe(response => {
      debugger;
      if(response["count"]==0){
        alert('No Record Found');
        this.basicInfo = [];
        this.closureListData = new MatTableDataSource(this.basicInfo);
      }
      else{
        console.log(response);
        this.basicInfo = [];
        this.responseList = response["data"];
        this.responseList.forEach(element => {
          if (element.txtStatus === "CLOSED") {
            this.basicInfo.push(element)
          }
          this.closureListData = new MatTableDataSource(this.basicInfo);
          this.closureListData.sort = this.closureSort;
          this.closureListData.paginator = this.closurePaginator;
          this.closeConnectionList =this.basicInfo;
        })
        if(this.basicInfo.length==0){
          this.basicInfo = [];
          this.closureListData = new MatTableDataSource(this.basicInfo);
          alert('No Record Found');
        }
      }
    })
  }

  onReset(){
    this.searchClosureForm.reset();
    // this.getAllCustomers(); --> Previous Code

  }

  onView(element){
    debugger;
    this.shared.changeMessage(element);
    this.router.navigate(['billing/viewCloseConnection']);
  }

  OnApprovedClose(element){
    console.log(element)
    const document = this.dialog.open(ApproveCloseModalComponent, {
      data: {
        element,
        message: 'Are you sure want to close?',
        buttonText: {
          ok: 'Yes',
          cancel: 'No'
        }
      }
    });
    document.afterClosed().subscribe(() => {
        console.log("ApproveCloseModalComponent closed");
        this.getAllCustomers();
    });
  }

  OnRejectClose(element){
    console.log(element)
    const document = this.dialog.open(RejectCloseModalComponent, {
      data: {
        element,
        message: 'Are you sure want to reject?',
        buttonText: {
          ok: 'Yes',
          cancel: 'No'
        }
      }
    });
    document.afterClosed().subscribe(() => {
        console.log("RejectCloseModalComponent closed");
        this.getAllCustomers();
    });
  }
  closeConnectionList: any = "";
  closeConnectionDTOList: any[] = [];
 
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.closeConnectionDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCustomerCode") {
        tableHeader.push({ text: 'Consumer Id', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "branch") {
        tableHeader.push({ text: 'Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBusinessName") {
        tableHeader.push({ text: 'Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtConnectionType") {
        tableHeader.push({ text: 'Connection Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
      else if (element === "suspensionDate") {
        tableHeader.push({ text: 'Closing Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }      
    });
    this.closeConnectionDTOList.push(tableHeader);
    console.log(this.closeConnectionDTOList);
    this.closeConnectionList.forEach((closeConnectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtCustomerCode") {
          tableRows.push({ text: closeConnectionList.txtCustomerCode, alignment: 'left' });
        } else if (element === "branch") {
          tableRows.push({ text: closeConnectionList.branch.name, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: closeConnectionList.unionCouncil.name, alignment: 'left' });
        } else if (element === "txtBusinessName") {
          tableRows.push({ text: closeConnectionList.txtBusinessName, alignment: 'left' });
        }else if (element === "txtConnectionType") {
          tableRows.push({ text: closeConnectionList.txtConnectionType, alignment: 'left' });
        }  else if (element === "suspensionDate") {
          tableRows.push({ text: closeConnectionList.suspensionDate, alignment: 'left' });
        } 
      });
      this.closeConnectionDTOList.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Closed Connection', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -80, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.closeConnectionDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Closed Connection List.pdf');
  }

   // ['txtperformanceAreaName','txtperformanceAreaCode', 
    // 'numkpiCode', 'txtKpiName', 'numMinValue', 'numMaxValue', 
    // 'txtFormula', 'txtUom', 'txtCostCenter', 
  tableColumn: string[] = [
    "sr",
    "txtCustomerCode",
    "branch",
    "unionCouncil",
    "txtBusinessName",
    "txtConnectionType", 
    "suspensionDate"
  ];
  selection = new FormControl([
    "sr",
    "txtCustomerCode",
    "branch",
    "unionCouncil",
    "txtBusinessName",
    "txtConnectionType", 
    "suspensionDate"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
