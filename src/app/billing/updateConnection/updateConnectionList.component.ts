...
export class UpdateConnectionListComponent implements OnInit {
  ...
  updateColumns: string[] = ['txtCustomerCode', 'branch', 'unionCouncil', 'txtBusinessName', 'txtConnectionType', 'dteModifiedDate', 'Action'];
  ...
  onGetAllCustomer(){
    this.billingService.getAllDumpCustomerService().subscribe(response => {
      ...
      this.statusList = [];
        ...
        this.updateConnectionList = this.statusList;
    ...
    this.updateConnectionList.forEach((updateConnectionList, i) => {
      ...
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: this.dateFormat.transformOperation(updateConnectionList.dteModifiedDate.toString()), alignment: 'left' });
        } 
      });
