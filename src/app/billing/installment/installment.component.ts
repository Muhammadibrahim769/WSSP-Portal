import { Component, OnInit, ViewChild } from '@angular/core';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { BillingService } from '@app/services/billing.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
export interface PeriodicElement {
  billNo: number;
  billPeriod: string;
  totalBillAmount: number;
  consumerId: number;
  installmentAmount: number;
  totalNoOfInstallments:number;
  
}

const Table_DATA: PeriodicElement[] = [
  {consumerId: 1,billPeriod: 'Jan-20 To Feb-20', billNo: 12, totalBillAmount: 1700, installmentAmount: 1000,totalNoOfInstallments:6},
  {consumerId: 2, billPeriod: 'Jan-20 To Feb-20', billNo: 22, totalBillAmount: 5453, installmentAmount:4000 ,totalNoOfInstallments:2},
  {consumerId: 3,billPeriod: 'Jan-20 To Feb-20', billNo: 55, totalBillAmount: 6443, installmentAmount: 4500, totalNoOfInstallments:6 },
  {consumerId:22,billPeriod: 'Jan-20 To Feb-20', billNo: 5, totalBillAmount: 933, installmentAmount:700 ,totalNoOfInstallments:6 },
  {consumerId: 42,billPeriod: 'Jan-20 To Feb-20', billNo: 33, totalBillAmount: 1320, installmentAmount: 900, totalNoOfInstallments:4},
  {consumerId: 32,billPeriod: 'Jan-20 To Feb-20', billNo: 23, totalBillAmount: 1422, installmentAmount: 1000, totalNoOfInstallments:4},
  {consumerId: 322,billPeriod: 'Mar-20 To Apr-20', billNo: 256, totalBillAmount: 1224, installmentAmount:200,totalNoOfInstallments:4},
  {consumerId: 555,billPeriod: 'Mar-20 To Apr-20', billNo: 67, totalBillAmount: 1365, installmentAmount: 1000,totalNoOfInstallments:7 },
  {consumerId:11,billPeriod: 'Nov-20 To Dec-20', billNo: 76, totalBillAmount: 1878, installmentAmount:1500 , totalNoOfInstallments:7},
  {consumerId: 65,billPeriod: 'Nov-20 To Dec-20', billNo: 88, totalBillAmount: 250, installmentAmount: 190,totalNoOfInstallments:7},
];
@Component({
  selector: 'app-dashboard',
  templateUrl: './installment.component.html'
})
export class InstallmentComponent implements OnInit {

  branchList:any [] = [];
  unionCouncilList:any [] = [];

  step = 0;
  billInstallmentForm:FormGroup;
  displayedColumns: string[] = [ 'consumerId','billNo', 'billPeriod', 'totalBillAmount','totalNoOfInstallments', 'installmentAmount','Actions'];
  dataSource = new MatTableDataSource<PeriodicElement>(Table_DATA);
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  ngAfterViewInit() {
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
  }
   constructor(private formBuilder: FormBuilder, private billingService: BillingService) { }
   ngOnInit(): void {
    this.billInstallmentForm = this.formBuilder.group({
      txtBranch: [''],     
      txtUnionCouncil: [''],
      numBillNo:[''],
      serConsumerId: [''],
      txtBillPeriod: [''],
      numTotalBillAmount: [''],
      numTotalNoOfInstallments:[''],
      numInstallmentAmount:['']
    })

    this.billingService.getAllCustomerTemplateService().subscribe( response=>{
      debugger;
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];

    })
    this.pdfImplementation();
   }
   billAdjustmentList: any[] = [];
   billAdjustmentDTOList: any[] = [];
   pdfImplementation() {    
     debugger;
     this.billAdjustmentList = this.dataSource["data"];
     console.log("daownload data ")
     console.log(this.billAdjustmentList)
     debugger;
    
     this.billAdjustmentDTOList.push([
       { text: 'Sr#', bold: true },
       { text: 'Consumer Id', bold: true },
       { text: 'Bill No', bold: true },
       { text: 'Bill Period', bold: true },
       { text: 'Total No of Installments', bold: true },
       { text: 'Total Bill Amount', bold: true },      
       { text: 'Installment Amount', bold: true }     
     ]);
     this.billAdjustmentList.forEach((billAdjustmentList, i) => {
       debugger;
       this.billAdjustmentDTOList.push([
         { text: (i + 1).toString(), alignment: 'right' },
         { text: billAdjustmentList.consumerId, alignment: 'left' },
         { text: billAdjustmentList.billNo, alignment: 'left' },
         { text: billAdjustmentList.billPeriod, alignment: 'left' },
         { text: billAdjustmentList.totalNoOfInstallments, alignment: 'right' },
         { text: billAdjustmentList.totalBillAmount, alignment: 'left' },
         { text: billAdjustmentList.installmentAmount, alignment: 'right' }
       ]);
     });
   }
   downloadPDF() {
     debugger;
     console.log(this.selection);
     this.billAdjustmentDTOList = [];
     let tableHeader: any[] = []
     this.selection.value.forEach(element => {
       if (element === "sr") {
         tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "consumerId") {
         tableHeader.push({ text: 'Consumer Id', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "billNo") {
         tableHeader.push({ text: 'Bill No', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "billPeriod") {
         tableHeader.push({ text: 'Bill Period', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "totalNoOfInstallments") {
         tableHeader.push({ text: 'Bill To', bold: true, fillColor: '#dddddd', alignment: 'center' });
       }else if (element === "totalBillAmount") {
         tableHeader.push({ text: 'Total Bill Amount', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } 
       else if (element === "installmentAmount") {
         tableHeader.push({ text: 'Installment Amount', bold: true, fillColor: '#dddddd', alignment: 'center' });
       }      
     });
     this.billAdjustmentDTOList.push(tableHeader);
     console.log(this.billAdjustmentDTOList);
     this.billAdjustmentList.forEach((billAdjustmentList, i) => {
       debugger;
       let tableRows: any[] = []
       this.selection.value.forEach(element => {
         if (element === "sr") {
           tableRows.push({ text: (i + 1), alignment: 'right' });
         } else if (element === "consumerId") {
           tableRows.push({ text: billAdjustmentList.consumerId, alignment: 'left' });
         } else if (element === "billNo") {
           tableRows.push({ text: billAdjustmentList.billNo, alignment: 'left' });
         } else if (element === "billPeriod") {
           tableRows.push({ text: billAdjustmentList.billPeriod, alignment: 'left' });
         } else if (element === "totalNoOfInstallments") {
           tableRows.push({ text: billAdjustmentList.totalNoOfInstallments, alignment: 'left' });
         }else if (element === "totalBillAmount") {
           tableRows.push({ text: billAdjustmentList.totalBillAmount, alignment: 'left' });
         }  else if (element === "installmentAmount") {
           tableRows.push({ text: billAdjustmentList.installmentAmount, alignment: 'left' });
         } 
       });
       this.billAdjustmentDTOList.push(tableRows)
     });
     debugger;
 
     const documentDefinition = {
       pageOrientation: 'landscape',
       content: [
         {
           style: 'tableExample',
           table: {
             headerRows: 1,
             // widths: [200, 170, 200, 170],
             body: [
               [{
                 image: 'data:image/png;base64,',
               }, [{ text: 'Bill Installment', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
               {
                 image: 'data:image/png;base64,', width: 100,
                 height: 100,
                 alignment: 'right', margin: [0, 0, -80, 0],
               }],
             ]
           },
           layout: 'noBorders'
         },
         {
           style: 'tableExample',
           table: {
             headerRows: 1,
             body: this.billAdjustmentDTOList
           }
         },
       ],
       styles: {
         header: {
           fontSize: 18,
           bold: true,
           margin: [0, 0, 0, 10]
         },
         subheader: {
           fontSize: 16,
           bold: true,
           margin: [0, 10, 0, 5]
         },
         tableExample: {
           margin: [0, 5, 0, 15]
         },
         tableHeader: {
           bold: true,
           fontSize: 10,
           color: 'black'
         },
         tableHeaderValue: {
           fontSize: 10,
           color: 'black'
         }
       },
       defaultStyle: {
         // alignment: 'justify'
       }
     };
 
     pdfMake.createPdf(documentDefinition).download('Bill Installment List.pdf');
   }
 

   tableColumn: string[] = [
     "sr",
     "consumerId",
     "billNo",
     "billPeriod",
     "totalNoOfInstallments",
     "totalBillAmount", 
     "installmentAmount"
   ];
   selection = new FormControl([
     "sr",
     "consumerId",
     "billNo",
     "billPeriod",
     "totalNoOfInstallments",
     "totalBillAmount", 
     "installmentAmount"
   ]);
   selectingColumns() {
     debugger;
     if (this.selection.value.length > 0) {
       this.tableColumn.forEach(singleColumn => {
         let display = "";
         if (singleColumn !== "sr") {
           let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
           if (columnSelected >= 0) {
             display = "table-cell";
           } else {
             display = "none";
           }
           const slides = document.getElementsByClassName('mat-column-' + singleColumn);
           for (let i = 0; i < slides.length; i++) {
             const slide = slides[i] as HTMLElement;
             slide.style.display = display;
           }
         }
       });
     }
   }

}
