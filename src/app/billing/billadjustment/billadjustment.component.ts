import { Component, OnInit, ViewChild } from '@angular/core';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { BillingService } from '@app/services/billing.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-dashboard',
  templateUrl: './billadjustment.component.html'
})
export class BilladjustmentComponent implements OnInit {

  branchList:any [] = [];
  unionCouncilList:any [] = [];
  statusList: any [] = [];
  testList: any [] = [];
  billAdjustmentData:any="";
  step = 0;
  billAdjustmentList: any[] = [];
  billAdjustmentDTOList: any[] = [];
  billAdjustmentForm:FormGroup;
  billAdjustmentColumns: string[] = ['txtCustomerCode', 'txtBillNo', 'dteBillFrom', 'dteBillTo', 'numTotalAmount', 'numAdjustment', 'Actions'];
  
  billAdjustmentListData = new MatTableDataSource();
  @ViewChild("billAdjustmentSort", { static: true }) billAdjustmentSort: MatSort;
  @ViewChild("billAdjustmentPaginator", { static: true }) billAdjustmentPaginator: MatPaginator;

  ngAfterViewInit() {
  }
   constructor(private formBuilder: FormBuilder, private billingService: BillingService) { }

   ngOnInit(): void {
    this.billAdjustmentForm = this.formBuilder.group({
      txtBranch: [''],     
      txtUnionCouncil: [''],
      numBillNo:[''],
      serConsumerId: [''],
      txtBillPeriod: [''],
      numTotalBillAmount: [''],
      numTotalAdjustmentAmount:['']
    })
    this.billingService.getAllCustomerTemplateService().subscribe( response=>{
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];
    })
    this.getBillAdjustments();
   } 

   getBillAdjustments(){
    this.billingService.getAllAdjustmentsService().subscribe( response =>{
      debugger;
      console.log(response);
      this.billAdjustmentData = response;
      let data = this.billAdjustmentData.filter(x=>(x["txtBillType"] === "ADJ"));
      this.billAdjustmentListData = new MatTableDataSource(data);
      this.billAdjustmentListData.sort = this.billAdjustmentSort;
      this.billAdjustmentListData.paginator = this.billAdjustmentPaginator;
      // this.pdfImplementation();
    })
   }

  onReset(){
    this.billAdjustmentForm.reset();
  }

   pdfImplementation() {    
     debugger;
     this.billAdjustmentList = this.statusList;
     console.log("daownload data ")
     console.log(this.billAdjustmentList)
     debugger;
    
     this.billAdjustmentDTOList.push([
       { text: 'Sr#', bold: true },
       { text: 'Consumer Code', bold: true },
       { text: 'Bill No', bold: true },
       { text: 'Bill From', bold: true },
       { text: 'Bill To', bold: true },
       { text: 'Total Bill Amount', bold: true },      
       { text: 'Adjustment Amount', bold: true }     
     ]);
     this.billAdjustmentList.forEach((billAdjustmentList, i) => {
       debugger;
       this.billAdjustmentDTOList.push([
         { text: (i + 1).toString(), alignment: 'right' },
         { text: billAdjustmentList.txtCustomerCode, alignment: 'left' },
         { text: billAdjustmentList.txtBillNo, alignment: 'left' },
         { text: billAdjustmentList.dteBillFrom, alignment: 'left' },
         { text: billAdjustmentList.dteBillTo, alignment: 'right' },
         { text: billAdjustmentList.numTotalAmount, alignment: 'left' },
         { text: billAdjustmentList.numAdjustment, alignment: 'right' }
       ]);
     });
   }
   
  
   downloadPDF() {
     debugger;
     console.log(this.selection);
     this.billAdjustmentDTOList = [];
     let tableHeader: any[] = []
     this.selection.value.forEach(element => {
       if (element === "sr") {
         tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "txtCustomerCode") {
         tableHeader.push({ text: 'Consumer Code', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "txtBillNo") {
         tableHeader.push({ text: 'Bill No', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "dteBillFrom") {
         tableHeader.push({ text: 'Bill From', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } else if (element === "dteBillTo") {
         tableHeader.push({ text: 'Bill To', bold: true, fillColor: '#dddddd', alignment: 'center' });
       }else if (element === "numTotalAmount") {
         tableHeader.push({ text: 'Total Bill Amount', bold: true, fillColor: '#dddddd', alignment: 'center' });
       } 
       else if (element === "numAdjustment") {
         tableHeader.push({ text: 'Adjustment Amount', bold: true, fillColor: '#dddddd', alignment: 'center' });
       }      
     });
     this.billAdjustmentDTOList.push(tableHeader);
     console.log(this.billAdjustmentDTOList);
     this.billAdjustmentList.forEach((billAdjustmentList, i) => {
       debugger;
       let tableRows: any[] = []
       this.selection.value.forEach(element => {
         if (element === "sr") {
           tableRows.push({ text: (i + 1), alignment: 'right' });
         } else if (element === "txtCustomerCode") {
           tableRows.push({ text: billAdjustmentList.txtCustomerCode, alignment: 'left' });
         } else if (element === "txtBillNo") {
           tableRows.push({ text: billAdjustmentList.txtBillNo, alignment: 'left' });
         } else if (element === "dteBillFrom") {
           tableRows.push({ text: billAdjustmentList.dteBillFrom, alignment: 'left' });
         } else if (element === "dteBillTo") {
           tableRows.push({ text: billAdjustmentList.dteBillTo, alignment: 'left' });
         }else if (element === "numTotalAmount") {
           tableRows.push({ text: billAdjustmentList.numTotalAmount, alignment: 'left' });
         }  else if (element === "numAdjustment") {
           tableRows.push({ text: billAdjustmentList.numAdjustment, alignment: 'left' });
         } 
       });
       this.billAdjustmentDTOList.push(tableRows)
     });
     debugger;
 
     const documentDefinition = {
       pageOrientation: 'landscape',
       content: [
         {
           style: 'tableExample',
           table: {
             headerRows: 1,
             // widths: [200, 170, 200, 170],
             body: [
               [{
                 image: 'data:image/png;base64,',
               }, [{ text: 'Bill Adjustment', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
               {
                 image: 'data:image/png;base64,', width: 100,
                 height: 100,
                 alignment: 'right', margin: [0, 0, -80, 0],
               }],
             ]
           },
           layout: 'noBorders'
         },
         {
           style: 'tableExample',
           table: {
             headerRows: 1,
             body: this.billAdjustmentDTOList
           }
         },
       ],
       styles: {
         header: {
           fontSize: 18,
           bold: true,
           margin: [0, 0, 0, 10]
         },
         subheader: {
           fontSize: 16,
           bold: true,
           margin: [0, 10, 0, 5]
         },
         tableExample: {
           margin: [0, 5, 0, 15]
         },
         tableHeader: {
           bold: true,
           fontSize: 10,
           color: 'black'
         },
         tableHeaderValue: {
           fontSize: 10,
           color: 'black'
         }
       },
       defaultStyle: {
         // alignment: 'justify'
       }
     };
 
     pdfMake.createPdf(documentDefinition).download('Bill Adjustment List.pdf');
   }
 

   tableColumn: string[] = [
     "sr",
     "txtCustomerCode",
     "txtBillNo",
     "dteBillFrom",
     "dteBillTo",
     "numTotalAmount", 
     "numAdjustment"
   ];
   selection = new FormControl([
     "sr",
     "txtCustomerCode",
     "txtBillNo",
     "dteBillFrom",
     "dteBillTo",
     "numTotalAmount", 
     "numAdjustment"
   ]);
   selectingColumns() {
     debugger;
     if (this.selection.value.length > 0) {
       this.tableColumn.forEach(singleColumn => {
         let display = "";
         if (singleColumn !== "sr") {
           let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
           if (columnSelected >= 0) {
             display = "table-cell";
           } else {
             display = "none";
           }
           const slides = document.getElementsByClassName('mat-column-' + singleColumn);
           for (let i = 0; i < slides.length; i++) {
             const slide = slides[i] as HTMLElement;
             slide.style.display = display;
           }
         }
       });
     }
   }
}
