import { AfterViewInit, Component, OnInit, ViewChild } from '@angular/core';
import { FormGroup,FormBuilder, FormControl } from '@angular/forms';
import {MatPaginator} from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import {MatTableDataSource} from '@angular/material/table';
import { Router } from '@angular/router';
import { BillingService } from '@app/services/billing.service';
import { CrudService } from '@app/_services/crud.service';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
export interface PeriodicElement {
  branch: string;
  unionCouncil: number;
  consumerId: string;
  connectionType: string;
  suspensionDate: string;
}

const Table_DATA: PeriodicElement[] = [
  {consumerId: '1', branch: 'A', unionCouncil: 1, connectionType: 'Domestic', suspensionDate: '3/9/2020'},
  {consumerId: '13',  branch: 'A', unionCouncil: 4, connectionType: 'Domestic', suspensionDate: '3/9/2020'},
  {consumerId: '21', branch: 'C', unionCouncil: 6, connectionType: 'Aquifer', suspensionDate: '6/8/2019'},
  {consumerId: '4', branch: 'B', unionCouncil: 9, connectionType: 'Commercial', suspensionDate: '6/8/2019'},
  {consumerId: '67', branch: 'C', unionCouncil: 10, connectionType: 'Aquifer', suspensionDate: '6/2/2019'},
  {consumerId: '23', branch: 'C', unionCouncil: 12, connectionType: 'Domestic', suspensionDate: '3/9/2012'},
  {consumerId: '7', branch: 'B', unionCouncil: 14, connectionType: 'Commercial', suspensionDate: '6/8/2019'},
  {consumerId: '78', branch: 'A', unionCouncil: 15, connectionType: 'Aquifer', suspensionDate: '6/1/2019'},
  {consumerId: '72', branch: 'B', unionCouncil: 18, connectionType: 'Domestic', suspensionDate: '3/9/2020'},
  {consumerId: '34', branch: 'A', unionCouncil: 20, connectionType: 'Aquifer', suspensionDate: '6/12/2016'},
];

@Component({
  selector: 'app-dashboard',
  templateUrl: './connectionresumption.component.html'
})
export class ConnectionresumptionComponent implements AfterViewInit {

  branchList:any [] = [];
  unionCouncilList:any [] = [];
  connectionTypeList:any [] = [];
  step = 0;
  searchResumeForm: FormGroup;

  statusList:any [] = [];
  responseList:any [] = [];

  resumeColumns: string[] = [ 'txtCustomerCode','branch', 'unionCouncil', 'txtBusinessName', 'txtConnectionType', 'suspensionDate','Action'];
  resumeListData = new MatTableDataSource();
  @ViewChild("resumeSort", { static: true }) resumeSort: MatSort;
  @ViewChild("resumePaginator", { static: true }) resumePaginator: MatPaginator

  ngAfterViewInit() {
  }


  constructor(private formBuilder: FormBuilder, private billingService: BillingService,
    private shared: CrudService, private router: Router) { }

  ngOnInit(): void {
    this.searchResumeForm = this.formBuilder.group({
      txtCustomerCode: [''],
      branch: [''],
      unionCouncil: [''],
      txtConnectionType:[''],
      suspensionDate: ['']
    })

    this.getAllCustomers();
    this.billingService.getAllCustomerTemplateService().subscribe( response=>{
      debugger;
      console.log(response["branchList"]);
      this.branchList = response["branchList"];

      console.log(response["unionConcilList"]);
      this.unionCouncilList = response["unionConcilList"];

      console.log(response["connectionTypeList"]);
      this.connectionTypeList = response["connectionTypeList"];
    })
  }

  getAllCustomers(){
    this.billingService.getAllCustomerService().subscribe( 
      response => { 
        debugger;
        console.log("getAllSuspensionService")
        console.log(response.data)
        response.data.forEach(element => {
          debugger;
          if(element.txtStatus === "SUSPENDED"){
            this.statusList.push(element)
          }
        });
        this.resumeListData = new MatTableDataSource(this.statusList);
        this.resumeListData.sort = this.resumeSort;
        this.resumeListData.paginator = this.resumePaginator;
        this.resumeConnectionList = this.statusList;

    })
  }

  onSubmit(){
    debugger;
    console.log(this.searchResumeForm.value)
    let formValue = this.searchResumeForm.value;
    formValue.branch = {"id": + this.searchResumeForm.controls.branch.value }
    formValue.unionCouncil = { "id": +this.searchResumeForm.controls.unionCouncil.value }
    let code = this.searchResumeForm.controls.txtCustomerCode.value;
    let brachID = this.searchResumeForm.controls.branch.value;
    let UcID = this.searchResumeForm.controls.unionCouncil.value;
    let connType = this.searchResumeForm.controls.txtConnectionType.value;
    let resumeDate = this.searchResumeForm.controls.suspensionDate.value;

    this.billingService.searchSuspendCustomerListServiceByCode(code, brachID, UcID, connType ,resumeDate).subscribe( response => {
      debugger;
      console.log(response);
      if(response["count"]== 0){
        this.statusList = [];
        this.resumeListData = new MatTableDataSource(this.statusList);
        alert('No Record Found');  
      }
      else{
        this.statusList = [];
        this.responseList = response["data"];
        this.responseList.forEach(element => {
          debugger;
          if(element.txtStatus === "SUSPENDED"){
            this.statusList.push(element)
          }
          this.resumeListData = new MatTableDataSource(this.statusList);
          this.resumeListData.sort = this.resumeSort;
          this.resumeListData.paginator = this.resumePaginator;
          this.resumeConnectionList = this.statusList;
        })
        if(this.statusList.length == 0){
          this.statusList = [];
          this.resumeListData = new MatTableDataSource(this.statusList);
          alert('No Record Found'); 
        }
      }
    })
  }

  onReset(){
    // this.getAllCustomers();
    this.searchResumeForm.reset();
  }

  onView(element){
    this.shared.changeMessage(element);
    this.router.navigate(['billing/connectionResumption/resumeConnection']);
  }

  clearSuspenionDate(){
    this.searchResumeForm.controls.suspensionDate.reset('');
  }
  resumeConnectionList: any = "";
  resumeConnectionDTOList: any[] = [];
  
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.resumeConnectionDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        tableHeader.push({ text: 'Sr#', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtCustomerCode") {
        tableHeader.push({ text: 'Consumer Id', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "branch") {
        tableHeader.push({ text: 'Branch', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "unionCouncil") {
        tableHeader.push({ text: 'Union Council', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } else if (element === "txtBusinessName") {
        tableHeader.push({ text: 'Name', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }else if (element === "txtConnectionType") {
        tableHeader.push({ text: 'Connection Type', bold: true, fillColor: '#dddddd', alignment: 'center' });
      } 
      else if (element === "suspensionDate") {
        tableHeader.push({ text: 'Suspended Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }      
    });
    this.resumeConnectionDTOList.push(tableHeader);
    console.log(this.resumeConnectionDTOList);
    this.resumeConnectionList.forEach((resumeConnectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          tableRows.push({ text: (i + 1), alignment: 'right' });
        } else if (element === "txtCustomerCode") {
          tableRows.push({ text: resumeConnectionList.txtCustomerCode, alignment: 'left' });
        } else if (element === "branch") {
          tableRows.push({ text: resumeConnectionList.branch.name, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: resumeConnectionList.unionCouncil.name, alignment: 'left' });
        } else if (element === "txtBusinessName") {
          tableRows.push({ text: resumeConnectionList.txtBusinessName, alignment: 'left' });
        }else if (element === "txtConnectionType") {
          tableRows.push({ text: resumeConnectionList.txtConnectionType, alignment: 'left' });
        }  else if (element === "suspensionDate") {
          tableRows.push({ text: resumeConnectionList.suspensionDate, alignment: 'left' });
        } 
      });
      this.resumeConnectionDTOList.push(tableRows)
    });
    debugger;

    const documentDefinition = {
      pageOrientation: 'landscape',
      content: [
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            // widths: [200, 170, 200, 170],
            body: [
              [{
                image: 'data:image/png;base64,',
              }, [{ text: 'Connection Resumption ', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
                image: 'data:image/png;base64,', width: 100,
                height: 100,
                alignment: 'right', margin: [0, 0, -30, 0],
              }],
            ]
          },
          layout: 'noBorders'
        },
        {
          style: 'tableExample',
          table: {
            headerRows: 1,
            body: this.resumeConnectionDTOList
          }
        },
      ],
      styles: {
        header: {
          fontSize: 18,
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subheader: {
          fontSize: 16,
          bold: true,
          margin: [0, 10, 0, 5]
        },
        tableExample: {
          margin: [0, 5, 0, 15]
        },
        tableHeader: {
          bold: true,
          fontSize: 10,
          color: 'black'
        },
        tableHeaderValue: {
          fontSize: 10,
          color: 'black'
        }
      },
      defaultStyle: {
        // alignment: 'justify'
      }
    };

    pdfMake.createPdf(documentDefinition).download('Connection Resumption List.pdf');
  }

   // ['txtperformanceAreaName','txtperformanceAreaCode', 
    // 'numkpiCode', 'txtKpiName', 'numMinValue', 'numMaxValue', 
    // 'txtFormula', 'txtUom', 'txtCostCenter', 
  tableColumn: string[] = [
    "sr",
    "txtCustomerCode",
    "branch",
    "unionCouncil",
    "txtBusinessName",
    "txtConnectionType", 
    "suspensionDate"
  ];
  selection = new FormControl([
    "sr",
    "txtCustomerCode",
    "branch",
    "unionCouncil",
    "txtBusinessName",
    "txtConnectionType", 
    "suspensionDate"
  ]);
  selectingColumns() {
    debugger;
    if (this.selection.value.length > 0) {
      this.tableColumn.forEach(singleColumn => {
        let display = "";
        if (singleColumn !== "sr") {
          let columnSelected = this.selection.value.findIndex(x => x === singleColumn)
          if (columnSelected >= 0) {
            display = "table-cell";
          } else {
            display = "none";
          }
          const slides = document.getElementsByClassName('mat-column-' + singleColumn);
          for (let i = 0; i < slides.length; i++) {
            const slide = slides[i] as HTMLElement;
            slide.style.display = display;
          }
        }
      });
    }
  }
}
