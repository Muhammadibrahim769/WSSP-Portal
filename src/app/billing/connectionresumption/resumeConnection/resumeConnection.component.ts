...
@Component({
  selector: 'app-dashboard',
    templateUrl: './resumeConnection.component.html'
})
export class ResumeConnectionComponent implements OnInit {
    code: any;
    step = 0;
    show = false;
    ...
    constructor(private dialog: MatDialog, private billingService: BillingService,
      private router: Router, private shared: CrudService,private utilsFunctions: UtilsFunctions) { }

    ngOnInit() {
      debugger;
      this.shared.currentMessage.subscribe(response => {
        debugger;
        this.billingService.getSuspendedCustomerByCode(response["txtCustomerCode"]).subscribe(data=>{
    ...
    openResumeDialog(){
      ...
      console.log("this.resumeInfo");    
      console.log(this.resumeInfo);
      this.billingService.resumeConnectionService(this.resumeInfo.txtCustomerCode).subscribe(response=>{
        console.log(response);
        this.router.navigate(['billing/connectionResumption']);
      })    
    }
        ...
        content: [
          {
            ...
            table: {
              ...
              },  [{ text: 'Resume Connection', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
  ...
  //  { text: 'New Connection:', fontSize: 14, bold: true },
            ...
            table: {
              ...
              body: [
                [
                  ...
                  { text: this.resumeInfo.branch.name, style: 'tableHeaderValue' },
                  { text: 'Union Council:', style: 'tableHeader' },
                  { text: this.resumeInfo.unionCouncil.name , style: 'tableHeaderValue' },
  
                ],
                [
                  { text: 'Connection Type:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtConnectionType, style: 'tableHeaderValue' },
                  { text: 'Consumer ID:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtCustomerCode, style: 'tableHeaderValue' } 
  
                ],
                [
  
                  { text: 'Bussiness Name:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtBusinessName, style: 'tableHeaderValue' },
                  { text: 'Cnic:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtCnicNo,style: 'tableHeaderValue' }
  
                ],
                [
  
                  { text: 'NTN:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtNtnNo, style: 'tableHeaderValue' },
                  { text: 'STN:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtStnNo,style: 'tableHeaderValue' }
  
                ],
                 [
  
                  { text: 'Area:', style: 'tableHeader' },
                  { text: this.resumeInfo.txtShippingAddress, style: 'tableHeaderValue' },
                  { text: 'Sector Mohallah', style: 'tableHeader' },
                  { text: this.resumeInfo.txtMohallah, style: 'tableHeaderValue' },
                ...
                [
                  ...
                  { text: this.resumeInfo.txtStreetNo, style: 'tableHeaderValue' },
                  { text: 'House No', style: 'tableHeader' },
                  { text: this.resumeInfo.txtHouseNo, style: 'tableHeaderValue' },
                ...
                [
                  ...
                  { text: this.resumeInfo.txtMobileNo, style: 'tableHeaderValue' },
                  { text: 'Status', style: 'tableHeader' },
                  { text: this.resumeInfo.txtStatus, style: 'tableHeaderValue' },  
                ],


                // [  
                //   { text: 'Tender Description:', style: 'tableHeader' },
                //   { text: this.resumeInfo.txtTenderDescription, style:'tableHeaderValue', colSpan: 3 },
      ...
      pdfMake.createPdf(documentDefinition).download('Resume Connection Detail.pdf');
    }
}

// if(this.resumeInfo["txtStatus"] === "SUSPENDED"){
//     let id = this.resumeInfo.customerId;
//     this.resumeInfo["txtStatus"] ="ACTIVE"
//     this.billingService.updateCustomerListServiceByID(id, this.resumeInfo).subscribe(response =>{
//       debugger;
//       response = this.resumeInfo;
