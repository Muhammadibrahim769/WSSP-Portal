...
@Component({
  ...
  templateUrl:'./suspendedList.component.html'
})
export class SuspendedListComponent implements AfterViewInit {
  ...
  suspensionColumns: string[] = ['txtCustomerCode', 'branch', 'unionCouncil', 'txtBusinessName', 'txtConnectionType', 'suspensionDate', 'Actions'];
  ...
  onReset(){
    // this.getAllCustomers();
    this.searchSuspendForm.reset();
  }

  onView(element){
    debugger;
    this.shared.changeMessage(element);
    this.router.navigate(['billing/viewSuspendInformation']);
  }

  openSuspendApproved(element){
    console.log(element)
    const document = this.dialog.open(SuspendApprovedModalComponent, {
      data: {
        element,
    ...
    document.afterClosed().subscribe(() => {
        console.log("openSuspendApproved closed");
        this.getAllCustomers();
    });
  }

  openSuspendReject(element){
    console.log(element)
    const document = this.dialog.open(SuspendRejectModalComponent, {
      data: {
        element,
        message: 'Are you sure want to reject?',
        buttonText: {
    ...
    document.afterClosed().subscribe(() => {
        console.log("openSuspendReject closed");
        this.getAllCustomers();
    });
  }
  
  
  suspendConnectionList: any = "";
  suspendConnectionDTOList: any[] = [];
  
  downloadPDF() {
    debugger;
    console.log(this.selection);
    this.suspendConnectionDTOList = [];
    let tableHeader: any[] = []
    this.selection.value.forEach(element => {
      if (element === "sr") {
        ...
        tableHeader.push({ text: 'Suspension Date', bold: true, fillColor: '#dddddd', alignment: 'center' });
      }      
    });
    this.suspendConnectionDTOList.push(tableHeader);
    console.log(this.suspendConnectionDTOList);
    this.suspendConnectionList.forEach((suspendConnectionList, i) => {
      debugger;
      let tableRows: any[] = []
      this.selection.value.forEach(element => {
        if (element === "sr") {
          ...
          tableRows.push({ text: suspendConnectionList.txtCustomerCode, alignment: 'left' });
        } else if (element === "branch") {
          tableRows.push({ text: suspendConnectionList.branch.name, alignment: 'left' });
        } else if (element === "unionCouncil") {
          tableRows.push({ text: suspendConnectionList.unionCouncil.name, alignment: 'left' });
        } else if (element === "txtBusinessName") {
          tableRows.push({ text: suspendConnectionList.txtBusinessName, alignment: 'left' });
        }else if (element === "txtConnectionType") {
          tableRows.push({ text: suspendConnectionList.txtConnectionType, alignment: 'left' });
        }  else if (element === "suspensionDate") {
          tableRows.push({ text: suspendConnectionList.suspensionDate, alignment: 'left' });
        } 
      });
      this.suspendConnectionDTOList.push(tableRows)
    });
    debugger;
      ...
      content: [
        {
          ...
          table: {
            ...
            body: [
              ...
              }, [{ text: 'Suspend Connection', style: 'dateHeader', alignment: 'center', margin: [200, 60, 150, 0], bold: true, fontSize: 16 }],
              {
        ...
        },
        {
          ...
          table: {
            headerRows: 1,
            body: this.suspendConnectionDTOList
          }
        },
      ],
      styles: {
    ...
    pdfMake.createPdf(documentDefinition).download('Suspend Connection List.pdf');
  }
